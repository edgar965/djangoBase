# -*- coding: utf-8 -*-
"""Hilfe → Klassenmodell → Statistik: die „Übrigen" einer Endung löschen.
...
class UebrigePutzView(ZugriffMixin, View):
    ...
    def _endung(self, roh):
        """Die Endung aus der Anfrage — geprüft, nie ein Pfad.
    ...
    def _endungen(self, rohliste):
        """Mehrere Endungen prüfen — alle oder keine.
    ...
    def get(self, request):
        """Vorschau — für eine Endung (``endung=``) oder mehrere
        (``endungen=.log,.dump``)."""
        ...
        if viele:
            ...
            if endungen is None:
                return JsonResponse({"fehler": "Unzulässige Auswahl"}, status=400)
            return JsonResponse(suche.vorschau_mehrere(endungen))
        endung = self._endung(request.GET.get("endung"))
        if endung is None:
            return JsonResponse({"fehler": "Unzulässige Endung"}, status=400)
    ...
    def post(self, request):
        try:
            ...
            return JsonResponse({"fehler": "Ungültiges JSON"}, status=400)
        ...
        if "endungen" in daten:
            ...
            if isinstance(erwartet, int) and erwartet != gefunden:
                return JsonResponse(
                    {
                        "fehler": (
                            "Der Bestand hat sich geändert: erwartet %d, "
                            "gefunden %d. Bitte neu ansehen." % (erwartet, gefunden)
                        ),
                        "anzahl": gefunden,
                    },
                    status=409,
            ...
            return JsonResponse(
                {
                    "fehler": (
                        "Der Bestand hat sich geändert: erwartet %d, "
                        "gefunden %d. Bitte neu ansehen." % (erwartet, len(treffer))
                    ),
                    "anzahl": len(treffer),
                },
                status=409,
