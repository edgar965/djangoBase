# -*- coding: utf-8 -*-
"""Hilfe -> Skills: DER Werkzeugkasten.
...
from ..skills import Bericht, Umbaunetz, fixer, fixer_finden, werkzeuge
from ..skills.lehren_review import BEREICHE, LEHREN, Lehrenstand
from ..skills.rangliste import fixerrangliste, lehrenrangliste, rangliste
...
class SkillsView(ZugriffMixin, View):
    def get(self, request):
        ...
        if request.GET.get("auftrag") == "1":
            ...
            return HttpResponse(Lehrenstand.auftragstext(), content_type="text/plain; charset=utf-8")
    ...
    def post(self, request):
        ...
        if request.POST.get("aktion") == "fixrang":
            ...
            return redirect("%s?verschoben=%s#fixer" % (request.path, slug if ok else ""))
        if request.POST.get("aktion") == "lehrenrang":
            slug = request.POST.get("rang_slug", "")
            ziel = request.POST.get("rang_ziel", "")
            ok = lehrenrangliste().verschieben(slug, ziel, list(LEHREN))
            return redirect("%s?verschoben=%s#lehren" % (request.path, slug if ok else ""))
        ...
        gewaehlt = request.POST.getlist("werkzeug") or self._bereich_slugs(request.POST.get("bereich"))
    ...
    def _fix(self, request):
        ...
        if bereich:
            ...
            if ziel != basis and basis not in ziel.parents:
                return self._seite(
                    request,
                    "Bereich '%s' liegt außerhalb des Projekts — abgelehnt." % bereich,
                    [],
                    fix={"slug": slug, "bereich": bereich, "n": 0, "modus": "vorschau"},
                )
        ...
        try:
            ...
            return self._seite(request, "FEHLER beim Fixen: %s: %s" % (type(e).__name__, e), [])
        return self._seite(request, text, [], fix={"slug": slug, "bereich": bereich, "n": n, "modus": modus})

    @staticmethod
    def _fix_vorschau(fixer, v):
        zeilen = [
            "VORSCHAU: %s" % fixer.titel,
            v.als_dict()["zusammenfassung"],
        ...
        for a in v.aenderungen:
            zeilen.append(
                "  %-52s %s%s"
                % (a.name, a.was, "" if a.machbar else "  [BLOCKIERT: %s]" % "; ".join(a.warnungen))
            )
        if v.hinweis:
            zeilen += ["", v.hinweis]
        return "\n".join(zeilen)
    ...
    def _fix_bericht(fixer, erg):
        zeilen = [
            "ANGEWENDET: %s" % fixer.titel,
            "%d geschrieben, %d zurueckgespielt, %d übersprungen."
            % (len(erg["geschrieben"]), len(erg["zurueckgespielt"]), len(erg["uebersprungen"])),
            "",
        ]
        for e in erg["geschrieben"]:
            zeilen.append("  OK          %-46s Sicherung: %s" % (e["datei"], e["sicherung"]))
        ...
        zeilen += [
            "",
            "Rueckgaengig: die Originale liegen im Sicherungsordner "
    ...
    def _laufen(self, slugs, daten, bericht):
        ...
        for w in gewaehlt:
            ...
            if eingabe:
                feld = eingabe[0]
                argumente[feld] = daten.get("arg_%s" % w.slug) or daten.get(feld) or eingabe[2]
            erg, dauer = self._ausfuehren(w, argumente)
            bericht.anhaengen(w, erg, dauer, datetime.now().strftime("%d.%m.%Y %H:%M:%S"))
    ...
    def _seite(self, request, ausgabetext, gelaufen, fix=None):
        return render(
            ...
            "djangobase/hilfe/skills.html",
            {
                ...
                "netz": {
                    "titel": Umbaunetz.titel,
                    "tut": Umbaunetz.tut,
                    "warum": Umbaunetz.warum,
                    "grenzen": Umbaunetz.grenzen,
                },
    ...
    @staticmethod
    def _lehren():
        """Die Lehren je Bereich, mit ihrem Ankreuzstand."""
        ...
        for bereich in BEREICHE:
            eintraege = [
                {"lehre": lehre, "an": stand.get(lehre.slug, True)}
                for lehre in LEHREN
                if lehre.bereich == bereich
            ]
            if eintraege:
                # Dictionary gewollt: geht unveraendert in die Vorlage.
                aus.append({"name": bereich, "lehren": eintraege, "anzahl": len(eintraege)})
        return aus

    @staticmethod
    def _gruppenkopf(nummer, bereich, anzahl):
        """Abschnitts-Zeile der Tabelle — mit eigenem Sammellauf-Knopf.
        ...
        return (
            ...
            % (escape(bereich["name"]), escape(bereich["warum"]), nummer, anzahl)
        )

    @classmethod
    def _bereich_slugs(cls, nummer):
        """Die Werkzeuge EINES Abschnitts — für den Knopf in seiner Zeile."""
        try:
            nummer = int(nummer)
    ...
    def _tabelle(self, gelaufen):
        ...
        for nummer, abschnitt in enumerate(rang.abschnitte(alle)):
            ...
            zeilen.append(
                {
                    "gruppe": True,
                    "zellen": [{"html": self._gruppenkopf(nummer, b, len(eintraege)), "colspan": 6}],
                }
            )
    ...
    def _zeile(self, w, nr, gelaufen):
        """Eine Werkzeug-Zeile. Herausgeloest, weil _tabelle sonst über
        ...
        return {
            ...
            "zellen": [
                {
                    "html": (
                        '<input type="checkbox" name="werkzeug" value="%s"'
                        ' class="sk1-wahl"%s>' % (escape(w.slug), " checked" if w.slug in gelaufen else "")
                    ),
                    "sort": 1 if w.slug in gelaufen else 0,
                    "klasse": "sk1-mitte",
                },
                ...
                {
                    "html": (
                        '<input type="number" name="rang_ziel" value="%d"'
                        ...
                        '<input type="hidden" name="rang_slug" value="%s">' % (nr, escape(w.slug))
                    ),
                    "sort": nr,
                    "klasse": "sk1-mitte",
                },
                    ...
                    "html": (
                        ...
                        % (
                            ...
                            (
                                ...
                                if getattr(w, "ruft_endpunkte_auf", False)
                                else ""
                            ),
                        )
                    ),
                ...
                {
                    "html": (
                        '<button type="submit" name="werkzeug" value="%s"'
    ...
    def _fixtabelle(self, fix):
        """Die Fix-Werkzeuge in derselben Tabelle wie die Prüfer.
    ...
    def _fixzeile(self, f, nr, fix):
        ...
        p = d.get("pruefung")
        behebt = (
            ('Nr. %d<span class="sk1-dauer">%s</span>' % (p["nr"], escape(p["titel"])))
            if p
            else '<span class="sk1-leise">—</span>'
        )
        offen = fix and fix.get("modus") == "vorschau" and fix.get("slug") == f.slug and fix.get("n")
        knoepfe = (
            '<button type="submit" name="fix" value="vorschau:%s"'
            ' class="sk1-knopf" formnovalidate>'
            '<i class="bi bi-search"></i> Vorschau</button>' % escape(f.slug)
        )
        if offen:
            knoepfe += (
                '<button type="submit" name="fix" value="anwenden:%s"'
            ...
            "zellen": [
                {
                    "html": (
                        ...
                        ' title="Auf diese Nummer verschieben">'
                        '<i class="bi bi-arrow-right-short"></i></button>'
                        '<input type="hidden" name="rang_slug" value="%s">' % (nr, escape(f.slug))
                    ),
                    "sort": nr,
                    "klasse": "sk1-mitte",
                },
                {
                    "html": (
                        '<span class="sk1-name">%s</span>'
                        '<span class="sk1-dauer">%s</span>' % (escape(f.titel), escape(d["dauer"]))
                    ),
                    "sort": f.titel,
                },
                {"html": behebt, "sort": p["nr"] if p else 999},
                {"html": escape(d["tut"])},
                {"html": '<span class="sk1-leise">%s</span>' % escape(d["grenzen"])},
    ...
    def _lehrentabelle(self):
        """Die Lehren in derselben Tabelle wie Prüfer und Fixer.
        ...
        alle = list(LEHREN)
        da = {lehre.slug: lehre for lehre in alle}
        zeilen = []
        for nr, slug in enumerate(rang.reihenfolge(alle), start=1):
            lehre = da.get(slug)
            if lehre is not None:
                zeilen.append(self._lehrezeile(lehre, nr, stand.get(slug, True)))
        return {
            "key": "db-lehren",
            "spalten": [
                {
                    "label": ('<input type="checkbox" id="sk-lehre-alle" title="Alle aus- oder abwaehlen">'),
                    "key": "wahl",
                    "sortAus": True,
                },
                {
                ...
                {"label": "Prüfung", "key": "pruefung", "titel": "Welches Werkzeug hält diese Regel?"},
            ],
            "zeilen": zeilen,
    ...
    def _lehrezeile(lehre, nr, an):
        ...
        if nummern:
            pruefung = " · ".join(
                'Nr. %d<span class="sk1-dauer">%s</span>' % (r, escape(titel)) for r, titel in nummern
            )
        else:
            # KEINE PRUEFUNG IST EINE AUSSAGE, keine fehlende Angabe: Diese
            # Regel haengt allein an der Sorgfalt dessen, der schreibt.
            pruefung = '<span class="sk1-leise">keine — diese Regel prüft kein Werkzeug</span>'
        text = '<span class="sk1-fixtut">%s</span>' % escape(lehre.regel)
        text += '<span class="sk1-dauer"><b>Warum:</b> %s</span>' % escape(lehre.warum)
        if lehre.beleg:
            text += '<span class="sk1-dauer"><b>Beleg:</b> %s</span>' % escape(lehre.beleg)
        return {
            "klasse": "" if an else "db-aus",
            "zellen": [
                {
                    "html": (
                        '<input type="checkbox" name="lehre" value="%s"'
                        ' class="sk-lehre-wahl"%s>' % (escape(lehre.slug), " checked" if an else "")
                    ),
                    "sort": 1 if an else 0,
                    "klasse": "sk1-mitte",
                },
                {
                    "html": (
                        ...
                        ' verschieben">'
                        '<i class="bi bi-arrow-right-short"></i></button>'
                        '<input type="hidden" name="rang_slug" value="%s">' % (nr, escape(lehre.slug))
                    ),
                    "sort": nr,
                    "klasse": "sk1-mitte",
                },
                {"html": escape(lehre.bereich), "sort": lehre.bereich},
                {"html": '<span class="sk1-name">%s</span>' % escape(lehre.titel), "sort": lehre.titel},
                {"html": text},
                {"html": pruefung, "sort": nummern[0][0] if nummern else 999},
            ],
        }

    def _rahmen(self, zeilen):
        """Kopf und Rumpf der Tabelle - getrennt vom Zeilenbau, damit
        ...
        return {
            ...
            "spalten": [
                {
                    "label": ('<input type="checkbox" id="sk1-alle" title="Alle aus- oder abwählen">'),
                    "key": "wahl",
                    "sortAus": True,
                },
    ...
    def _eingabefeld(w):
        ...
        return '<input type="text" name="arg_%s" value="%s" size="8" title="%s" class="sk1-arg">' % (
            escape(w.slug),
            escape(vorgabe),
            escape(beschriftung),
        )
