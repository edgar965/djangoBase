# -*- coding: utf-8 -*-
"""Hilfe · Werkzeug Klassenmodell — das Objektmodell des Projekts als Bild.
...
from ..umbau import (
    ablage,
    aufrufnetz as aufrufnetz_modul,
    codequalitaet as codequalitaet_modul,
    codezahlen as codezahlen_modul,
    globalbestand as globalbestand_modul,
    klassenmodell as klassenmodell_modul,
)
...
class Letzterlauf:
    """Was zuletzt gezeigt wurde — Reiter, Quelle, Startklasse.
    ...
    def merken(cls, daten):
        ablage.schreiben(
            cls.BEREICH,
            cls.SCHLUESSEL,
            dict((k, daten.get(k, "")) for k in ("reiter", "bereich", "start", "tiefe", "was")),
        )


class Modellspeicher(Speicher):
    """Hält das eingelesene Modell, bis jemand ausdruecklich neu liest.
...
class Quellenspeicher(Speicher):
    """Die waehlbaren Quellen — einmal gezählt, dann gemerkt.
...
class Netzspeicher(Speicher):
    """Das Aufrufnetz — liest jede ``.py`` zweimal (Definitionen, dann
...
class Bestandsspeicher(Speicher):
    """Dasselbe für den Modulebenen-Bestand: einmal lesen, oft ansehen."""
...
class Zahlenspeicher(Speicher):
    """Die Bestandszahlen — der Durchgang geht über JEDE Datei, nicht
...
class Qualitaetsspeicher(Speicher):
    """Die Messung — vier Werkzeuge über 711 Dateien: gemessen **19
...
class KlassenmodellView(ZugriffMixin, View):
    """Zeigt die Seite; auf Knopfdruck rechnet sie das Bild."""

    vorlage = "djangobase/hilfe/klassenmodell.html"

    def get(self, request):
        """Zeigt den letzten Lauf, statt leer dazustehen.
        ...
        if letzter:
            try:
                ...
                logger.warning("letzten Lauf nicht wiederholbar", exc_info=True)
    ...
    def post(self, request):
        ...
        return self._zeigen(request, request.POST, rechnen=self._darf_rechnen(request.POST))

    @classmethod
    def _darf_rechnen(cls, daten):
        """Hat jemand einen Knopf gedrueckt — oder nur den Reiter gewechselt?
    ...
    def _aus_speicher(klasse, wurzel, neu, rechnen):
        """``(Wert, Alter)`` — rechnet nur, wenn gerechnet werden darf.
    ...
    def _zeigen(self, request, daten, rechnen=False):
        """Eine Sicht bauen — aus dem Formular ODER aus dem letzten Lauf.
        ...
        if reiter != "baum":
            bestand, alter = self._aus_speicher(Bestandsspeicher, wurzel, neu, rechnen)
            if bestand is None:
            ...
            if reiter in ("klassen", "funktionen"):
                ...
                netz, _n = self._aus_speicher(Netzspeicher, wurzel, neu, rechnen)
                if netz is None:
                    return self._leerer_reiter(request, reiter, daten)
                if reiter == "klassen":
                    ...
                    # Auswahlfeld.
                    modell, _a = self._aus_speicher(Modellspeicher, wurzel, neu, rechnen)
                    if modell is None:
                        return self._leerer_reiter(request, reiter, daten)
                    ...
                    zusatz["rollen"] = gliedern((e.name, e.datei) for e in bestand.funktionen)
        ...
        if neu:
            ...
            Netzspeicher.leeren()
        modell, alter = self._aus_speicher(Modellspeicher, wurzel, neu, rechnen)
        if modell is None:
            return self._leerer_reiter(request, "baum", daten)
        start = (daten.get("start") or "").strip() or None
        try:
            tiefe = max(1, min(4, int(daten.get("tiefe") or TIEFE_VORGABE)))
        ...
        return self._seite(
            request,
            bild=(Klassenbild(kaesten, linien, gewaehlt, steckbriefe).svg() if kaesten else ""),
    ...
    def _leerer_reiter(self, request, reiter, daten):
        """Der Reiter ohne Ergebnis — mit dem Knopf, der es holt.
    ...
    def _statistik(self, request, daten, wurzel, neu, rechnen):
        """Die Bestandszahlen — Dateien, Zeilen, Anweisungen je Art.
        ...
        """
        zahlen, alter = self._aus_speicher(Zahlenspeicher, wurzel, neu, rechnen)
        if zahlen is None:
        ...
        return self._seite(
            ...
            ausgelassen_wo=sorted(zahlen.ausgelassen_wo.items(), key=lambda p: -p[1]),
    ...
    def _qualitaet(self, request, daten, wurzel, neu, rechnen):
        """Die Messung mit vier Werkzeugen — und die Befundliste dazu.
        ...
        was = daten.get("was") or daten.get("was_zuletzt") or "qualitaet"
        if was not in ("qualitaet", "befunde"):
            was = "qualitaet"
        messung, alter = self._aus_speicher(Qualitaetsspeicher, wurzel, neu, rechnen)
        if messung is None:
        ...
        zusatz["pannen"] = [{"datei": d, "verfahren": v, "grund": g} for d, v, g in messung.pannen]
        return self._seite(
            request, reiter="qualitaet", alter=int(alter) if alter is not None else None, **zusatz
        )
    ...
    def _befundliste(cls, messung):
        """Alle Treffer aller Verfahren als EINE nach Gewicht sortierte Liste.
    ...
    def _stufenzaehler(cls, befunde):
        """``[(stufe, anzahl)]`` fuer den Kopf - auch die leeren Stufen."""
    ...
    @staticmethod
    def _wurzel(bereich):
        """Welcher Teil des Projekts wird gelesen?
    ...
    def _aeste(modell, wie_viele=12):
        """Die dicksten Aeste als Vorschlagsliste — Wer hält wie viele?"""
        gezaehlt = []
    ...
    def _seite(self, request, **zusatz):
        ...
        daten = {
            ...
            "aktiv": "klassenmodell",
            "reiter": "baum",
            "reiter_liste": [{"key": k, "label": label, "icon": i} for k, label, i in self.REITER],
