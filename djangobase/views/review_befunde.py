# -*- coding: utf-8 -*-
"""Hilfe -> Review: die gespeicherten Befunde eines Prüfwerkzeugs ausliefern.
...
class ReviewBefundeView(ZugriffMixin, View):
    """GET: die gespeicherten Befunde eines Werkzeug-Partners."""

    def get(self, request, slug):
        partner = self._partner(slug)
        if partner is None:
            return JsonResponse({"fehler": "Unbekanntes Werkzeug"}, status=404)
            ...
            return JsonResponse(
                {
                    ...
                    "belegt": False,
                    "hinweis": (
                        "Keine Ablage unter %s. Entweder lief hier noch nie "
                        "ein Review, oder der Serverprozess sieht ein anderes "
                        "Benutzerprofil als die Konsole (LOCALAPPDATA)." % lager.ablage
                    ),
                }
            )
        ...
        return JsonResponse(
            {
                ...
                if laeufe
                    ...
                    "Für %s liegen keine gespeicherten Befunde. Ein Lauf über "
                    "„Prüfen lassen“ legt sie an." % wurzel
                ),
            }
        )
    ...
    @staticmethod
    def _partner(slug):
        """Der konfigurierte Werkzeug-Partner zu diesem Slug — oder ``None``.
