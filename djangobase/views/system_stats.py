...
WOZU (übernommen aus shortlongx am 12.08.2026)
-----------------------------------------------
...
zum Stolpern bringt, wäre schlimmer als keine Anzeige.

DIE MESSUNG DAHINTER (shortlongx, 28.07.2026)
---------------------------------------------
Eine Aktualisierung kostet 0,22 s (``nvidia-smi`` rund 0,07 s +
``psutil.cpu_percent(interval=0.15)``). Die Leiste fragt während eines Laufs
...
def api_system_stats(request):
    ...
    Fällt die Messung aus - keine Grafikkarte, ``psutil`` fehlt, ``nvidia-smi``
    hängt -, soll die Seite darueber nicht rot werden, sondern die Leiste
