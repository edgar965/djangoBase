# -*- coding: utf-8 -*-
"""Karten - Titel, Sammelknopf und Tabelle einer Testcase-Liste.
...
class Karten:
    ...
    LEER = (
        "Keine Tests in dieser Liste. Entweder greifen die Labels in "
    ...
    def __init__(self, tabellen, bereiche=None):
        ...
        self.bereiche = bereiche or getattr(tabellen, "bereiche", None) or Bereiche.aus_einstellungen()
    ...
    def je_kategorie(self, tests, titel, key, tab, icon="bi-list-check", leer=None):
        """[karte] - eine Karte mit allen Fällen der Kategorie."""
    ...
    def _karte(self, titel, tests, key, tab, icon, leer):
        ...
        return {
            ...
            "tabelle": self.tabellen.tabelle(
                [Eintrag.aus_test(t) for t in tests], key=key, tab=tab, leer=leer or self.LEER
            ),
        }

    def eine(self, tabelle, titel, anzahl, icon="bi-collection-play"):
        """Eine einzelne Karte (Suiten) - ohne Sammelknopf und ohne Bereiche."""
        return [{"titel": titel, "anzahl": anzahl, "icon": icon, "tabelle": tabelle}]

    @staticmethod
    def label(tests):
        """Ein ``manage.py test``-Label, das GENAU diese Faelle faehrt.
