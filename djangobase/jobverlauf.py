# -*- coding: utf-8 -*-
"""Der Verlauf der Job-Laeufe: wann, wie lange, mit welchem Ausgang.
...
class Jobverlauf:
    """Liest und schreibt die Laufhistorie.
    ...
    @staticmethod
    def _standardpfad():
        """``BASE_DIR/logs/joblaeufe.jsonl`` - neben den Logdateien."""
    ...
    # ------------------------------------------------------------ schreiben
    def notieren(
        self, kennung, dauer_s, erfolg, fehler="", hinweis="", cpu_s=None, argumente=None, eigenstaendig=True
    ):
        """Einen beendeten Lauf festhalten.
    ...
    def _kuerzen_wenn_noetig(self):
        """Alte Zeilen wegwerfen, damit die Datei nicht unbegrenzt waechst."""
        try:
    ...
    def laeufe(self, kennung=None, hoechstens=None):
        """Alle Laeufe, juengster zuerst. ``kennung`` filtert auf einen Job."""
        raus = []
        try:
            if not self.pfad.exists():
                return raus
    ...
    def letzter(self, kennung):
        """Der juengste Lauf eines Jobs, oder ``None``."""
        treffer = self.laeufe(kennung, hoechstens=1)
        return treffer[0] if treffer else None

    def zusammenfassung(self):
        """Je Job eine Zeile: letzter Lauf, Dauer, Anzahl, Fehlerzahl.
        ...
        for eintrag in je_job.values():
            eintrag["dauer_schnitt"] = (
                eintrag["dauer_summe"] / eintrag["laeufe"] if eintrag["laeufe"] else 0.0
            )
        return je_job
