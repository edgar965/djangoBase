...
class ModulSchneider:
    ...
    def gebundene_namen(knoten):
        """Namen, die INNERHALB des Blocks entstehen — also nie „offen" sind.
    ...
    def bloecke(self, namen):
        ...
        if fehlend:
            raise KeyError("nicht in %s: %s" % (self.pfad.name, ", ".join(fehlend)))
        raus = []
        for name in namen:
            k = defs[name]
    ...
    def bedarf(self, namen):
        ...
        noetige_konstanten = {n: konstanten[n] for n in benutzt if n in konstanten and n not in importe}
        ...
        offen = {
            ...
            for n in benutzt
            if n not in importe
            and n not in konstanten
            and n not in defs
            and n not in gebunden
            and n not in eingebaut
    ...
    def schreiben(self, namen, ziel, kopf, zusatz_importe=(), trocken=False, ebenen_tiefer=0):
        ...
        for zeile in sorted(set(importe.values())):
            teile.append(self._ebene_anheben(zeile, ebenen_tiefer) if ebenen_tiefer else zeile)
        ...
        for _name, text, _a, _e in bloecke:
            ...
            if ebenen_tiefer:
                ...
                text = "\n".join(self._ebene_anheben(z, ebenen_tiefer) for z in text.split("\n"))
            teile.append(text)
            teile.append("")
        neu = "\n".join(teile).rstrip() + "\n"
        neu = neu.replace("\n\n\n\n", "\n\n\n")
        ...
        return {
            "ziel": str(ziel),
            "funktionen": len(bloecke),
            "zeilen_neu": neu.count("\n") + 1,
            "zeilen_rest": resttext.count("\n") + 1,
            "importe": sorted(importe),
            "konstanten": sorted(konstanten),
            "offen": sorted(offen),
        }


def main():
    """Kommandozeile — Probelauf ist die Vorgabe.
    ...
    bericht = ModulSchneider(argumente[0]).schreiben(namen, argumente[1], kopf, trocken=not schreiben)
    print(
        "%s  <- %d Definitionen, %d Zeilen (Rest: %d)"
        % (bericht["ziel"], bericht["funktionen"], bericht["zeilen_neu"], bericht["zeilen_rest"])
    )
