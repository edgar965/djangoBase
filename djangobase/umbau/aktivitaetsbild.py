# -*- coding: utf-8 -*-
"""Ein Ablauf als UML-Aktivitaetsdiagramm — Achse, Rauten, Pfeile.
...
class Teil:
    """Ein gezeichnetes Element mit Mittelpunkt und Groesse."""

    __slots__ = ("art", "text", "x", "y", "breite", "hoehe", "ziel", "zeile", "knoten")

    def __init__(self, art, text, x, y, breite, hoehe, ziel="", zeile=0, knoten=None):
        self.art = art
        self.text = text
        self.x = x  # Mitte
        self.y = y  # Oberkante
...
class Aktivitaetsbild:
    """Ein ``Ablauf`` im Stil eines UML-Aktivitaetsdiagramms."""
    ...
    def anordnen(self):
        ...
        self.breite = int(
            max(
                [t.rechts for t in self.teile]
                + [x for _a, x, _b, _c in [(0, k[0], 0, 0) for k in self.kanten]]
                + [achse + SPUR]
            )
            + RAND
        )
    ...
    def _eins(self, k, achse, y, voriges):
        ...
        if k.art == "absicherung":
            return self._folge(list(k.rumpf) + list(k.immer), achse, y, voriges)
        hoehe = KASTEN_H if k.art != "ende" else KASTEN_H
        art = "ausgang" if k.art == "ende" else "kasten"
        teil = Teil(art, text, achse, y, KASTEN_B, hoehe, getattr(k, "ziel", ""), k.zeile, k)
    ...
    def _frage(self, k, text, achse, y, voriges):
        """Raute auf der Achse, ``ja`` nach links, danach zurueck."""
        raute = Teil("frage", text, achse, y, RAUTE, RAUTE, "", k.zeile, k)
        ...
        if k.ja:
            ...
            voriges_links = None
            for kind in k.ja:
                y_links, voriges_links = self._eins(kind, neben, y_links, voriges_links)
        ...
        if ende_links is not None:
            ...
            self._kante(ende_links.x, ende_links.unten, balken.links, balken.y + 5, "")
    ...
    def _schleife(self, k, text, achse, y, voriges):
        """Rumpf auf der Achse, dazu ein Ruecklaufpfeil rechts."""
        balken = Teil("balken", "", achse, y, 120, 10)
        self.teile.append(balken)
        if voriges is not None:
        ...
        self._marke_setzen(raute, "[fertig]")
    ...
    def _marke_setzen(self, raute, marke):
        """Beschriftung an den geradeaus weiterlaufenden Ausgang."""
        self.kanten.append((raute.x, raute.unten, raute.x, raute.unten + LUFT * 0.6, marke, False, None))

    def _erster_ab(self, raute):
        """Das erste Teil, das nach dieser Raute in die Nebenspur kam."""
    ...
    def _marke(k, ja):
        text = getattr(k, "text", "")
        return "[%s]" % (text if ja else "sonst")
    ...
    def svg(self):
        ...
        aus = [
            ...
            '<rect x="0" y="0" width="%d" height="%d" style="fill:#ffffff"/>' % (self.breite, self.hoehe),
        ]
    ...
    def _pfeilspitze():
        return (
            '<defs><marker id="ak-spitze" markerWidth="9" '
            'markerHeight="9" refX="8" refY="3" orient="auto">'
            '<path d="M0,0 L8,3 L0,6 z" fill="%s"/></marker></defs>' % FARBEN["linie"]
        )

    @staticmethod
    def _stil():
        """Ein weisser Hof hinter jeder Beschriftung.
    ...
    def _marke_zeichnen(x1, y1, x2, y2, marke):
        """Die Beschriftung einer Kante — dorthin, wo Platz ist.
        ...
        * Ein senkrechter Weg hat gar keine Breite. Die Marke steht
        ...
        if waagerecht:
            ...
            stelle = 'x="%.0f" y="%.0f" text-anchor="middle"' % ((x1 + x2) / 2.0, min(y1, y2) - 9)
        else:
            kurz = Aktivitaetsbild._passend(marke, SPUR - 30)
            stelle = 'x="%.0f" y="%.0f"' % (max(x1, x2) + 10, (y1 + y2) / 2.0 + 4)
        ...
        return '<text class="ak-m" %s>%s%s</text>' % (stelle, titel, escape(kurz))

    @staticmethod
    def _passend(text, platz_px):
        """So viele Zeichen, wie in ``platz_px`` passen — rund 6 px je
        ...
        return text[: hoechstens - 1 - len(schluss)] + "…" + schluss
    ...
    def _teil_zeichnen(self, t):
        ...
        if t.art == "ende":
            m = t.y + KREIS / 2.0
            return [
                '<circle cx="%.0f" cy="%.0f" r="%d" fill="none" '
                'stroke="%s" stroke-width="1.5"/>' % (t.x, m, KREIS // 2, FARBEN["balken"]),
                '<circle cx="%.0f" cy="%.0f" r="%d" fill="%s"/>' % (t.x, m, KREIS // 2 - 4, FARBEN["balken"]),
            ]
        if t.art == "balken":
            return [
                '<rect x="%.0f" y="%.0f" width="%d" height="%d" '
                'fill="%s"/>' % (t.links, t.y, t.breite, t.hoehe, FARBEN["balken"])
            ]
        if t.art == "frage":
            ...
            inhalt = ['<polygon points="%s" fill="%s" stroke="%s"/>' % (punkte, fuell, rand)]
            return self._umhuellen(t, inhalt)
        fuell, rand, _t = FARBEN["kasten"]
        rund = 22 if t.art == "ausgang" else 6
        inhalt = [
            '<rect x="%.0f" y="%.0f" width="%d" height="%d" rx="%d" '
            'fill="%s" stroke="%s"/>' % (t.links, t.y, t.breite, t.hoehe, rund, fuell, rand)
        ]
        inhalt.extend(self._text(t))
        return self._umhuellen(t, inhalt)

    def _umhuellen(self, t, inhalt):
        """Die Herkunft als Daten an den Kasten haengen.
        ...
        if t.ziel:
            angaben["ziel"] = t.ziel
        felder = " ".join(
            'data-%s="%s"' % (name, escape(str(wert))) for name, wert in sorted(angaben.items()) if wert
        )
        if not felder:
            return inhalt
    ...
    def _text(self, t):
        """Bis zu zwei Zeilen mittig im Kasten."""
        zeilen = self._umbrechen(t.text)
        hoch = t.y + t.hoehe / 2.0 - (len(zeilen) - 1) * 8 + 5
        aus = [
            '<text class="ak-t" x="%.0f" y="%.0f" text-anchor="middle">'
    ...
    @staticmethod
    def _zeilennummer(t):
        """LINKS oben im Kasten — nie rechts.
        ...
        """
        return '<text class="ak-z" x="%.0f" y="%.0f">%d</text>' % (t.links + 9, t.y + 15, t.zeile)
