# -*- coding: utf-8 -*-
"""Einen Ablauf als SVG zeichnen — von oben nach unten, mit Rauten.
...
class Kasten:
    """Ein gezeichnetes Teil mit seinem Platz."""
...
class Ablaufbild:
    ...
    def anordnen(self):
        ...
        self.breite = int(max([k.x + k.breite for k in self.kaesten], default=BREITE) + RAHMEN)
        return self

    def _folge(self, knoten, x, y, marke=""):
        """Eine Folge untereinander setzen; gibt das neue ``y`` zurueck."""
    ...
    def svg(self):
        ...
        for x, y1, y2 in self.striche:
            teile.append('<path class="ab-ast" d="M%d %d L%d %d"/>' % (x, y1, x, y2))
    ...
    def _zeichnen(self, kasten):
        ...
        if kasten.marke:
            aus.append(
                '<text class="ab-m" x="%d" y="%d">%s</text>'
        ...
        if k.art == "frage":
            ...
            aus.append(
                '<rect class="ab-k" x="%d" y="%d" width="%d" '
                'height="%d" rx="%d" fill="%s" stroke="%s"/>'
                % (kasten.x, kasten.y, kasten.breite, hoch, rund, fuell, rand)
            )
        aus.append(
            '<text class="ab-t" x="%d" y="%d">%s</text>'
            % (kasten.x + 14, kasten.y + hoch / 2 + 4, escape(self._beschriftung(k)))
        )
        rechts = kasten.x + kasten.breite
        aus.append(
            '<text class="ab-z" x="%d" y="%d" text-anchor="end">%s</text>'
            % (
                rechts - 10,
                kasten.y + hoch / 2 + 4,
                escape("%s%d" % ("→ " + k.ziel + "  " if k.ziel else "", k.zeile)),
            )
        )
        if k.ziel:
            aus.append("</g>")
    ...
    def _raute(kasten, fuell, rand, hoch):
        """Eine Raute in Kastenbreite — sonst passt kein Text hinein.
        ...
        return '<polygon class="ab-k" points="%s" fill="%s" stroke="%s"/>' % (punkte, fuell, rand)

    @staticmethod
    def _beschriftung(k):
        if k.art == "frage":
            return "wenn %s" % k.text
        return k.text
