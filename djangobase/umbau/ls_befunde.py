# -*- coding: utf-8 -*-
"""Die Befunde eines Laufs — gewichtet, gefiltert, gezählt, als Tabellenzeilen.
...
BESCHRIFTUNG = {"error": "Fehler", "warning": "Warnung", "information": "Hinweis"}


class LsBefunde:
    """Sichten auf die Befunde EINES Ergebnisses."""
    ...
    # ── Auswahl ──────────────────────────────────────────────────────────
    def roh(self):
        """Alle Befunde ohne die stummgeschalteten JavaScript- und Rahmen-Meldungen.
    ...
    def gefiltert(self):
        """Nur bis zur Anzeigestufe, nach Gewicht, Datei, Zeile sortiert."""
    ...
    def kennzahlen(self):
        ...
        return {
            "fehler": zahl["error"],
            "warnungen": zahl["warning"],
            "hinweise": zahl["information"],
            "gesamt": len(alle),
            ...
            "konsumenten": (len(self.rahmen.konsumenten()) if self.rahmen is not None else 0),
        }

    def je_regel(self):
        """``[(regel, anzahl, stufe)]`` — die häufigste zuerst."""
        zaehler, stufe = {}, {}
        for b in self.roh():
            zaehler[b["regel"]] = zaehler.get(b["regel"], 0) + 1
            stufe.setdefault(b["regel"], b["stufe"])
        return sorted(((r, n, stufe[r]) for r, n in zaehler.items()), key=lambda t: (-t[1], t[0]))
    ...
    def tabelle(self):
        """Die Struktur für ``djangobase/_tabelle.html`` — höchstens ``deckel`` Zeilen."""
        zeilen = []
            ...
            zeilen.append(
                {
                    ...
                    "id": "%s|%d|%d" % (b["datei"], b["zeile"], b["spalte"]),
                    "zellen": [
                        {
                            "html": BESCHRIFTUNG.get(b["stufe"], b["stufe"]),
                            "klasse": "ls-stufe",
                            "sort": GEWICHT.get(b["stufe"], 2),
                        },
                        {
                            "html": '<code class="ls-datei">%s</code>' % escape(b["datei"]),
                            "klasse": "ls-datei-zelle",
                        },
                        {"html": "%d" % b["zeile"], "klasse": "num", "sort": b["zeile"]},
                        {"html": escape(b["regel"] or "—"), "klasse": "ls-regel"},
                        ...
                        # Python-Sitzung; JavaScript-Befunde (tsc) bekommen keinen Knopf.
                        {
                            "html": (
                                (
                                    '<button type="button" class="ls-knopf ls-ref" '
                                    'title="Wer benutzt das? Definition? Umbenennen?">'
                                    '<i class="bi bi-diagram-2"></i></button>'
                                )
                                if b["datei"].endswith(".py")
                                else ""
                            ),
                            "klasse": "ls-aktion",
                        },
        ...
        return {
            ...
            "klasse": "ls-tabelle",
            "spalten": [{"label": label, "key": k, "num": n} for label, k, n in self.SPALTEN],
            "zeilen": zeilen,
            "leer": "keine Befunde — oder noch kein Lauf",
        }
