# -*- coding: utf-8 -*-
"""Werkzeugverzeichnis - was im Kasten liegt, in der Reihenfolge des Gebrauchs.
...
class Werkzeugverzeichnis:
    """Die Module von ``djangobase.umbau``, gruppiert nach Arbeitsschritt."""
    ...
    GRUPPEN = (
        (
            "Umstellen",
            "Viele Dateien in einem Lauf - Probelauf ist die Vorgabe.",
            ("serverabrufe", "protokoll", "jsimporte", "stilklassen"),
        ),
        (
            "Ansehen",
            "Was vor dem Schnitt zu klaeren ist: was steht drin, wer ruft wen.",
            ("strukturbericht", "aufrufgraph"),
        ),
        (
            "Schneiden",
            "Definitionen herausheben - über den AST, nicht per Textsuche.",
        ...
        ("Grundlagen", "Bausteine, auf denen die anderen aufsetzen.", ("codesicht", "kommateilung")),
    )

    def eintraege(self):
        """[{gruppe, gruppe_zweck, modul, aufruf, klassen, zweck, schreibt}]"""
        nach_modul = {}
    ...
    def gruppen(self):
        """[(titel, zweck, [eintrag, ...])] - für die Ausgabe auf der Seite."""
        raus = []
        for titel, zweck, _module in Werkzeugverzeichnis.GRUPPEN:
            raus.append((titel, zweck, [e for e in self.eintraege() if e["gruppe"] == titel]))
        return raus

    # ------------------------------------------------------------------ intern
    @staticmethod
    def _zweck(modul):
        """Erste Docstring-Zeile, ohne den vorangestellten Klassennamen."""
