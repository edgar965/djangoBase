# -*- coding: utf-8 -*-
"""Wer ruft wen — über das ganze Projekt, für Funktionen UND Klassen.
...
class Stelle:
    """Eine Definition: Funktion oder Klasse, mit ihrem Ort."""
...
class Aufrufnetz:
    """Liest ein Projekt und weiß danach, wer wen ruft."""
    ...
    def _definitionen(self, baum, datei):
        for knoten in baum.body:
            if isinstance(knoten, (ast.FunctionDef, ast.AsyncFunctionDef)):
                self.stellen.setdefault(
                    knoten.name, Stelle(knoten.name, Stelle.FUNKTION, datei, knoten.lineno)
                )
        for knoten in ast.walk(baum):
            if isinstance(knoten, ast.ClassDef):
                self.stellen.setdefault(knoten.name, Stelle(knoten.name, Stelle.KLASSE, datei, knoten.lineno))
    ...
    @staticmethod
    def _gerufener(ruf):
        """Nur der Aufruf BEIM NAMEN zaehlt.
    ...
    def steckbrief(self, name):
        """``{name, art, datei, zeile, genutzt_von, nutzt}`` oder ``None``."""
    ...
    def kennzahlen(self):
        funktionen = [s for s in self.stellen.values() if s.art == Stelle.FUNKTION]
        ungenutzt = [s.name for s in funktionen if not self.genutzt_von.get(s.name)]
        return {
