# -*- coding: utf-8 -*-
"""Der Ablauf EINER Funktion: Reihenfolge, Verzweigungen, Ausgaenge.
...
class Knoten:
    """Gemeinsame Grundform aller Ablauf-Teile."""

    art = "knoten"

    def __init__(self, text, zeile, ziel="", aufruf="", empfaenger="", merkmal=""):
        self.text = text
        self.zeile = zeile
...
class Ende(Knoten):
    """Hier ist der Ablauf zu Ende — ``return``, ``raise``, ``break``."""
...
class Wiederholung(Knoten):
    """``for`` oder ``while`` — der Rumpf laeuft mehrfach."""
...
class Absicherung(Knoten):
    """``try`` mit ``except``/``finally`` — was schiefgehen darf."""
...
class Ablauf:
    """Liest EINEN Funktionsrumpf als Folge von Knoten.
    ...
    def _eine(self, a):
        ...
        if isinstance(a, (ast.For, ast.AsyncFor)):
            schleife = Wiederholung(
                "für jedes %s in %s" % (self._text(a.target), self._text(a.iter)), a.lineno
            )
            schleife.rumpf = self._folge(a.body)
            return schleife
        if isinstance(a, ast.While):
            schleife = Wiederholung("solange %s" % self._text(a.test), a.lineno)
            schleife.rumpf = self._folge(a.body)
            return schleife
        if isinstance(a, ast.Try):
            sicher = Absicherung("versuchen", a.lineno)
        ...
        if isinstance(a, ast.Return):
            return Ende("zurück: %s" % self._text(a.value) if a.value else "zurück", a.lineno)
        if isinstance(a, ast.Raise):
            return Ende("Fehler: %s" % self._text(a.exc) if a.exc else "Fehler weiterreichen", a.lineno)
        if isinstance(a, ast.Break):
            return Ende("Schleife verlassen", a.lineno)
        if isinstance(a, ast.Continue):
            return Ende("nächster Durchgang", a.lineno)
        if isinstance(a, (ast.With, ast.AsyncWith)):
            # Ein `with` ist kein Ablaufschritt, sein Rumpf schon.
            aus = self._folge(a.body)
            return aus[0] if len(aus) == 1 else (self._buendeln(aus, a.lineno) if aus else None)
        return self._anweisung(a)

    @staticmethod
    def _buendeln(knoten, zeile):
        """Mehrere Knoten aus einem ``with`` als Schleifenrumpf halten —
        damit die Reihenfolge nicht verlorengeht."""
        traeger = Wiederholung("im Block", zeile)
        traeger.art = "block"
        traeger.rumpf = knoten
        return traeger

    def _anweisung(self, a):
        """Eine gewoehnliche Zeile — nur zeigen, wenn sie etwas TUT.
        ...
        return Schritt(
            ...
            a.lineno,
            self._ziel(erster),
            self._aufrufname(erster),
            self._empfaenger(erster),
            self._merkmal(erster),
        )

    @staticmethod
    def _aeusserster(a):
        """Der Aufruf, der diese Anweisung ausmacht — oder ``None``."""
    ...
    def _aufrufname(aufruf):
        return getattr(aufruf.func, "attr", None) or getattr(aufruf.func, "id", "")

    @staticmethod
    def _merkmal(aufruf):
        """Das erste Argument — aber nur, wenn es ein einfacher Name ist.
    ...
    def _empfaenger(aufruf):
        """``self.service.prepare()`` -> ``service``; ``self.tun()`` -> ''.
    ...
    def _sagt_etwas(aufruf):
        name = getattr(aufruf.func, "attr", None) or getattr(aufruf.func, "id", "")
        return name not in STUMM

    def _ziel(self, aufruf):
        """``Klasse.methode`` im Projekt — oder leer."""
    ...
    @staticmethod
    def _text(knoten):
        """Die Zeile, wie sie dasteht — gekuerzt.
        ...
        try:
            ...
            return ""
        roh = " ".join(roh.split())
        return roh if len(roh) <= BREIT else roh[: BREIT - 1] + "…"

    def als_dict(self):
        return {
            "name": self.bezug.anzeige,
            "modul": self.bezug.modul,
