# -*- coding: utf-8 -*-
"""Statische `style="…"` durch CSS-Klassen ersetzen.
...
NAMEN = {
    ...
    "padding: 10px; text-align: left; border-bottom: 2px solid #444;": "hb-kopfzelle",
    ...
    "margin-left:auto;font-size:0.7rem;color:var(--text-muted);": "hb-kuerzel",
    ...
    "height:4px;background:var(--bg-card);border-radius:2px;overflow:hidden;": "hb-balken",
    "font-size:0.7rem;color:var(--text-muted);margin-top:3px;": "hb-untertitel",
    "font-size:0.85rem;color:var(--accent);margin-bottom:10px;": "hb-abschnittstitel",
    "font-size:0.7rem;color:var(--text-muted);margin-bottom:6px;line-height:1.4;": "hb-erklaerung",
...
class Stilklassen:
    ...
    @staticmethod
    def _name(fassung):
        """Notname aus der ersten Eigenschaft — nur fuer haeufige Fassungen
    ...
    def _klassen_einsetzen(self):
        """`data-neueklasse` in ein echtes `class`-Attribut ueberfuehren.
        ...
        self.text = re.sub(r'\sdata-neueklasse="([^"]+)"', lambda t: ' class="%s"' % t.group(1), self.text)
    ...
    def css(self):
        ...
        for fassung, name in sorted(self.klassen.items(), key=lambda p: p[1]):
            ...
            zeilen.append(".%s.%s.%s { %s; }   /* %dx */" % (name, name, name, regeln, zaehler[fassung]))
        return "\n".join(zeilen)

    def css_einsetzen(self):
        """Die neuen Regeln ans Ende des ersten <style>-Blocks setzen.
        ...
        if "</style>" not in self.text:
            ...
            if not treffer:
                raise ValueError("%s: weder <style> noch content-Block" % self.pfad)
...
def main():
    ...
    for datei in argumente:
        ...
        vorlage.umbauen()
        print(
            "%s: %d Stellen -> %d Klassen%s"
            % (datei, vorlage.ersetzt, len(vorlage.klassen), "" if schreiben else "  (Probelauf)")
        )
        if schreiben:
