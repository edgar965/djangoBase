...
class Strukturbericht:
    ...
    THEMEN = [
        ...
        ("kleidung", ("garment", "cloth", "wardrobe", "mh_proxy", "pattern", "preset")),
    ...
    def eintraege(self):
        ...
        for k in self.baum.body:
            ...
            raus.append((k.name, art, k.lineno, laenge, self.thema(k.name), self._ist_endpunkt(k)))
        return raus
    ...
    def gruppen(self):
        g = defaultdict(list)
        for name, _art, zeile, laenge, thema, endpunkt in self.eintraege():
            g[thema].append((name, zeile, laenge, endpunkt))
        return g

    def bericht(self, mit_gruppen=False):
        zeilen = ["%s — %d Zeilen" % (self.pfad.name, self.quelle.count("\n") + 1)]
        eintraege = self.eintraege()
        endpunkte = sum(1 for e in eintraege if e[5])
        zeilen.append("%d Definitionen, davon %d Endpunkte" % (len(eintraege), endpunkte))
        zeilen.append("")
        if mit_gruppen:
            for thema, posten in sorted(self.gruppen().items(), key=lambda kv: -sum(p[2] for p in kv[1])):
                summe = sum(p[2] for p in posten)
                zeilen.append("=== %-10s %3d Definitionen, %5d Zeilen" % (thema, len(posten), summe))
                for name, zeile, laenge, endpunkt in sorted(posten, key=lambda p: -p[2]):
                    zeilen.append(
                        "    %-46s %5d Z. ab %5d %s" % (name[:46], laenge, zeile, "API" if endpunkt else "")
                    )
        else:
            for name, art, zeile, laenge, thema, endpunkt in sorted(eintraege, key=lambda e: -e[3]):
                zeilen.append(
                    "  %-46s %5d Z. ab %5d  %-9s %s"
                    % (name[:46], laenge, zeile, thema, "API" if endpunkt else art)
                )
        return "\n".join(zeilen)
