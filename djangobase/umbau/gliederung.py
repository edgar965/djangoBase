# -*- coding: utf-8 -*-
"""Rolle im Projekt — die Gliederung, die Klassen und Funktionen teilen.
...
def rolle(datei):
    """Die Rolle einer Datei — ``'Tests'``, ``'Ansichten'``, …"""
    ...
    for etikett, marken in ROLLEN:
        ...
        if letzte in marken or (etikett == "Tests" and teile and teile[-1].startswith("test_")):
            return etikett
    return UEBRIGE


def untergruppe(datei):
    """Das Verzeichnis — oder der Dateiname, wenn es keines gibt.
...
def nach_rolle(eintraege):
    ...
    for name, datei in eintraege:
        je_rolle.setdefault(rolle(datei), {}).setdefault(untergruppe(datei), []).append(name)

    raus = []
    for etikett, gruppen in je_rolle.items():
        teile = [
            {"name": g, "namen": sorted(v), "zahl": len(v)}
            for g, v in sorted(gruppen.items(), key=lambda p: (-len(p[1]), p[0]))
        ]
