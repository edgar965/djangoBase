...
class KlassenBauer:
    ...
    def _methode(text, alt, neu):
        ...
        eingerueckt = "\n".join(("    " + z) if z.strip() else z for z in text.split("\n"))
        return "    @staticmethod\n" + eingerueckt

    @staticmethod
    def _aufrufe_umbiegen(text, klasse, zuordnung):
        for alt, neu in zuordnung.items():
            text = re.sub(r"(?<![\w.])%s\(" % re.escape(alt), "%s.%s(" % (klasse, neu), text)
        return text

    # ---------------------------------------------------------------- schreiben

    def klasse_bauen(self, klasse, ziel, kopf, zuordnung, ebenen_tiefer=0, zusatz_importe=(), entfernen=True):
        ...
        for alt, text, _a, _e in sorted(bloecke, key=lambda b: b[2]):
            if ebenen_tiefer:
                text = "\n".join(
                    ...
                    if z.lstrip().startswith("from .")
                    else z
                    for z in text.split("\n")
                )
            text = self._aufrufe_umbiegen(text, klasse, {k: v for k, v in zuordnung.items() if k != alt})
