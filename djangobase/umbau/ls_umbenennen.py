# -*- coding: utf-8 -*-
"""Einen ``WorkspaceEdit`` des Servers ansehen — und mit Netz anwenden.
...
class Umbenennung:
    """Vorschau und Anwendung eines ``WorkspaceEdit``."""
    ...
    def aenderungen(self):
        """``{Pfad: [(zeile0, spalte0, endzeile0, endspalte0, neu), …]}``.
        ...
        for d in self.edit.get("documentChanges") or []:
            if "textDocument" in d:
                raus.setdefault(pfad_aus_uri(d["textDocument"]["uri"]), []).extend(
                    self._edits(d.get("edits") or [])
                )
    ...
    # ── Vorschau ─────────────────────────────────────────────────────────
    def vorschau(self):
        """``[{datei, zeile, alt, neu, zeile_text}]`` — nach Datei und Zeile."""
        raus = []
        for pfad, edits in self.aenderungen().items():
            ...
            for z0, s0, z1, s1, neu in sorted(edits):
                text = zeilen[z0] if z0 < len(zeilen) else ""
                alt = text[s0:s1] if z0 == z1 else text[s0:]
                raus.append(
                    {
                        "datei": self._rel(pfad),
                        "zeile": z0 + 1,
                        "alt": alt,
                        "neu": neu,
                        "zeile_text": text.strip(),
                    }
                )
    ...
    # ── Anwenden ─────────────────────────────────────────────────────────
    def anwenden(self):
        """Schreiben mit Sicherung und Netz. ``{dateien, stellen, sicherung, fehler}``."""
        ...
        for pfad, edits in self.aenderungen().items():
            try:
                ...
                fehler.append("%s: nicht lesbar (%s)" % (self._rel(pfad), e))
                continue
            ...
            if pfad.suffix == ".py":
                try:
                    ...
                    fehler.append(
                        "%s: nach dem Umbau nicht mehr kompilierbar "
                        "(Zeile %s) — unverändert gelassen" % (self._rel(pfad), e.lineno)
                    )
                    continue
        ...
        return {
            "dateien": dateien,
            "stellen": stellen,
            "sicherung": str(ablage) if dateien else "",
            "fehler": fehler,
        }

    @staticmethod
    def _anwenden_auf(text, edits):
        """Edits von hinten nach vorn auf den Text — Zeilenenden inklusive."""
