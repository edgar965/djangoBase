# -*- coding: utf-8 -*-
"""Einen Weg als SVG zeichnen — Kaesten, Bogen, Spalten nach Tiefe.
...
class Karte:
    """Ein Kasten mit seinem Platz im Bild."""
...
class Workflowbild:
    ...
    def anordnen(self):
        """Je Tiefe ein Block, der umbricht statt in die Laenge zu gehen.
        ...
        for tiefe in sorted(spalten):
            eintraege = sorted(spalten[tiefe], key=lambda s: s.bezug.anzeige.lower())
    ...
    def _zeilen_waehlen(spalten):
        """Die Zeilenzahl, bei der das Bild am ehesten Bildschirmform hat.
        ...
        for zeilen in range(6, 31):
            breite = sum(max(1, -(-len(e) // zeilen)) for e in spalten.values()) * SPALTE
            hoehe = min(max(len(e) for e in spalten.values()), zeilen) * ZEILE
            if not hoehe:
                continue
    ...
    def svg(self):
        ...
        teile = [
            ...
            'class="wf-bild">' % (self.breite, self.hoehe, self.breite, self.hoehe)
        ]
    ...
    def _abschluss(self):
        """Der Vermerk, wenn das Bild weniger zeigt als den ganzen Weg.
        ...
        if not getattr(self.weg, "abgeschnitten", False):
            return []
        return [
            '<text class="wf-mehr" x="%d" y="%d">%s</text>'
            % (
                RAHMEN,
                self.hoehe - RAHMEN // 2,
                "… hier geht der Weg weiter, als das Bild zeigt "
                "(%d Klassen in %d Schritten gezeichnet)" % (len(self.weg.klassen), len(self.weg.schritte)),
            )
        ]
    ...
    def _bogen(a, b, grund):
        """Waagerecht austreten, waagerecht eintreten.
        ...
        return '<path class="wf-l" d="M%.1f %.1f C%.1f %.1f %.1f %.1f %.1f %.1f" stroke="%s"%s/>' % (
            x1,
            y1,
            x1 + kraft,
            y1,
            ...
            x2,
            y2,
            FARBEN.get(grund, "#7a8a99"),
            strich,
        )

    def _kaesten(self):
        aus = []
        for karte in sorted(self.karten.values(), key=lambda k: (k.x, k.y)):
            bezug = karte.schritt.bezug
            klasse = "wf-k wf-k-start" if karte.schritt.tiefe == 0 else "wf-k"
            aus.append(
                '<rect class="%s" x="%d" y="%d" width="%d" '
                'height="%d"/>' % (klasse, karte.x, karte.y, BREITE, HOEHE)
            )
            aus.append(
                '<text class="wf-n" x="%d" y="%d">%s</text>'
                % (karte.x + 9, karte.y + 17, escape(self._kuerzen(bezug.anzeige, 26)))
            )
            aus.append(
                '<text class="wf-m" x="%d" y="%d">%s:%d</text>'
                % (karte.x + 9, karte.y + 31, escape(self._kuerzen(bezug.modul, 28)), bezug.zeile)
            )
        return aus

    @staticmethod
    def _kuerzen(text, wie_viel):
        """Von LINKS kuerzen bei Modulpfaden — hinten steht das Wesentliche.
