# -*- coding: utf-8 -*-
"""Kommt die Aufzeichnung auch in Projekten an, die djangoBases Vorlagen NICHT nutzen?
...
FREMDE_SEITE = b"<!doctype html><html><head><title>Kameras</title></head><body><h1>Kameras</h1></body></html>"


def _mw(antwort):
    """Middleware mit fester Antwort - ohne echte Request-Kette."""
...
class MiddlewareTest(SimpleTestCase):
    ...
    def test_fremde_seite_bekommt_die_module(self):
        """DER gemeldete Fall: eine Seite ganz ohne djangoBase-Vorlagen."""
        ...
        for modul in (
            ...
            self.assertIn(modul, text, "Auf /kameras/ fehlt %r — genau der gemeldete Befund" % modul)

    def test_wird_vor_body_ende_eingehaengt(self):
        """Sonst stünde es hinter dem schließenden Tag - Browser verzeihen das,
    ...
    def test_content_length_wird_nachgezogen(self):
        """Ein zu kurzes Content-Length schneidet die Seite im Browser ab."""
    ...
    def test_fragment_ohne_body_bleibt_unberuehrt(self):
        """Ein nachgeladenes Schnipsel hat kein </body> - dort würde die
    ...
    def test_weiterleitung_und_fehler_bleiben_unberuehrt(self):
        for code in (302, 404, 500):
            ...
            self.assertNotIn(
                b"aufzeichner_leiste.js", antwort.content, "Status %d darf nicht angefasst werden" % code
            )

    def test_keine_doppelte_einbindung(self):
        """Bringt eine Vorlage die Module schon mit, hält sich die Middleware
        zurück - zwei Instanzen desselben Moduls zeichnen jeden Klick doppelt
        auf (belegt am selben Tag)."""
        roh = HttpResponse(
            b'<html><body><script src="/static/djangobase/js/'
    ...
    def test_eigener_endpunkt_ausgenommen(self):
        """Der Steuer-Endpunkt liefert JSON, aber sicher ist sicher.
        ...
        antwort = self._durch(HttpResponse(FREMDE_SEITE), pfad=Basiswurzel.weg() + "tests/aufzeichnung/")
    ...
    def test_kaputte_einbettung_laesst_die_seite_heil(self):
        """Die Aufzeichnung ist ein Werkzeug, kein Bestandteil der Anwendung -
...
class SelbsteintragTest(SimpleTestCase):
    """Die Middleware trägt sich selbst in MIDDLEWARE ein.
    ...
    def test_steht_in_der_kette(self):
        ...
        self.assertIn(
            AUFZEICHNUNG_MIDDLEWARE,
            list(settings.MIDDLEWARE),
            "djangobase.apps.ready() muss die Middleware nachtragen",
        )

    def test_steht_hinter_allen_fremden(self):
        """Sie schreibt in den fertigen Antwort-Inhalt. Weiter vorn käme sie an
        ...
        self.assertFalse(
            danach,
            "Nach der Aufzeichnungs-Middleware stehen fremde "
            "Middlewares: %s. Sie könnten die Antwort ersetzen, "
            "nachdem die Module eingehängt wurden." % danach,
        )
