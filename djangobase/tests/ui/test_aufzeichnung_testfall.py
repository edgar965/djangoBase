# -*- coding: utf-8 -*-
"""Aus einer Aufzeichnung wird ein Testfall — per Knopf, mit eindeutigen Zielen.
...
class AblageTest(SimpleTestCase):
    """Punkt 1: Wohin die Datei geht — und wann NICHT."""
    ...
    def setUp(self):
        ...
        s.anhaengen(
            ...
            [
                ...
                {"art": "abruf", "methode": "GET", "pfad": "/api/kurse/", "status": 200, "t": 1.0},
    ...
    def test_vorhandene_datei_wird_nicht_ueberschrieben(self):
        """Sie kann von Hand ergänzte Zusicherungen tragen — genau die, die
    ...
    def test_ohne_ziel_klare_meldung_statt_geratenem_ort(self):
        """Eine Datei, die irgendwo landet, findet niemand wieder."""
    ...
    def test_endpunkt_kennt_die_aktion(self):
        """Ohne die Aktion bliebe der Knopf ein Knopf ohne Wirkung."""
...
class AbspielerPruefungTest(SimpleTestCase):
    """Punkt 2: Der Abspieler vergleicht die Abrufe."""
    ...
    def test_abrufe_werden_verglichen_statt_uebersprungen(self):
        ...
        self.assertNotIn(
            "if (s.art === 'abruf') return true",
            self.quelle,
            "Abrufe dürfen nicht mehr kommentarlos übersprungen "
            "werden — sie sind die Zusicherung der Aufnahme",
        )

    def test_abrufe_werden_nicht_nachgefahren(self):
        """Ein aufgezeichnetes POST würde seine Wirkung erneut auslösen.
    ...
    def test_treffer_wird_verbraucht(self):
        """Sonst erfüllte EIN Abruf zwei gleiche Erwartungen."""
...
class KennungTest(SimpleTestCase):
    """Punkt 3: Mehrdeutige Selektoren werden eindeutig gemacht."""
    ...
    def test_nummer_wird_mitgeschrieben(self):
        """Nur wenn der Selektor mehrdeutig blieb — sonst wäre sie eine
    ...
    def test_abspieler_raet_nicht_mehr(self):
        """Weder Nummer noch eindeutiger Text: lieber ein gemeldeter
        Fehlschlag als ein falscher Klick, der als grün durchgeht."""
        finden = self.absp.split("static async finden(s)", 1)[1].split("\n  }", 1)[0]
        self.assertNotIn(
            "|| treffer[0]", finden, "Der erste Treffer darf nicht mehr als Notlösung geklickt werden"
        )
        self.assertIn("return null", finden)

    def test_server_reicht_die_nummer_durch(self):
        """Ohne dieses Feld käme der Abspieler auf ein anderes Element."""
    ...
    def test_nummer_wird_begrenzt(self):
        """Was aus dem Browser kommt, ist Eingabe von außen."""
        from djangobase.aufzeichnung_steuerung import Steuerung as St

        self.assertEqual(St._pruefen({"art": "klick", "ziel": "b", "nr": 10**9, "t": 1})["nr"], 9999)
        self.assertNotIn("nr", St._pruefen({"art": "klick", "ziel": "b", "nr": "hallo", "t": 1}))
