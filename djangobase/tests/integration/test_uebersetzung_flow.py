...
class UebersetzungFlowTest(StoreIsolationMixin, BasisTest):
    ...
    def test_status_zaehlt_aktuell_veraltet_fehlt(self):
        ...
        Uebersetzung.objects.create(quelle=q1, sprache="en", text="Places", quelle_hash=ue._hash("Orte"))
        ue.katalog_leeren()
    ...
    def test_geaenderter_quelltext_macht_uebersetzung_veraltet(self):
        ...
        Uebersetzung.objects.create(
            quelle=q, sprache="en", text="Load more", quelle_hash=ue._hash("alter text")
        )  # falscher Hash
        ue.katalog_leeren()
