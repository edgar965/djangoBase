# -*- coding: utf-8 -*-
"""Wann gilt eine Tabellenzelle als ZAHL — und wann als Text?
...
JS = Path(__file__).resolve().parents[2] / "static" / "djangobase" / "js" / "tabellen_sortierung.js"
...
def _als_python(js_muster):
    """`/…/` in ein Python-Muster übersetzen (ohne Flags — es hat keine)."""
...
class ZahlErkennungTest(SimpleTestCase):
    """Das Muster aus der ausgelieferten Datei, an echten Zellinhalten."""
        ...
        for inhalt, erwartet in self.FAELLE:
            with self.subTest(zelle=inhalt):
                ...
                self.assertIs(
                    ist,
                    erwartet,
                    "%r gilt als %s, erwartet %s"
                    % (inhalt, "Zahl" if ist else "Text", "Zahl" if erwartet else "Text"),
                )

    def test_die_gegenprobe_wuerde_anschlagen(self):
        """Das ALTE Verhalten muss durchfallen — sonst prüft der Test nichts.
        ...
        self.assertTrue(alt("Speed.mp4"), "die alte Fassung hielt Dateinamen fuer Zahlen")
        self.assertFalse(self.muster.match("Speed.mp4"), "die neue darf das nicht mehr")

    def test_das_muster_steht_nur_einmal_in_der_datei(self):
        """Zwei Fassungen desselben Musters laufen auseinander."""
        text = JS.read_text(encoding="utf-8")
