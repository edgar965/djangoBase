...
class ReviewPaketTest(BasisTest):
    ...
    def setUp(self):
        ...
        (self.wurzel / "paket" / "modul.py").write_text("def f():\n    return 42\n", encoding="utf-8")
    ...
    def test_ein_verzeichnis_meint_alle_quelldateien_darin(self):
        ...
        """
        (self.wurzel / "paket" / "zweit.py").write_text("def zwei():\n    return 2\n", encoding="utf-8")
        (self.wurzel / "paket" / "liesmich.txt").write_text("kein Quelltext", encoding="utf-8")
        paket = self._lauf()._paket({"slug": "a", "name": "Bereich A", "dateien": ["paket/"]}, "")
        self.assertIn("return 42", paket, "erste Datei des Pakets fehlt")
        self.assertIn("return 2", paket, "zweite Datei des Pakets fehlt")
        self.assertNotIn("kein Quelltext", paket, "eine .txt gehört nicht in den Quelltext-Anhang")
    ...
    def test_eine_datei_bleibt_eine_datei(self):
        ...
        paket = self._lauf()._paket({"slug": "a", "name": "Bereich A", "dateien": ["paket/modul.py"]}, "")
        self.assertIn("return 42", paket)
    ...
    def test_pfad_ausserhalb_der_wurzel_wird_nicht_gesendet(self):
        ...
        paket = self._lauf()._paket({"slug": "a", "name": "A", "dateien": ["../%s" % self.geheim.name]}, "")
    ...
    def test_fehlende_datei_meldet_sich_statt_zu_schweigen(self):
        paket = self._lauf()._paket({"slug": "a", "name": "A", "dateien": ["paket/gibtesnicht.py"]}, "")
    ...
    def test_funktionsauswahl_schneidet_und_nennt_fehlende(self):
        ...
        paket = self._lauf()._paket(
            {
                "slug": "a",
                "name": "A",
                "dateien": [{"pfad": "paket/gross.py", "funktionen": ["zwei", "Drei", "gibtesnicht"]}],
            },
            "",
        )
        ...
        self.assertIn(
            "NICHT gefunden: gibtesnicht",
            paket,
            "ein fehlender Name muss im Paket stehen — sonst hält das Modell den Ausschnitt für vollständig",
        )
            ...
            paket = lauf._paket(
                {
                    "slug": "a",
                    "name": "A",
                    "dateien": [{"pfad": "paket/riesig.py", "funktionen": ["ganz_hinten"]}],
                },
                "",
            )
    ...
    def test_bereich_mit_eigener_wurzel(self):
        ...
        paket = self._lauf()._paket(
            {"slug": "g", "name": "Geteilt", "wurzel": str(fremd), "dateien": ["geteilt.py"]}, ""
        )
        self.assertIn("WERT = 1", paket)

        # Und die Grenze gilt weiter: aufwaerts aus der EIGENEN Wurzel ist nichts zu holen.
        paket = self._lauf()._paket(
            {"slug": "g", "name": "Geteilt", "wurzel": str(fremd), "dateien": ["../geheim.txt"]}, ""
        )
        self.assertIn("außerhalb", paket.lower())

    def test_eigene_frage_ersetzt_die_bereichsfragen(self):
        paket = self._lauf()._paket(
            {"slug": "a", "name": "A", "dateien": [], "fragen": ["Vorgabe?"]}, "Meine eigene Frage"
        )
...
class ReviewFadenTest(BasisTest):
    ...
    def test_netzfehler_wird_zum_zustand_nicht_zur_ausnahme(self):
        ...
        with mock.patch.object(
            ReviewPartner, "fragen", side_effect=ReviewFehler("Dienst meldet: kein Guthaben")
        ):
...
class NebenlaeufigkeitTest(BasisTest):
    ...
    def test_nachfassen_meldet_nur_was_wirklich_lief(self):
        ...
        self.assertEqual(
            lauf.nachfassen("noch eine Frage", slug="f"),
            [],
    ...
    def test_wurzel_gross_klein_egal_unter_windows(self):
        ...
        lauf = ReviewLauf("dialog", PARTNER, wurzel=Path(str(wurzel).upper()), ablage=wurzel / "_ablage")
...
class LaufRegisterTest(BasisTest):
    def test_aelteste_laeufe_fallen_heraus(self):
        ...
        for _ in range(r.MAX_LAEUFE + 3):
            lauf = ReviewLauf(
                "frage", PARTNER, wurzel=Path(tempfile.gettempdir()), ablage=Path(tempfile.gettempdir())
            )
            laeufe.append(lauf)
            r.hinzu(lauf)
...
@override_settings(
    DJANGOBASE={
        "zugriff": "staff",
        "review_partner": [PARTNER],
        "review_ablage": _ABLAGE.name,
        "review_bereiche": [{"slug": "a", "name": "Bereich A", "dateien": []}],
    }
)
class ReviewSeiteTest(BasisTest):
    ...
    def test_die_pruefung_schreibt_nur_in_ihre_eigene_ablage(self):
        """DER WAECHTER: Ohne ihn faellt der naechste Rueckfall nicht auf.
        ...
        self.assertNotEqual(
            ablage.resolve(), Path(settings.BASE_DIR).resolve(), "Die Mitschriften landen im Projektstamm"
        )
    ...
    def test_nur_konfigurierte_bereiche_zaehlen(self):
        ...
        with mock.patch.object(ReviewPartner, "fragen", return_value="ok"):
            a = self.staff_client().post(
                reverse("djangobase:review_start"),
                data=json.dumps({"modus": "bereiche", "partner": "test", "bereiche": ["a", "../../etc"]}),
                content_type="application/json",
            )
        self.assertEqual(a.status_code, 200)
    ...
    def test_status_unbekannter_lauf(self):
        a = self.staff_client().get(reverse("djangobase:review_status", args=["gibtesnicht"]))
