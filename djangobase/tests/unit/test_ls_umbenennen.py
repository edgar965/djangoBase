# -*- coding: utf-8 -*-
"""Umbenennung: Vorschau, Anwenden mit Sicherung, Kompilier-Netz, Zeilenenden."""
...
def _edit(pfad, stellen):
    """WorkspaceEdit in der ``changes``-Form: stellen = [(zeile0, s0, s1, neu)]."""
    return {
        "changes": {
            uri(pfad): [
                {
...
class UmbenennungTest(unittest.TestCase):
    ...
    def test_vorschau_zeigt_jede_stelle(self):
        u = Umbenennung(
            _edit(self.datei, [(0, 4, 7, "neu"), (3, 4, 7, "neu")]), self.wurzel, self.wurzel / "sicherung"
        )
        v = u.vorschau()
        self.assertEqual(
            [(x["zeile"], x["alt"], x["neu"]) for x in v], [(1, "alt", "neu"), (4, "alt", "neu")]
        )
    ...
    def test_anwenden_schreibt_mit_sicherung_und_behaelt_crlf(self):
        ...
        self.assertEqual(self.datei.read_bytes(), b"def neu():\r\n    return 1\r\n\r\nx = neu()\r\n")
    ...
    def test_netz_faengt_kaputten_umbau(self):
        ...
        u = Umbenennung(_edit(self.datei, [(0, 4, 7, "1 2")]), self.wurzel, self.wurzel / "sicherung")
        b = u.anwenden()
    ...
    def test_document_changes_form(self):
        edit = {
            "documentChanges": [
                {
                    "textDocument": {"uri": uri(self.datei), "version": 1},
                    "edits": [
                        {
                            "range": {
