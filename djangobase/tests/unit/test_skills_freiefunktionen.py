# -*- coding: utf-8 -*-
"""Freie Funktionen — und wohin die Klasse gehört, die aus ihnen wird.
...
class FreieFunktionenTest(BasisTest):
    ...
    def _warum(satz, teil):
        for b in satz.befunde:
            if teil in b.warum:
                return b.warum
        raise AssertionError("%r steht in keinem Befund: %s" % (teil, [b.warum for b in satz.befunde]))
    ...
    def test_die_funktionen_werden_gezaehlt(self):
        ...
        self.assertTrue(any("5 Funktionen auf Modulebene" in z for z in satz.kopf), satz.kopf)

    def test_das_buendel_wird_als_klasse_vorgeschlagen(self):
        satz = self._lauf({"a.py": self.BUENDEL}, ab="3")
        self.assertIn("als Klasse `PersonVerwaltung`", self._warum(satz, "PersonVerwaltung"))
    ...
    def test_django_ansichten_gehoeren_in_eine_view_klasse(self):
        """Der haeufigste Fall: Der URL-Router ruft sie, keine Klasse."""
        satz = self._lauf(
            {
                "a.py": self.BUENDEL,
                "urls.py": (
        ...
        self.assertIn("klassenbasierte Ansicht", self._warum(satz, "Django-Ansichten"))
    ...
    def test_eine_testklasse_haelt_keinen_produktionscode(self):
        """DER FEHLER AUS DEM ERSTEN LAUF (24.08.2026).
    ...
    def test_ein_dienst_ist_eine_warnung(self):
        self.assertEqual(self._gewicht("services/rechnen.py"), Befund.WARNUNG)
    ...
    def test_eine_testhilfe_ebenso(self):
        self.assertEqual(self._gewicht("tests/unit/test_hilfe.py"), Befund.HINWEIS)

    def test_gemeldet_werden_sie_trotzdem(self):
        """Weglassen wäre schlimmer: Dann sähe niemand mehr, wie viele
        es sind."""
    ...
    def test_ein_verb_ist_kein_gegenstand(self):
        """DER BEFUND (24.08.2026): Für `path_resolver.py` kam
    ...
    def test_ohne_buendel_gilt_der_dateiname(self):
        """`mqtt.py` hält `get_client`, `publish_sighting`,
    ...
    def test_wo_die_klasse_schon_steht_fehlt_keine(self):
        ...
        self.assertIn("Klasse steht schon da", self._warum(satz, "Klasse steht schon da"))
        self.assertEqual([b.was for b in satz.befunde], ["3 Weiterleitungen vor 1 Klasse(n)"])

    def test_echte_logik_bleibt_ein_auftrag(self):
        """Eine Klasse im Modul allein genügt nicht — die freien
