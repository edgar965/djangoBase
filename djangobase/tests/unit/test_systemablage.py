# -*- coding: utf-8 -*-
"""Zwischendateien ohne ``dir=`` — und die Ausnahmen, die bleiben muessen.
...
class EineZwischendateiOhneOrt(WerkzeugBasis):
    """Gegeben: Der Ort bleibt offen — dann waehlt ihn das System."""
    ...
    def test_auch_direkt_importiert(self):
        """``from tempfile import mkstemp`` ist derselbe Aufruf."""
        projekt = self.projekt({"a.py": "from tempfile import mkstemp\nx = mkstemp()\n"})
...
class JedeAnlegerfunktion(WerkzeugBasis):
    """Gegeben: ``tempfile`` bietet mehrere Wege zum selben Ergebnis."""
    ...
    def test_alle_werden_erkannt(self):
        ...
        self.assertEqual(len(projekt.fahren(Systemablage)), len(Systemablage.ANLEGER))

    def test_ein_verzeichnis_zaehlt_mit(self):
        """``TemporaryDirectory`` ist der teuerste Fall — dort landet
...
class EineMitOrt(WerkzeugBasis):
    ...
    def test_egal_was_darin_steht(self):
        """Wohin genau, entscheidet das Projekt — nicht dieses
        Werkzeug."""
        projekt = self.projekt(
            {
                "a.py": (
    ...
    def test_aber_die_daneben_schon(self):
        """Die Gegenprobe: Eine richtige Stelle darf die falsche
...
class EinTest(WerkzeugBasis):
    """Gegeben: Eine Pruefung legt eine Zwischendatei an.
    ...
    def test_selbstraeumendes_in_pruefung_uebergangen(self):
        """`TemporaryDirectory` im `with` — der Ordner ist danach weg."""
        projekt = self.projekt(
    ...
    def test_mkstemp_in_pruefung_wird_gemeldet(self):
        """Am Dateinamen als Pruefung erkannt — und trotzdem gemeldet."""
        projekt = self.projekt({"app/test_x.py": "import tempfile\nx = tempfile.mkstemp()\n"})
        zeilen = projekt.fahren(Systemablage)
        self.assertEqual(len(zeilen), 1, zeilen)
        self.assertIn("test_x.py", zeilen[0]["ort"])

    def test_mkdtemp_im_pruefordner_wird_gemeldet(self):
        """Am Ordner als Pruefung erkannt — und trotzdem gemeldet."""
        projekt = self.projekt({"app/tests/hilfe.py": "import tempfile\nx = tempfile.mkdtemp()\n"})
        zeilen = projekt.fahren(Systemablage)
        self.assertEqual(len(zeilen), 1, zeilen)
        self.assertIn("hilfe.py", zeilen[0]["ort"])

    def test_mit_dir_bleibt_uebergangen(self):
        """`dir=` ist der richtige Weg — auch in einer Pruefung."""
        projekt = self.projekt(
...
class EineKaputteDatei(WerkzeugBasis):
    """Gegeben: Eine Datei, die sich nicht zerlegen laesst."""
...
class DerEigeneAnlassfall(WerkzeugBasis):
    """Gegeben: Der Fall, den das Werkzeug bei sich traegt."""
...
class EineBelegteAusnahme(WerkzeugBasis):
    """Gegeben: Der Vermerk nimmt die Stelle von DIESER Lehre aus.
    ...
    def test_ohne_vermerk_bleibt_es_ein_befund(self):
        """Die Gegenprobe: Das Werkzeug ist nicht still geworden."""
        ohne = self.MIT_VERMERK.replace(
            'Lehre gilt hier nicht ("keine-temp-dateien-im-system"): Das', "Der Grund steht hier nicht: Das"
        )
        zeilen = self.projekt({"a.py": ohne}).fahren(Systemablage)
        self.assertEqual(len(zeilen), 1, zeilen)

    def test_ein_vermerk_fuer_eine_ANDERE_lehre_zaehlt_nicht(self):
        """Der Vermerk gilt nur fuer die Lehre, die er beim Namen nennt."""
        fremd = self.MIT_VERMERK.replace("keine-temp-dateien-im-system", "irgendeine-andere-lehre")
        zeilen = self.projekt({"a.py": fremd}).fahren(Systemablage)
        self.assertEqual(len(zeilen), 1, zeilen)


class EineOrtsnennung(WerkzeugBasis):
    """Gegeben: `gettempdir()` — der Aufruf legt selbst nichts an.
    ...
    def test_die_meldung_nennt_den_aufruf(self):
        projekt = self.projekt({"a.py": "from tempfile import gettempdir\nz = gettempdir()\n"})
        self.assertIn("gettempdir", projekt.fahren(Systemablage)[0]["befund"])

    def test_auch_in_einer_pruefung(self):
        """Kein Anleger, also greift die Test-Ausnahme hier gar nicht."""
        projekt = self.projekt({"app/tests/x.py": "import tempfile\nz = tempfile.gettempdir()\n"})
        self.assertEqual(len(projekt.fahren(Systemablage)), 1)

    def test_ein_vermerk_nimmt_sie_aus(self):
        """Der belegte Fall: `gettempdir()` in einer ZUSICHERUNG.
