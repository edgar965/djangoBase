# -*- coding: utf-8 -*-
"""Übersprungen — findet es die Abkürzungen, und meldet es nichts Falsches?
...
class EsFindetDieAbkuerzungen(BasisTest):
    def test_umgebungsschalter_ohne_vorgabe_ist_ein_fehler(self):
        """Der Fall aus CamTrack: sechs Prüfungen, die NIE liefen."""
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "@unittest.skipUnless(os.environ.get('ECHT'), 'nur mit ECHT')\n"
                "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
                "        self.assertTrue(True)\n"
            }
        )
        self.assertEqual(_gewichte(satz).get(Befund.FEHLER), 1, [b.was for b in satz.befunde])

    def test_skiptest_im_rumpf_ist_eine_warnung(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
    ...
    def test_dauerhaft_stillgelegt_ist_ein_hinweis(self):
        """``@skip`` ohne Bedingung ist wenigstens sichtbar."""
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "@unittest.skip('spaeter')\n"
                "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
                "        self.assertTrue(True)\n"
            }
    ...
    def test_die_stelle_wird_benannt(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "class Abc(unittest.TestCase):\n"
                "    def test_x(self):\n"
                "        self.skipTest('x')\n"
            }
        )
...
class EinAusschalterIstKeinBefund(BasisTest):
    """DER FEHLALARM AUS DEM ERSTEN WURF (26.08.2026).
    ...
    def test_mit_vorgabe_ist_es_kein_fehler(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "@unittest.skipUnless(os.environ.get('GPU', '1') == '1', 'aus')\n"
                "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
                "        self.assertTrue(True)\n"
            }
        ...
        self.assertIsNone(
            _gewichte(satz).get(Befund.FEHLER),
            'Ein Ausschalter mit Vorgabe „an" läuft von selbst — das ist kein stillgelegter Test.',
        )

    def test_gemeldet_wird_er_trotzdem(self):
        """Nicht verschweigen: Wer ihn auf 0 setzt, soll wissen, was ausfällt."""
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "@unittest.skipUnless(os.environ.get('GPU', '1') == '1', 'aus')\n"
                "class A(unittest.TestCase):\n"
...
class EsMeldetNichtsFalsches(BasisTest):
    ...
    def test_ausserhalb_der_pruefungen_wird_nicht_gesucht(self):
        """``warteschlange.skip()`` in Anwendungscode ist kein Test-Skip."""
        satz = _lauf({"app/dienst.py": "def arbeiten(schlange):\n    schlange.skip()\n"})
        self.assertEqual(satz.befunde, [])

    def test_ein_fremdes_skip_im_pruefcode_zaehlt_nicht(self):
        """Nur ``skipTest`` oder ``pytest.skip`` — nicht jedes ``.skip()``."""
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
...
class DerKopfSagtWieVieleEsSind(BasisTest):
    ...
    def test_die_schweren_werden_getrennt_gezaehlt(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "@unittest.skipUnless(os.environ.get('ECHT'), 'x')\n"
                "class A(unittest.TestCase):\n"
                "    def test_x(self):\n"
                "        self.skipTest('y')\n"
            }
        )
        self.assertTrue(any("Umgebungsvariablen" in z for z in satz.kopf), satz.kopf)
