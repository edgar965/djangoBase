# -*- coding: utf-8 -*-
"""Was hat ein Umbau an den Funktionsruempfen wirklich geaendert?
...
class _Probe(Umbaugegenprobe):
    """Eine Ableitung, deren git-Zugriffe aus einem Woerterbuch kommen."""
...
class _Wurzel:
    """Tut so, als laege der neue Stand auf der Platte."""
...
class EineDateiOhneAenderung(unittest.TestCase):
    """Gegeben: Die Datei steht in git wie auf der Platte."""

    def test_es_wird_nichts_gemeldet(self):
        erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": _ALT}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])

    def test_auch_bei_anderer_formatierung_nicht(self):
        """Der Vergleich laeuft ueber den Syntaxbaum. Zeilenumbrueche und
        Klammern aendern nichts am Rumpf — sonst meldete jeder
        PEP-8-Durchlauf das ganze Projekt."""
        neu = _ALT.replace("return a + b", "return (\n        a\n        + b\n    )")
        erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])

    def test_und_bei_neuen_kommentaren_auch_nicht(self):
        neu = _ALT.replace("    return a + b", "    # Erklaerung, die vorher fehlte\n    return a + b")
        erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])


class EinNeuerDocstring(unittest.TestCase):
    """Gegeben: Nur der Docstring wurde umgeschrieben.
    ...
    def test_das_ist_keine_aenderung(self):
        neu = _ALT.replace(
            '"""Alter Docstring."""', '"""Ein ganz neuer, viel laengerer Text.\n\n    Mit Absatz.\n    """'
        )
        erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])

    def test_ein_neu_hinzugekommener_ebenfalls_nicht(self):
        neu = _ALT.replace("def zaehlen(x):", 'def zaehlen(x):\n    """Vorher stand hier keiner."""')
        erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])

    def test_eine_funktion_die_NUR_einen_docstring_hat_wirft_nicht(self):
        """Ohne Ersatz-Rumpf waere die Liste leer und ``ast.unparse``
        bekaeme ein Modul ohne Koerper."""
        alt = 'def leer():\n    """Nur Text."""\n'
        neu = 'def leer():\n    """Anderer Text."""\n'
        erg = _Probe(alt={"m.py": alt}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
        self.assertEqual(erg.zeilen, [])


class EinGeaenderterRumpf(unittest.TestCase):
    """Gegeben: Eine Funktion rechnet jetzt anders."""

    def setUp(self):
        neu = _ALT.replace("return a + b", "return a * b")
        self.erg = _Probe(alt={"m.py": _ALT}, neu={"m.py": neu}, geaendert=["m.py"]).laufen()
    ...
    def test_die_unveraenderte_wird_nicht_gemeldet(self):
        self.assertNotIn("zaehlen", [z["Rumpf"] for z in self.erg.zeilen])
...
class EinVerschwundenerRumpf(unittest.TestCase):
    """Gegeben: Eine Funktion gibt es nach der Aenderung nicht mehr.
...
class EineNeueDatei(unittest.TestCase):
    """Gegeben: Die Datei gab es vorher nicht.
...
class EinMassenumbau(unittest.TestCase):
    """Gegeben: Sehr viele geaenderte Dateien.
...
class DasWerkzeugSelbst(unittest.TestCase):
    """Gegeben: Es steht im Werkzeugkasten."""
    ...
    def test_es_sagt_warum_es_keinen_anlassfall_hat(self):
        """Ein Werkzeug ohne beides gilt als blind."""
