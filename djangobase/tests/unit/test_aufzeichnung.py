# -*- coding: utf-8 -*-
"""Die Testcase-Aufzeichnung: Bestand, Steuerung und der erzeugte Testfall.
...
class AufzeichnungTest(SimpleTestCase):
    """Bestand und Steuerung - auf einer Wegwerf-Datei, nie im Projekt."""
    ...
    def test_nur_eine_laeuft_gleichzeitig(self):
        """Ein zweiter Start liefert die LAUFENDE zurueck, statt eine zweite zu
...
class VerdichtungTest(SimpleTestCase):
    """Wiederkehrende Abrufe zaehlen, statt die Aufnahme zu fluten.
    ...
    def _abruf(self, pfad="/api/ib/auto/", status=200, t=1.0):
        return {"art": "abruf", "methode": "GET", "pfad": pfad, "status": status, "t": t}
    ...
    def test_abwechselnde_endpunkte_bleiben_getrennt(self):
        """Der Grund, warum die Verdichtung auf dem Server sitzt: Im Browser
    ...
    def test_klick_bleibt_eigener_schritt(self):
        """Klicks sind der Inhalt der Aufnahme. Wuerden sie zusammengefasst,
    ...
    def test_abrufe_zaehlen_nur_bis_zum_naechsten_klick(self):
        """Sonst verschmelzen die Abrufe einer halben Stunde zu einer Zeile und
    ...
    def test_anderer_status_faellt_nicht_zusammen(self):
        """Ein Poll, der ploetzlich 500 liefert, ist die interessanteste Zeile
        der ganzen Aufnahme - sie darf nicht in der Erfolgszeile verschwinden."""
        self.s.anhaengen(
            self.a.id,
            [
                self._abruf(status=200, t=1),
                self._abruf(status=200, t=2),
    ...
    def test_zaehler_des_browsers_kommt_an(self):
        """Der Browser verdichtet direkt aufeinanderfolgende Wiederholungen
...
class TestfallTest(SimpleTestCase):
    ...
    def setUp(self):
        ...
        self.a = self.s.beenden(
            a.id, logs=[{"zeit": "2026-08-20 18:00:00", "stufe": "ERROR", "logger": "x", "text": "kaputt"}]
        )
    ...
    def test_get_abrufe_ohne_doppelte(self):
        self.assertEqual(Testfall(self.a).abrufe(), [{"pfad": "/api/depot/", "status": 200}])

    def test_schreibzugriff_wird_nicht_nachgefahren(self):
        """Ein aufgezeichnetes POST loeste damals eine Wirkung aus - beim
        ...
        for zeile in quelltext.splitlines():
            if "/api/order/" in zeile:
                self.assertTrue(zeile.strip().startswith("#"), "Schreibzugriff steht als Code: %r" % zeile)
