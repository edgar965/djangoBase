# -*- coding: utf-8 -*-
"""``open()`` ohne ``with`` und ohne ``close()`` — und die Ausnahmen.
...
class EineOffeneDatei(WerkzeugBasis):
    """Gegeben: Die Datei wird geöffnet und liegen gelassen."""
...
class EinTryFinally(WerkzeugBasis):
    """Gegeben: Die Datei wird im ``finally`` geschlossen."""
    ...
    def test_und_die_uebergabe_an_einen_schliesser(self):
        """``self._schliessen(datei)`` — so macht es der Indexer."""
        projekt = self.projekt(
            {
                "a.py": (
                    "class X:\n"
...
class EinRueckgabewert(WerkzeugBasis):
    """Gegeben: Die Funktion gibt die Datei zurück.
    ...
    def test_sie_wird_nicht_gemeldet(self):
        projekt = self.projekt({"a.py": ("def oeffnen(p):\n    datei = open(p, 'w')\n    return datei\n")})
        self.assertEqual(projekt.fahren(OffeneDatei), [])
...
class ZweiFunktionen(WerkzeugBasis):
    """Gegeben: Geschlossen wird in einer ANDEREN Funktion."""

    def test_das_zaehlt_nicht(self):
        """Absichtlich streng: Wer die Datei woanders schließt, gibt
...
class EineKaputteDatei(WerkzeugBasis):
    """Gegeben: Eine Datei, die sich nicht zerlegen lässt."""
