...
class SammelzustandTest(BasisTest):
    """Das Werkzeug gegen echte Dateien — findet und erfindet nicht."""
    ...
    def test_es_traegt_seinen_anlassfall(self):
        """Ein Pruefer ohne Anlassfall meldet null und sieht dabei aus wie
        ein sauberes Projekt. Zweimal passiert am 17.08.2026."""
        werkzeug = werkzeug_finden(self.SLUG)
        self.assertIsNotNone(werkzeug.anlassfall)
        self.assertTrue(werkzeug.anlassfall.warum, "der Anlassfall muss sagen, welcher Vorfall hier steht")

    # ------------------------------------------------------ findet den Fall
    def test_findet_den_vorfall(self):
        """DER FALL, FUER DEN DAS WERKZEUG GEBAUT IST.
        ...
        self.assertIn("frames_processed", namen, "der gesammelte Zähler wurde nicht gemeldet")
        self.assertIn(
            "stillstand",
            namen,
    ...
    def test_der_vorfall_ist_eine_warnung_keine_randnotiz(self):
        """Zugriff je Entitaet und gesammeltes Schreiben in DERSELBEN Methode
        ...
        self.assertIn(
            "slug", satz.befunde[0].warum, "die Meldung muss sagen, WOMIT je Entität nachgeschlagen wird"
        )

    def test_die_abhilfe_nennt_die_unterinstanz(self):
        """Ein Befund ohne nächsten Schritt ist eine Beschwerde."""
        werkzeug = werkzeug_finden(self.SLUG)
    ...
    def test_ein_befund_je_feld_nicht_je_fundstelle(self):
        """Ein Zaehler in vier Methoden ist EIN Umbau, nicht vier Baustellen.
        ...
        self.assertEqual(len(satz.befunde), 1, "ein Feld muss genau einmal im Bericht stehen")
        self.assertEqual(
            satz.befunde[0].gewicht, "warnung", "von mehreren Fundstellen muss die schwerste bleiben"
        )

    # ------------------------------------------------- erfindet ihn NICHT
    def test_eine_instanz_je_kamera_ist_der_normalfall(self):
        """Der wichtigste Nicht-Befund ueberhaupt.
        ...
        self.assertFalse(satz.befunde, "eine Klasse je Entität darf nie gemeldet werden")

    def test_fester_schluessel_ist_keine_verteilung(self):
        """``self._teile['kopf']`` ist eine Struktur mit benannten Faechern,
    ...
    def test_eigene_stelle_als_index_ist_keine_verteilung(self):
        """``self.punkte[self.stand]`` ist Buchfuehrung ueber die eigene
    ...
    def test_datensatz_wird_uebersprungen(self):
        """Ein ``@dataclass`` beschreibt EINE Sache. Haelt er ein Verzeichnis,
    ...
    def test_kennungsfolge_bleibt_gemeinsam(self):
        """``next_id`` ist absichtlich geteilt — je Entitaet gefuehrt vergaebe
    ...
    def test_sammelbehaelter_ist_das_ergebnis_nicht_der_zustand(self):
        """``self.to_keep.append(person)`` IST, was die Klasse liefert.
    ...
    def test_alter_wert_als_vorgabe_ist_keine_rechnung(self):
        """``self.x = leser.zahl(..., self.x)`` nimmt den alten Wert als
    ...
    def test_was_dem_dienst_selbst_gehoert(self):
        """Ein Dienst hat EINEN Faden und EINE Sperre. Das ist kein geteilter
    ...
    def test_vermerk_nimmt_den_gewollten_fall_heraus(self):
        """Eine Gesamtzahl fuer die Startseite IST gewollt — sie soll nur
        ...
        self.assertFalse(mit.befunde, 'der Vermerk „geteilt gewollt" greift nicht mehr')

    def test_testdateien_bleiben_draussen(self):
        """Ein ``setUp`` legt je Pruefung ein Objekt an — das sieht aus wie
    ...
    def test_leeres_projekt_bleibt_still(self):
        """Wer im Leeren etwas meldet, sucht woanders."""
        satz = self._lauf({})
