# -*- coding: utf-8 -*-
"""Die Sperrklinke muss auch rot werden können.
...
def _klinke():
    """Die Sperrklinke als Exemplar — NICHT als Modulname.
    ...
    heraus.
    """
    return _grenzen.GrundtestBefundgrenzen("test_kein_werkzeug_ueberschreitet_seine_grenze")
...
class _NeuesWerkzeug:
    """Bauart ``BefundWerkzeug``: meldet über ``pruefen()``."""
...
class _AeltererTyp:
    """Bauart ``Werkzeug``: meldet über ``laufen()`` — kein ``pruefen``."""
...
def _lauf(grenzen, werkzeuge):
    """Die Sperrklinke mit gestellten Werkzeugen fahren.

    Liefert die Fehlermeldung — oder ``''``, wenn sie durchging.
    """
    fall = _klinke()
    with (
        override_settings(DJANGOBASE={"befundgrenzen": grenzen}),
        mock.patch("djangobase.skills.werkzeug_finden", side_effect=lambda s: werkzeuge.get(s)),
    ):
        try:
...
class BeideBauartenWerdenGezaehlt(SimpleTestCase):
    """Ein Fixer heißt nicht ``laufen`` — dieselbe Falle wie zweimal zuvor."""
...
class SieWirdRot(SimpleTestCase):
    ...
    def test_genau_auf_der_grenze_ist_gruen(self):
        self.assertEqual(_lauf({"x": 3}, {"x": _NeuesWerkzeug(["hinweis"] * 3)}), "")

    def test_darunter_ist_gruen(self):
        self.assertEqual(_lauf({"x": 9}, {"x": _NeuesWerkzeug(["hinweis"] * 3)}), "")

    def test_null_heisst_null(self):
        """`altlast` steht auf 0 — ein einziger Befund muss reichen."""
...
class GrenzenJeGewicht(SimpleTestCase):
    """219 Hinweise sind bei `code-qualität` fast alle Rang C — rot
    werden soll es bei den schweren."""

    WERKZEUG = {"q": _NeuesWerkzeug(["fehler"] * 2 + ["warnung"] * 5 + ["hinweis"] * 200)}
    ...
    def test_die_hinweise_zaehlen_dann_nicht_mit(self):
        """Zweihundert Hinweise, aber die Grenze gilt den Fehlern."""
        self.assertEqual(_lauf({"q": {"fehler": 2, "warnung": 5}}, self.WERKZEUG), "")
...
class WasSchiefgehenKann(SimpleTestCase):
    def test_ein_verschwundenes_werkzeug_ist_rot(self):
        """Sonst fällt eine Grenze still weg, sobald jemand ein Werkzeug
    ...
    def test_ohne_grenzen_wird_uebersprungen_nicht_bestanden(self):
        """Ein übersprungener Test soll nie grün melden — er steht gelb.
