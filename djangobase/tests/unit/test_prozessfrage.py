# -*- coding: utf-8 -*-
"""`Prozessfrage` — und was sie vor dem Loeschen verhindert.
...
class ProzessfrageTest(BasisTest):
    ...
    def test_ein_beendeter_prozess_lebt_nicht(self):
        """Ein echter Prozess, den wir starten und beenden."""
        ...
        self.assertTrue(Prozessfrage.lebt(kind.pid), "Ein laufender Prozess muss als lebend gelten")
        kind.kill()
    ...
    def test_die_frage_toetet_niemanden(self):
        """DER Grund für ctypes statt `os.kill`.
        ...
        self.assertIsNone(
            kind.poll(), "Die Frage hat den Prozess beendet — genau das macht `os.kill` unter Windows"
        )

    def test_unsinnige_eingaben_gelten_als_lebend(self):
        """Im Zweifel wird nichts geloescht."""
        for eingabe in (None, "abc", -1, 0, ""):
            self.assertTrue(Prozessfrage.lebt(eingabe), "Bei %r muss die Antwort JA lauten" % (eingabe,))
    ...
    def test_ein_name_ohne_nummer_gibt_none(self):
        for name in ("gb_1f8okk8w", "p", "pabc", "irgendwas", ""):
            self.assertIsNone(Prozessfrage.nummer_aus(name), "Bei %r darf keine Nummer herauskommen" % name)
