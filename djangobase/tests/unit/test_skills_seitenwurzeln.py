# -*- coding: utf-8 -*-
"""Seitenwurzeln — derselbe Massstab wie im Backend, andere Haelfte.
...
class SeitenwurzelnTest(BasisTest):
        ...
        satz = self._lauf(
            {
                ...
                "b.html": "<script>const y = new Zwei(2);</script>",
            },
            ab="0",
        )
        self.assertTrue(any("2 Wurzeln insgesamt" in z for z in satz.kopf), satz.kopf)
    ...
    def test_eine_seite_mit_EINER_wurzel_ist_das_ziel(self):
        ...
        self.assertFalse(satz.befunde, "so soll es aussehen: eine Seiten-Klasse")
    ...
    def test_bausteine_sind_keine_seiten(self):
        ...
        satz = self._lauf(
            {"_teil_frigate.html": ("<script>const a = new Eins(1); const b = new Zwei(2);</script>")}, ab="0"
        )
        self.assertFalse(satz.befunde)
...
class DieSeiteRechnetIhrGewicht(BasisTest):
    def test_ohne_wartezeit_hinweis(self):
        self.assertEqual(Seite("a.html", [("X", 1), ("Y", 2)], 0).gewicht, "hinweis")
