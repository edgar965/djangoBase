# -*- coding: utf-8 -*-
"""Das Klassenmodell — was aus dem Quelltext gelesen und gezeichnet wird.
...
class WasAusDemQuelltextGelesenWird(BasisTest):
    ...
    def test_eine_erzeugte_klasse_ist_eine_beziehung(self):
        ...
        self.assertEqual(m.klassen["Belegung"].haelt, [("zimmer", "Zimmer", "1")])
    ...
    def test_eine_sammlung_haelt_viele(self):
        ...
        self.assertEqual(m.klassen["Belegung"].haelt, [("gaeste", "Gast", "0..*")])
    ...
    def test_fremde_oberklassen_zaehlen_nicht(self):
        """`Exception` ist keine Klasse dieses Projekts."""
...
class DasBildWirdGezeichnet(BasisTest):
    ...
    def test_ein_unterstrich_macht_das_feld_privat(self):
        ...
        self.assertIn("- _geheim : int", Klassenbild(kaesten, linien, "A").svg())
    ...
    def test_spitze_klammern_im_namen_werden_entschaerft(self):
        """Der Kasteninhalt kommt aus fremdem Quelltext — er darf das SVG
        nicht aufbrechen."""
        m = _projekt({"a.py": ("class A:\n    def __init__(self):\n        self.x = '<script>'\n")})
        kaesten, linien = m.nachbarschaft("A", tiefe=1)
...
class JedeKlasseInGenauEinenTopf(BasisTest):
    ...
    def test_die_summe_stimmt(self):
        """Kein Doppel, keine Lücke — sonst taugt die Zahl nichts."""
        m = _projekt(
            {
                "a.py": (
    ...
    def test_eine_ausnahme_auch_am_namen(self):
        """`class JsonBodyError(ValueError)` erbt nicht von Exception."""
...
class EinTestIstKeinAst(BasisTest):
    """Der Einstieg ins Klassenbild darf keine Prüfung sein.
    ...
    def test_produktionscode_ist_kein_test(self):
        """`VideoCodecProbe` in `views/` heißt nur so."""
...
class DieRollenGliedernDasProjekt(BasisTest):
    """Zwei Ebenen statt einer Aufzählung.
    ...
    def test_die_summe_bleibt_vollstaendig(self):
        ...
        self.assertEqual(sum(r["zahl"] for r in m.nach_rolle()), len(m.klassen))
...
class DerSteckbriefZeigtBeideRichtungen(BasisTest):
    """Nicht nur was eine Klasse hält, auch WER sie hält.
    ...
    def test_wer_mich_haelt_steht_drin(self):
        ...
        self.assertEqual(sorted(g["feld"] for g in s["genutzt_von"]), ["t", "viele"])
    ...
    def test_wer_von_mir_erbt_steht_drin(self):
        self.assertEqual(self._modell().steckbrief("Halter")["beerbt_von"], ["Erbe"])
    ...
    def test_niemand_haelt_mich_ist_eine_leere_liste(self):
        """Und keine Ausnahme — das ist der Normalfall bei 933 von 1004."""
        self.assertEqual(self._modell().steckbrief("Halter")["genutzt_von"], [])


class KeinSammelbegriffAlsGruppe(BasisTest):
    """Eine Gruppe heißt wie die Datei, nicht „(Wurzel)".
...
class EinSicherungsordnerIstKeineQuelle(BasisTest):
    """Ein Abzug des Projekts darf nicht als Projekt gelten.
    ...
    def test_die_echte_datei_gewinnt(self):
        """Nicht nur die Zahl stimmt — der Ort muss der echte sein."""
        self.assertEqual(_projekt(self.DATEIEN).klassen["Echt"].datei, "echt.py")

    def test_dieselbe_liste_gilt_fuer_das_aufrufnetz(self):
        """Zwei Kopien liefen beim nächsten Zusatz auseinander."""
