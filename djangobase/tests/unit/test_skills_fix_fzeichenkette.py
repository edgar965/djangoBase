# -*- coding: utf-8 -*-
"""Leere f-Zeichenketten entschärfen — und warum es nur DIESEN Fixer gab.
...
class DasNetzHaeltStand(BasisTest):
    def test_nach_dem_schreiben_ist_nichts_uebrig(self):
        werkzeug, ordner = _fixer({"a.py": "x = f'leer'\n"})
        aenderung = werkzeug.vorschau().aenderungen[0]
        open(aenderung.pfad, "w", encoding="utf-8").write(aenderung.neuer_text)
        self.assertEqual(werkzeug.pruefen(aenderung), [])

    def test_eine_kaputte_datei_wird_gemeldet(self):
        """Das Netz muss anschlagen, sonst bleibt Kaputtes liegen."""
        werkzeug, _o = _fixer({"a.py": "x = f'leer'\n"})
        aenderung = werkzeug.vorschau().aenderungen[0]
        open(aenderung.pfad, "w", encoding="utf-8").write("def (:\n")
        self.assertTrue(werkzeug.pruefen(aenderung))

    def test_uebriggebliebene_werden_gemeldet(self):
        """Nicht nur „parst noch" — auch „ist wirklich erledigt"."""
        werkzeug, _o = _fixer({"a.py": "x = f'leer'\n"})
        aenderung = werkzeug.vorschau().aenderungen[0]
        open(aenderung.pfad, "w", encoding="utf-8").write("y = f'immer noch'\n")
        self.assertTrue(werkzeug.pruefen(aenderung))


class KeineDuplikate(BasisTest):
    """Ein Befund gehört genau EINEM Werkzeug.
    ...
    def test_code_qualitaet_listet_fremde_befunde_nicht_selbst(self):
        """`pyflakes` meldet auch tote Einfuhren — die führt
        ...
        self.assertEqual(ANDERSWO["FStringMissingPlaceholders"], "fix-fzeichenkette")

    def test_sie_verschwinden_aber_nicht(self):
        """Weglassen wäre schlimmer als doppelt melden: Die Zahl bleibt,
        mit dem Namen des Werkzeugs, das den Befund führt."""
        from djangobase.umbau.codequalitaet import Codequalitaet

        werkzeug, ordner = _fixer({"a.py": "import os\n"})
