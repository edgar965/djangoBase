# -*- coding: utf-8 -*-
"""`BefundLager`: welcher Lauf der CodeRabbit-CLI gehoert zu welchem Repo?
...
class BefundlagerBasis(unittest.TestCase):
    """Baut eine Ablage nach — ohne Datenbank, ohne die echte CLI."""
    ...
    def lauf_anlegen(self, oben, ms, befunde, mit_git=True, verzeichnis=None):
        """Einen Lauf-Ordner bauen, wie die CLI ihn hinterlaesst.
        ...
        for i, (datei, grad) in enumerate(befunde):
            (ordner / ("%08d-uuid.json" % i)).write_text(
                json.dumps(
                    {
                        "fileName": datei,
                        "startLine": 10 + i,
                        ...
                        "comment": "**Befund %d**\n\nText." % i,
                        "severity": grad,
                        "commentCategory": "FUNCTIONAL_CORRECTNESS",
                        "type": "actionable",
                        "fingerprint": "f%d" % i,
                    }
                ),
        ...
        (ordner / "incrementalDiff.json").write_text('[{"filename": "a.py", "diff": "@@"}]', encoding="utf-8")
        return ordner
    ...
    def test_ein_lauf_mit_gitdatei_wird_gefunden(self):
        """GEGEBEN ein Lauf mit git.json — DANN zaehlen nur die Befunde."""
        self.lauf_anlegen("repohash", 1788000000000, [("a.py", "major"), ("b.py", "minor")])
        laeufe = self.lager().laeufe()
        self.assertEqual(len(laeufe), 1)
        self.assertEqual(laeufe[0]["anzahl"], 2, "Beidateien mitgezaehlt")
        self.assertEqual(laeufe[0]["uebersprungen"], 0, "Beidateien als unlesbaren Befund gewertet")
    ...
    def test_aelterer_lauf_ohne_gitdatei_kommt_mit(self):
        """GEGEBEN ein aelterer Lauf ohne git.json im selben Ordner —
        DANN steht er trotzdem in der Liste (der gemessene Normalfall)."""
        self.lauf_anlegen("repohash", 1788000000000, [("a.py", "major")])
        self.lauf_anlegen("repohash", 1787000000000, [("alt.py", "minor")], mit_git=False)
        laeufe = self.lager().laeufe()
        self.assertEqual(len(laeufe), 2, "aeltere Laeufe fallen heraus")
        # Neuester zuerst.
        self.assertEqual(laeufe[0]["id"], "1788000000000")
        self.assertFalse(laeufe[1]["belegt"], "ein Lauf ohne git.json darf nicht als belegt gelten")

    def test_fremdes_repo_bleibt_draussen(self):
        """GEGEBEN ein zweites Projekt in derselben Ablage —
        ...
        laeufe = self.lager().laeufe()
        dateien = [b["datei"] for lauf in laeufe for b in lauf["befunde"]]
        self.assertEqual(dateien, ["a.py"], "fremde Befunde eingemischt")

    def test_leerer_laufordner_faellt_heraus(self):
        """GEGEBEN ein abgebrochener Lauf ohne jede Datei —
    ...
    def test_ohne_ablage_keine_laeufe(self):
        """GEGEBEN gar keine Ablage — DANN eine leere Liste, kein Absturz."""
    ...
    def test_sortierung_schwer_zuerst(self):
        """GEGEBEN gemischte Schweregrade — DANN steht der schwerste oben."""
        self.lauf_anlegen(
            "repohash", 1788000000000, [("a.py", "minor"), ("b.py", "critical"), ("c.py", "major")]
        )
...
class BefundBasis(unittest.TestCase):
    ...
    def test_nichttextliches_feld_ist_kein_befund(self):
        ...
        for wert in (["x"], {"a": 1}, 42):
            ...
            self.assertFalse(Befund(roh).gueltig(), "nichttextlicher title (%r) durchgelassen" % (wert,))
        # GEGENPROBE: Der Wächter darf den Normalfall nicht mitnehmen.
        self.assertTrue(Befund({"fileName": "a.py", "title": "x"}).gueltig())
        # Und ein FEHLENDES Feld ist etwas anderes als ein falsches.
        self.assertTrue(Befund({"fileName": "a.py", "comment": "x", "title": None}).gueltig())

    def test_alle_stripfelder_stehen_im_waechter(self):
        """GEGEBEN irgendein Textfeld als Liste — DANN nie ein AttributeError.
        ...
        for feld in gesund:
            ...
            try:
                ...
                self.fail("Feld %r als Liste laesst als_dict() platzen: %s" % (feld, e))

    def test_stelle_nennt_die_zeilenspanne(self):
        """GEGEBEN Start- und Endzeile — DANN ``datei:12-18``."""
        self.assertEqual(
            Befund({"fileName": "a.py", "title": "x", "startLine": 12, "endLine": 18}).stelle, "a.py:12-18"
        )
        self.assertEqual(
            Befund({"fileName": "a.py", "title": "x", "startLine": 12, "endLine": 12}).stelle, "a.py:12"
        )
        self.assertEqual(Befund({"fileName": "a.py", "title": "x"}).stelle, "a.py")

    def test_kaputte_zeilennummer_stuerzt_nicht(self):
        """GEGEBEN ein Feld, das keine Zahl ist — DANN 0 statt Ausnahme.

        Fremde Felder ohne Netz durch ``int()`` zu schicken, ist ein
        Serverfehler, der erst auffaellt, wenn die CLI das Format aendert."""
        self.assertEqual(Befund({"fileName": "a.py", "title": "x", "startLine": "spaeter"}).zeile_von, 0)
