# -*- coding: utf-8 -*-
"""Die Fix-Werkzeuge stehen in einer Tabelle — mit eigenen Nummern.
...
class DieFixerStehenInEinerTabelle(BasisTest):
    ...
    def test_die_spalten_stehen_fest(self):
        ...
        self.assertEqual(namen, ["Rang", "Fix-Werkzeug", "Behebt", "Was es tut", "Grenzen", "Aktion"])
...
class DerAnwendenKnopfBrauchtEineVorschau(BasisTest):
    """Ein Knopf, der ohne Vorschau schreibt, wäre die Falle, gegen die es
    die Vorschau gibt."""
    ...
    def test_nach_einer_vorschau_mit_treffern_erscheint_anwenden(self):
        ...
        passend = [z for z in zeilen if "anwenden:%s" % erster.slug in z["zellen"][5]["html"]]
    ...
    def test_nur_der_gefragte_fixer_bekommt_den_knopf(self):
        ...
        mit = [z for z in SkillsView()._fixtabelle(fix)["zeilen"] if "anwenden:" in z["zellen"][5]["html"]]
        self.assertEqual(len(mit), 1)


class ZweiListenZweiAblagen(BasisTest):
    """Das Verschieben eines Fixers darf keine Prüfer-Nummer ändern."""
    ...
    def test_verschieben_ordnet_die_fixer_um(self):
        ...
        self.assertEqual(sorted(nachher), sorted(vorher), "Beim Verschieben darf keiner verschwinden.")
...
class KeineListeMehrNebenDerTabelle(BasisTest):
    ...
    def test_die_fixzeilen_sind_weg(self):
        ...
        self.assertEqual(
            markup.count('class="sk1-fixzeile"'),
            1,
    ...
    def test_die_tabelle_ist_eingebunden(self):
        ...
        markup = Path(get_template("djangobase/hilfe/skills.html").origin.name).read_text(encoding="utf-8")
