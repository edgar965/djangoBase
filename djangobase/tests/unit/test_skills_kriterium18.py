# -*- coding: utf-8 -*-
"""Kriterium 18 — freie Funktionen und globale Variablen in Klassen.
...
class Kriterium18Test(BasisTest):
    """Die beiden Werkzeuge zu Kriterium 18 gegen echte Dateien."""
    ...
    def test_utility_kandidat_wird_getrennt_gemeldet(self):
        """Ohne Zustand ist es eine Utility-Klasse - ein ANDERER Umbau."""
        satz = self._lauf(
            "klassen-kandidat",
            {
                "texte.py": (
    ...
    def test_alias_auf_methode_ist_kein_zustand(self):
        """``_kurz = Klasse.methode`` ist ein zweiter NAME, kein Zustand.
    ...
    def test_wegwerfname_wird_nicht_gemeldet(self):
        """``_`` aus einer Tupel-Entpackung ist kein globaler Zustand."""
        satz = self._lauf(
            "globaler-zustand",
            {
                "entpacken.py": (
    ...
    def test_ablaufskript_wird_uebersprungen(self):
        """In einem Skript IST die Modulebene das Programm.
        ...
        self.assertTrue(
            any("skript" in z.lower() for z in satz.kopf), "die uebersprungenen Skripte müssen im Kopf stehen"
        )

    def test_framework_funktionen_sind_keine_utility_kandidaten(self):
        """Wer vom RAHMENWERK gerufen wird, kann nicht in eine Klasse wandern.
        ...
        self.assertFalse(
            [b for b in satz.befunde if "Utility" in b.was], "Django-Views duerfen kein Utility-Kandidat sein"
        )
        ...
        self.assertFalse(
            [b for b in satz2.befunde if "Utility" in b.was],
    ...
    def test_was_schon_im_kontext_liegt_ist_kein_kandidat(self):
        ...
        for quelle, erwartet in faelle:
            ...
            self.assertEqual(werkzeug._sorte(knoten), erwartet, "falsch eingestuft: %s" % quelle)
