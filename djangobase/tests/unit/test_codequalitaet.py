# -*- coding: utf-8 -*-
"""Code-Qualität mit vier etablierten Werkzeugen.
...
class AlleVierLaufen(BasisTest):
    ...
    def test_vier_verfahren_stehen_da(self):
        namen = self._namen(_messen({"a.py": "x = 1\n"}))
    ...
    def test_jedes_nennt_sein_werkzeug(self):
        ...
        werkzeuge = set(v["werkzeug"] for v in _messen({"a.py": "x = 1\n"}).als_liste())
...
class JedesVerfahrenFindetSeinen(BasisTest):
    ...
    def test_pyflakes_findet_die_unbenutzte_variable(self):
        v = self._eines(_messen({"a.py": FEHLERHAFT}), "Echte Fehler")
    ...
    def test_arten_ist_ueberall_eine_liste_von_paaren(self):
        for v in _messen({"a.py": FEHLERHAFT + ZU_LANG}).als_liste():
            ...
            for eintrag in arten:
                self.assertEqual(len(eintrag), 2, "%s: %r" % (v["name"], eintrag))
...
class DasProjektSagtWieLangEineZeileSeinDarf(BasisTest):
    """``setup.cfg`` schlägt pycodestyles Vorgabe von 79 Zeichen.
    ...
    def test_ohne_konfiguration_gilt_die_vorgabe(self):
        ...
        self.assertGreaterEqual(
            stil["zahlen"]["gesamt"], 1, "Ohne setup.cfg muss die 90-Zeichen-Zeile als E501 auffallen."
        )

    def test_setup_cfg_hebt_die_grenze(self):
        werte = _messen({"m.py": self.MITTELLANG, "setup.cfg": "[pycodestyle]\nmax_line_length = 100\n"})
        stil = [v for v in werte.als_liste() if "PEP 8" in v["name"]][0]
        self.assertEqual(
            stil["zahlen"]["gesamt"],
            0,
    ...
    def test_tox_ini_geht_auch(self):
        werte = _messen({"m.py": self.MITTELLANG, "tox.ini": "[pycodestyle]\nmax_line_length = 100\n"})
        stil = [v for v in werte.als_liste() if "PEP 8" in v["name"]][0]
        self.assertEqual(stil["zahlen"]["gesamt"], 0)

    def test_eine_wirklich_zu_lange_zeile_faellt_weiter_auf(self):
        """Die Gegenprobe: Die Grenze heben heißt nicht abschalten."""
        werte = _messen({"m.py": ZU_LANG, "setup.cfg": "[pycodestyle]\nmax_line_length = 100\n"})
        stil = [v for v in werte.als_liste() if "PEP 8" in v["name"]][0]
...
class WasNichtGemessenWird(BasisTest):
    def test_laufzeitdaten_bleiben_draussen(self):
        m = _messen({"echt.py": "x = 1\n", "media/weg.py": FEHLERHAFT, "logs/auch_weg.py": FEHLERHAFT})
        self.assertEqual(len(m.dateien), 1)

    def test_ein_fehlendes_werkzeug_meldet_sich(self):
        """Eine leere Trefferliste sähe aus wie „nichts gefunden"."""
        v = Verfahren("Test", "gibtsnicht", "nichts")
        v.fehlt = "gibtsnicht"
