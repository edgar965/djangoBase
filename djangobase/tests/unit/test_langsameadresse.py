# -*- coding: utf-8 -*-
"""``localhost`` als Verbindungsziel — die Ausnahmen sind das Schwierige.
...
class EineAdresseImCode(WerkzeugBasis):
    """Gegeben: Eine Zeichenkette, die ein Verbindungsziel ist."""

    def test_sie_wird_gemeldet(self):
        projekt = self.projekt({"klient.py": "BASIS = 'http://localhost:11434'\n"})
    ...
    def test_jedes_schema(self):
        """Die Aufloesung haengt am Namen, nicht am Schema — ein
        Datenbankzugang ueber ``localhost`` kostet dasselbe."""
        projekt = self.projekt(
            {
                "a.py": (
                    "A = 'ws://localhost:9000/strom'\n"
    ...
    def test_127_0_0_1_nicht(self):
        projekt = self.projekt({"a.py": "BASIS = 'http://127.0.0.1:11434'\n"})
        self.assertEqual(projekt.fahren(LangsameAdresse), [])

    def test_und_0_0_0_0_auch_nicht(self):
        """Das ist die Adresse, auf der ein Server LAUSCHT."""
        projekt = self.projekt({"a.py": "ADRESSE = 'http://0.0.0.0:8090'\n"})
        self.assertEqual(projekt.fahren(LangsameAdresse), [])


class EinBeschriebenerFall(WerkzeugBasis):
    """Gegeben: Jemand schreibt AUF, dass ``localhost`` teuer ist.
    ...
    def test_aber_der_code_daneben_schon(self):
        """Die Gegenprobe zu den dreien: In derselben Datei mit
        Docstring UND Kommentar wird die echte Zeile gefunden."""
        projekt = self.projekt(
            {
                "a.py": (
                    'u"""Frueher http://localhost:11434."""\n'
...
class EineNamensliste(WerkzeugBasis):
    """Gegeben: ``localhost`` steht als NAME da, nicht als Adresse."""

    def test_allowed_hosts_wird_nicht_gemeldet(self):
        """Das ist der Vergleich gegen den ``Host``-Kopf einer
        EINGEHENDEN Anfrage — dort loest niemand etwas auf."""
        projekt = self.projekt({"settings.py": "ALLOWED_HOSTS = ['localhost', '127.0.0.1']\n"})
        self.assertEqual(projekt.fahren(LangsameAdresse), [])
    ...
    def test_csrf_trusted_origins_auch_nicht(self):
        """DER ERSTE FEHLALARM (28.08.2026, assistant).
    ...
    def test_aber_eine_echte_adresse_in_derselben_datei_schon(self):
        """Die Gegenprobe: Die Ausnahme gilt der Zuweisung, nicht der
...
class EinePruefung(WerkzeugBasis):
    """Gegeben: Ein Test, der die Adresse ABWEIST."""

    def test_er_wird_nicht_gemeldet(self):
        """DER ZWEITE FEHLALARM (28.08.2026, assistant).
    ...
    def test_auch_ohne_tests_ordner(self):
        """Zwei Haelften, zwei Faelle: Diese Datei liegt in KEINEM
        ...
        projekt = self.projekt({"app/test_ssrf.py": "ZIEL = 'http://localhost:8001'\n"})
        self.assertEqual(projekt.fahren(LangsameAdresse), [])

    def test_auch_im_tests_ordner_ohne_praefix(self):
        projekt = self.projekt({"app/tests/hilfe.py": "ZIEL = 'http://localhost:8001'\n"})
        self.assertEqual(projekt.fahren(LangsameAdresse), [])
...
class EineJsDatei(WerkzeugBasis):
    """Gegeben: Die Adresse steht im Frontend."""

    def test_sie_wird_gefunden(self):
        projekt = self.projekt({"static/app.js": "const B = 'http://localhost:8001/api';\n"})
        zeilen = projekt.fahren(LangsameAdresse)
        self.assertEqual(len(zeilen), 1, zeilen)
...
class EineKaputteDatei(WerkzeugBasis):
    ...
    def test_sie_wirft_nicht(self):
        projekt = self.projekt({"kaputt.py": "def (:\n", "gut.py": "B = 'http://localhost:11434'\n"})
        zeilen = projekt.fahren(LangsameAdresse)
        self.assertEqual(len(zeilen), 1, zeilen)
