# -*- coding: utf-8 -*-
"""Die Cache-Kopfzeile der Statik — auch im Entwicklungsbetrieb.
...
class Antwortprobe:
    """Eine winzige ASGI-Anwendung, die eine Antwort schickt und mitschreibt,
    ...
    async def __call__(self, scope, receive, send):
        self.aufgerufen += 1
        await send({"type": "http.response.start", "status": 200, "headers": list(self.kopfzeilen)})
        await send({"type": "http.response.body", "body": b"x"})

    def lauf(self, pfad, query=b"", typ="http", mit_schluesselwoertern=False):
        """Die Hülle einmal fahren; liefert das Kopfzeilen-Wörterbuch."""
        gesammelt = {}
...
class DieHuelleSitztRichtig(SimpleTestCase):
    def test_die_huelle_vertraegt_schluesselwoerter(self):
        """DER FALL, DER PASSIERT IST: Daphne ruft mit `scope=`, `receive=`,
        `send=` auf. Eigene Parameternamen ergeben HTTP 500 auf allem."""
        kopf = Antwortprobe().lauf("/static/a/x.js", mit_schluesselwoertern=True)
    ...
    def test_die_innere_anwendung_wird_immer_gerufen(self):
        """Die Hülle setzt Kopfzeilen, sie beantwortet nichts selbst."""
        probe = Antwortprobe()
...
class WasWelcheKopfzeileBekommt(SimpleTestCase):
    ...
    def test_eine_leere_kennung_zaehlt_nicht(self):
        """`?v=` ohne Wert entsteht aus `{{ fassung }}`, wenn die Variable
    ...
    def test_ausserhalb_der_statik_wird_nichts_gesetzt(self):
        """Seiten macht die Middleware, APIs entscheidet das Projekt."""
    ...
    def test_eine_vorhandene_kopfzeile_wird_ersetzt(self):
        """Nicht angehängt: Zwei `Cache-Control`-Zeilen fasst der Browser
        zusammen, und aus `no-cache` plus `max-age` wird etwas, das niemand
        gemeint hat."""
        probe = Antwortprobe([(b"content-type", b"text/css"), (b"cache-control", b"max-age=999")])
        kopf = probe.lauf("/static/app/x.css")
        self.assertEqual(kopf.get(b"cache-control"), b"no-cache")

    def test_andere_kopfzeilen_bleiben(self):
        probe = Antwortprobe([(b"content-type", b"text/css"), (b"last-modified", b"gestern")])
        kopf = probe.lauf("/static/app/x.css")
...
class WasSieInRuheLaesst(SimpleTestCase):
    def test_websockets_gehen_unveraendert_durch(self):
        """Ein `websocket`-Scope kennt gar keine Kopfzeilen — hier etwas zu
    ...
    def test_ein_anderes_static_url_wird_geachtet(self):
        self.assertIsNone(Antwortprobe().lauf("/static/app/x.css").get(b"cache-control"))
        self.assertEqual(Antwortprobe().lauf("/dateien/app/x.css").get(b"cache-control"), b"no-cache")
