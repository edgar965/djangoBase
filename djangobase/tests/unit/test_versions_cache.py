...
class TTLCacheTest(SimpleTestCase):
    ...
    def test_abgelaufener_abruf_liefert_sofort_den_alten_wert(self):
        ...
        for _ in range(40):  # auf den Faden warten
            if c.get_or_compute("k", langsam) == "neu":
                break
            time.sleep(0.1)
        self.assertEqual(c.get_or_compute("k", langsam), "neu", "der erneuerte Wert kam nicht an")
    ...
    def test_nur_ein_erneuerungsfaden_je_schluessel(self):
        ...
        time.sleep(1.2)
        self.assertEqual(zaehler["n"], 1, "es liefen %d Erneuerungen statt einer" % zaehler["n"])
    ...
    def test_fehler_im_hintergrund_laesst_den_alten_wert_stehen(self):
        ...
        time.sleep(0.3)
        self.assertEqual(
            c.get_or_compute("k", kaputt), "alt", "nach einem Fehler in der Erneuerung ist der Wert weg"
        )
