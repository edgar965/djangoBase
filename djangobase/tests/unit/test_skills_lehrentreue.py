# -*- coding: utf-8 -*-
"""Lehrentreue — findet es die fünf prüfbaren Regeln, und nur die?
...
def _lehren(satz):
    return {b.was.rsplit("„", 1)[-1].rstrip('")') for b in satz.befunde}
...
class EsFindetDieFuenfMuster(BasisTest):
    ...
    def test_nachbarsuche_ohne_workers(self):
        satz = _lauf(
            {
                "a.py": "from scipy.spatial import cKDTree\n\n\n"
                "def f(p):\n"
    ...
    def test_distinct_wiegt_schwerer(self):
        """Es ist das einzige Muster, das ein falsches ERGEBNIS liefert."""
        satz = _lauf({"a.py": "def f(qs):\n    return qs.values_list('x').distinct()\n"})
...
class EsMeldetNichtsFalsches(BasisTest):
    ...
    def test_query_ohne_kdtree_zaehlt_nicht(self):
        """``.query()`` heisst anderswo etwas ganz anderes — bei Django
    ...
    def test_distinct_mit_ordnung_ist_richtig(self):
        satz = _lauf({"a.py": "def f(qs):\n    return qs.order_by().values_list('x').distinct()\n"})
        self.assertEqual(satz.befunde, [])

    def test_distinct_ohne_values_list_zaehlt_nicht(self):
        """``Model.objects.distinct()`` allein hat das Problem nicht."""
        satz = _lauf({"a.py": "def f(qs):\n    return qs.distinct()\n"})
        self.assertEqual(satz.befunde, [])
...
class JedeGemeldeteLehreGibtEsAuch(BasisTest):
    def test_die_genannten_lehren_stehen_in_LEHREN(self):
        ...
        bekannt = {lehre.slug for lehre in LEHREN}
        self.assertTrue(
            _lehren(satz) <= bekannt, "Gemeldet, aber keine Lehre: %s" % (_lehren(satz) - bekannt)
        )
        self.assertEqual(len(_lehren(satz)), 5)

    def test_die_fuenf_lehren_zeigen_zurueck_auf_das_werkzeug(self):
        """Auf beiden Seiten dasselbe: Die Lehre nennt ihr Werkzeug, das
        ...
        da = {lehre.slug: lehre for lehre in LEHREN}
        for slug in fuenf:
...
class DerKopfZaehltJeLehre(BasisTest):
    ...
    def test_mit_befunden_steht_die_verteilung_da(self):
        ...
        self.assertTrue(any("keine-temp-dateien-im-system" in z for z in satz.kopf), satz.kopf)
