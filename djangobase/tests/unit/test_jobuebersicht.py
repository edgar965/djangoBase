# -*- coding: utf-8 -*-
"""Die Zeilen der Jobs-Seite: Bestand und Verlauf zusammengefuehrt.
...
class JobsSeiteBasis(MitTempordner):
    """Baut eine Uebersicht auf eigenen Dateien - ohne das echte Projekt."""
    ...
    def setUp(self):
        ...
        self.katalog = Jobkatalog(pfad=self.datei("jobkatalog.json"), erkennung=self.erkennung)
        self.verlauf = Jobverlauf(pfad=self.datei("joblaeufe.jsonl"))
        self.uebersicht = Jobuebersicht(katalog=self.katalog, verlauf=self.verlauf)
...
class EinJobDerNieLief(JobsSeiteBasis, SimpleTestCase):
    """Gegeben: ``mail_sync`` steht im Bestand, lief aber noch nie."""

    def test_steht_trotzdem_in_der_uebersicht(self):
        """Sonst faende man nie heraus, dass es ihn ueberhaupt gibt."""
...
class EinJobMitErfolgreichemLauf(JobsSeiteBasis, SimpleTestCase):
    """Gegeben: ``mail_sync`` lief erfolgreich, 12,5 Sekunden lang."""
    ...
    def test_liefert_den_zeitpunkt_auch_als_datum(self):
        """Als Text stuende auf der Seite UTC — also 21:11 statt 23:11.
...
class EinJobDerZuletztScheiterte(JobsSeiteBasis, SimpleTestCase):
    """Gegeben: Von zwei Jobs ist einer gescheitert."""
    ...
    def setUp(self):
        ...
        self.verlauf.notieren("zzz_scheitert", dauer_s=0.2, erfolg=False, fehler="ValueError: kaputt")

    def test_steht_ganz_oben(self):
        """Wer die Seite oeffnet, sucht das Kaputte - trotz Z am Anfang."""
        self.assertEqual(self.uebersicht.zeilen()[0]["kennung"], "zzz_scheitert")
...
class EinLaufOhnePassendenJobImBestand(JobsSeiteBasis, SimpleTestCase):
    """Gegeben: Ein Lauf im Verlauf, dessen Befehl es nicht mehr gibt."""
    ...
    def test_wird_trotzdem_gezeigt(self):
        """Sonst verschwaende ein noch laufender Ablauf lautlos aus der Sicht."""
...
class EineUebersichtMitVielenJobs(JobsSeiteBasis, SimpleTestCase):
    """Gegeben: Drei Jobs im Bestand, einer davon gelaufen."""
    ...
    def test_ermittelt_auf_knopfdruck_neu(self):
        """Der Knopf "Jetzt aktualisieren" fragt nicht nach dem Alter."""
