# -*- coding: utf-8 -*-
"""`totes-modul` schlägt an — und nur da, wo wirklich nichts mehr hängt.
...
class TotesModulTest(SimpleTestCase):
    ...
    def test_paket_lebt_wenn_sein_inhalt_gebraucht_wird(self):
        """``from a.b.c import x`` hält auch ``a.b`` am Leben."""
        orte, kopf = _lauf(
            {
                "paket/__init__.py": "",
...
class LebendeModuleTest(SimpleTestCase):
    """Die drei Fälle, die eine frühere Fassung löschen wollte."""

    def test_sternimport_haelt_das_modul_am_leben(self):
        """``from .x import *`` — der Modulname steht nur in der Datei selbst.
    ...
    def test_ein_einziger_import_von_aussen_reicht(self):
        """``teil2..5``: Ein Import ergibt genau EINEN Treffer im Text.
    ...
    def test_nur_im_eigenen_modul_benutzt_ist_kein_toter_code(self):
        """Kapselung ist kein toter Code — 122 Namen auf einmal."""
        orte, kopf = _lauf(
            {
...
class KonventionTest(SimpleTestCase):
    """Was per Konvention gefunden wird, darf nirgends erwähnt sein."""
    ...
    def test_skript_OHNE_main_ist_kein_befund(self):
        """``depot/ohlcv_check.py``: kein ``__main__``, aber Django-Aufbau,
    ...
    def test_schleife_auf_modulebene_ist_ein_skript(self):
        """``depot/vol_futures_probe.py`` hat weder ``__main__`` noch einen
    ...
    def test_management_command_ist_kein_befund(self):
        orte, kopf = _lauf(
            {"app/management/commands/tun.py": "class Command:\n    def handle(self):\n        pass\n"}
        )
        self.assertEqual(orte, [], kopf)
...
class RueckkopplungTest(SimpleTestCase):
    """Der Bericht, den die Prüfung selbst schreibt, macht nichts lebendig."""
        ...
        orte, kopf = _lauf(
            {
                ...
                "ergebnis/befunde.md": "# Befunde\n\n- tot.py: Modul wird nirgends erwähnt (niemand)\n",
            }
        )
        self.assertEqual(orte, ["tot.py"], "der eigene Bericht darf das Modul nicht am Leben halten: " + kopf)

    def test_eine_echte_erwaehnung_in_einer_vorlage_zaehlt_sehr_wohl(self):
        """DIE GEGENPROBE: Nur das ERGEBNIS-Verzeichnis ist ausgenommen."""
        orte, kopf = _lauf(
            {
                "tot.py": "def niemand():\n    return 1\n",
