    ...
    def test_tippfehler_faellt_auf_die_strengste_stufe_zurueck(self):
        for falsch in ("stff", "Staff", "staff ", "STAFF", "", None, "admin"):
            with self.subTest(wert=falsch):
                self.assertEqual(
                    self._status(falsch, self.nutzer),
                    403,
                    "zugriff=%r liess einen Nicht-Berechtigten durch" % falsch,
                )
                self.assertEqual(
                    self._status(falsch, self.chef), 200, "zugriff=%r sperrt auch Berechtigte aus" % falsch
                )
