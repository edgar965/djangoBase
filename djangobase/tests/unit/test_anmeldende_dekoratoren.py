# -*- coding: utf-8 -*-
"""Funktionen, die ein Dekorator beim Rahmen ANMELDET.
...
def _funktion(quelle: str):
    """Der erste Funktionsknoten aus einem Schnipsel."""
...
class EinSignalhandler(unittest.TestCase):
    """Gegeben: Eine Funktion, die Django als Empfaenger anmeldet."""
    ...
    def test_ueber_ein_modul_geschrieben(self):
        """``@signals.receiver(...)`` — verglichen wird der letzte
        Namensteil."""
        knoten = _funktion("@signals.receiver(post_save)\ndef f(s, i, **k): pass\n")
        self.assertTrue(Rahmenvorschrift.wird_angemeldet(knoten))


class EinTemplatetag(unittest.TestCase):
    """Gegeben: Django sucht die Funktion beim Namen aus dem Register."""
    ...
    def test_ein_simple_tag_mit_argumenten(self):
        knoten = _funktion("@register.simple_tag(takes_context=True)\ndef x(context): pass\n")
        self.assertTrue(Rahmenvorschrift.wird_angemeldet(knoten))

    def test_ein_inclusion_tag(self):
        knoten = _funktion('@register.inclusion_tag("t.html")\ndef x(): pass\n')
        self.assertTrue(Rahmenvorschrift.wird_angemeldet(knoten))

    def test_das_register_darf_heissen_wie_es_will(self):
        """``register`` ist ein Projektname, ``filter`` nicht."""
        knoten = _funktion("@meine_bibliothek.filter\ndef x(w): pass\n")
        self.assertTrue(Rahmenvorschrift.wird_angemeldet(knoten))


class EineCeleryAufgabe(unittest.TestCase):
    """Gegeben: Ein Aufgabenplaner meldet die Funktion an."""

    def test_task(self):
        self.assertTrue(Rahmenvorschrift.wird_angemeldet(_funktion("@app.task\ndef x(): pass\n")))

    def test_shared_task_mit_argumenten(self):
        self.assertTrue(
            Rahmenvorschrift.wird_angemeldet(_funktion("@shared_task(bind=True)\ndef x(self): pass\n"))
        )


class EinUmhuellenderDekorator(unittest.TestCase):
    """Gegeben: Ein Dekorator, der nichts anmeldet.
    ...
    def test_require_post_zaehlt_nicht(self):
        self.assertFalse(Rahmenvorschrift.wird_angemeldet(_funktion("@require_POST\ndef x(request): pass\n")))

    def test_csrf_exempt_auch_nicht(self):
        self.assertFalse(Rahmenvorschrift.wird_angemeldet(_funktion("@csrf_exempt\ndef x(request): pass\n")))

    def test_contextmanager_nicht(self):
        self.assertFalse(Rahmenvorschrift.wird_angemeldet(_funktion("@contextmanager\ndef x(): pass\n")))

    def test_transaction_atomic_nicht(self):
        self.assertFalse(Rahmenvorschrift.wird_angemeldet(_funktion("@transaction.atomic\ndef x(): pass\n")))

    def test_ganz_ohne_dekorator_erst_recht_nicht(self):
        self.assertFalse(Rahmenvorschrift.wird_angemeldet(_funktion("def x(): pass\n")))


class EineGemischteDatei(unittest.TestCase):
    """Gegeben: Signalhandler UND normale Funktionen in einer Datei.
    ...
    def test_nur_der_handler_faellt_weg(self):
        angemeldet = [
            k.name
            for k in ast.parse(self.QUELLE).body
            if isinstance(k, ast.FunctionDef) and Rahmenvorschrift.wird_angemeldet(k)
        ]
    ...
    def test_die_helfer_bleiben_kandidaten(self):
        offen = [
            k.name
            for k in ast.parse(self.QUELLE).body
            if isinstance(k, ast.FunctionDef) and not Rahmenvorschrift.wird_angemeldet(k)
        ]
        self.assertEqual(offen, ["_compute_ctag", "_refresh_ctag"])


class DieListe(unittest.TestCase):
    """Gegeben: Die Liste der anmeldenden Dekoratoren."""
    ...
    def test_und_nicht_die_umhuellenden(self):
        for name in ("require_POST", "csrf_exempt", "contextmanager", "atomic", "property", "staticmethod"):
            self.assertNotIn(name, Rahmenvorschrift.ANMELDENDE_DEKORATOREN, name)
