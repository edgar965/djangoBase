# -*- coding: utf-8 -*-
"""Rahmenmodule: was ein Laufzeit-``__all__`` ist — und was der Filter NICHT anfasst."""
...
def _befund(datei, regel, stufe="error"):
    return {"datei": datei, "zeile": 1, "spalte": 1, "stufe": stufe, "regel": regel, "text": "egal"}


class RahmenmoduleTest(unittest.TestCase):
    def _projekt(self, dateien):
        """Ein Wegwerf-Projekt; liefert (Ordner, Rahmenmodule)."""
    ...
    def test_wer_per_stern_aus_einem_rahmen_holt_ist_konsument(self):
        ordner, rm = self._projekt({"app/rahmen.py": RAHMEN, "app/seite.py": KONSUMENT})
        rm.einlesen({"app/seite.py"})
        self.assertEqual(rm.konsumenten(), {"app/seite.py"})
        self.assertIn("app/rahmen.py", rm.rahmen(), "das Ziel wird mitgelesen, auch ohne eigenen Befund")

    def test_stern_import_aus_einem_STATISCHEN_modul_zaehlt_nicht(self):
        """Steht ``__all__`` als Liste da, kann der Typprüfer es auflösen —
        dann ist eine Meldung über einen unbekannten Namen echt."""
        ordner, rm = self._projekt({"app/rahmen.py": STATISCH, "app/seite.py": KONSUMENT})
        rm.einlesen({"app/seite.py"})
        self.assertEqual(rm.konsumenten(), set())

    def test_unaufloesbares_ziel_schaltet_nichts_stumm(self):
        ordner, rm = self._projekt({"app/seite.py": "from .fehlt import *\nx = y\n"})
        rm.einlesen({"app/seite.py"})
        self.assertEqual(rm.konsumenten(), set())

    def test_kaputte_datei_wirft_nicht(self):
        ordner, rm = self._projekt({"app/seite.py": "def (:::\n"})
        rm.einlesen({"app/seite.py"})
    ...
    def test_haken_aus_zeigt_alles_haken_an_zaehlt_es_in_der_kennzahl(self):
        ...
        aus = LsBefunde(erg, LsKonfig({"rahmen_stumm": False, "js_stumm": []}), Rahmenmodule(ordner))
        self.assertEqual(len(aus.roh()), 2)
        self.assertEqual(aus.kennzahlen()["stumm_rahmen"], 0)

        an = LsBefunde(erg, LsKonfig({"rahmen_stumm": True, "js_stumm": []}), Rahmenmodule(ordner))
    ...
    def test_ohne_rahmenobjekt_bleibt_alles_wie_bisher(self):
        """Andere Aufrufer von ``LsBefunde`` dürfen nichts merken."""
