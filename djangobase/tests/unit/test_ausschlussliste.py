# -*- coding: utf-8 -*-
"""Ausschlussliste: deuten, lesen, schreiben — und was sie ablehnt."""
...
class AusschlusslisteTest(unittest.TestCase):
    ...
    def test_nackter_name_gilt_in_jeder_tiefe_pfad_ab_der_wurzel(self):
        with tempfile.TemporaryDirectory() as d:
            liste = self._liste(d, "# Kommentar\n\nsicherung\nwerkzeug/netz_*.py\n**/*.min.js\n")
            self.assertEqual(liste.muster(), ["**/sicherung", "werkzeug/netz_*.py", "**/*.min.js"])
            self.assertEqual(
                liste.namen(), ["sicherung"], "Globs kann Werkzeug.ausgeschlossen() nicht deuten"
            )

    def test_schraegstriche_und_schluss_werden_vereinheitlicht(self):
        with tempfile.TemporaryDirectory() as d:
            liste = self._liste(d, "werkzeug\\sicherung\\\n./brain/alt/\n")
