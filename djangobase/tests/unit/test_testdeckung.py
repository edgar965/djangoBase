# -*- coding: utf-8 -*-
"""Tests der Routen-Auswertung in `testdeckung`.
...
class RoutenTest(BasisTest):
    def _routen(self):
        """``_routen`` mit leeren Testtexten - alles gilt als ungeprüft."""
    ...
    def test_modul_kommt_von_der_klasse(self):
        """``_modul`` fragt ``view_class`` - sonst waere jede klassenbasierte
        ...
        self.assertTrue(Testdeckung._modul(RedirectView.as_view(url="/")).startswith("django."))
        self.assertEqual(Testdeckung._modul(freie_ansicht), PROJEKTMODUL)


class _Knoten:
    """So viel Syntaxknoten, wie die Pruefung anfasst."""
...
def _ausnahme(basen=("Exception",), mit_rumpf=False):
    ...
    if mit_rumpf:
        rumpf.append(ast.FunctionDef(name="tu", args=None, body=[], decorator_list=[]))
    return _Bezug(_Knoten(basen, rumpf))


class EineAusnahmeOhneEigenenRumpf(BasisTest):
    """Gegeben: ``class FreigabeTimeout(Exception): '''...'''``
    ...
    def test_auch_bei_einer_eigenen_fehlerbasis(self):
        self.assertTrue(Testdeckung._nur_eine_ausnahme(_ausnahme(("BankError",))))


class EineAusnahmeMitEigenemVerhalten(BasisTest):
    """Gegeben: Eine Ausnahme, die selbst etwas tut.

    Die Gegenprobe. Wer eine Methode traegt, hat etwas zu pruefen.
    """

    def test_bleibt_ein_testziel(self):
        self.assertFalse(Testdeckung._nur_eine_ausnahme(_ausnahme(mit_rumpf=True)))


class EineGewoehnlicheKlasse(BasisTest):
    """Gegeben: Eine Klasse, die keine Ausnahme ist."""

    def test_bleibt_ein_testziel(self):
        self.assertFalse(Testdeckung._nur_eine_ausnahme(_ausnahme(("object",))))
