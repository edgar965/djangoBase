# -*- coding: utf-8 -*-
"""`data-sort-aus`: eine Tabelle, deren Zeilenfolge etwas bedeutet.
...
class OrdnungZaehltTest(SimpleTestCase):
    """Die Marke nimmt von der Sortier-Pflicht aus - und nur davon."""

    databases = []

    def _pruefen(self, attribute):
        """Eine Wegwerf-Vorlage bauen und die beiden Regeln darauf fahren.
        ...
        (ordner / "templates" / "seite.html").write_text(VORLAGE % attribute, encoding="utf-8")
        with override_settings(
            BASE_DIR=str(ordner), DJANGOBASE_KONFORM_AUS=[], DJANGOBASE_KONFORM_TABELLEN_AUS=[]
        ):
        ...
        ohne_sortierung = "sortable" not in attr.lower() and marke not in attr.lower()
        return ohne_sortierung, "data-sort-key" not in attr.lower()

    def test_ohne_alles_fehlt_beides(self):
        """Gegenprobe: Die Regeln greifen ueberhaupt."""
        self.assertEqual(self._pruefen('class="bwa-table"'), (True, True))

    def test_marke_nimmt_von_der_sortierung_aus(self):
        self.assertEqual(self._pruefen('class="bwa-table" data-sort-aus data-sort-key="bwa"'), (False, False))

    def test_marke_ersetzt_den_schluessel_nicht(self):
        """Breiten bleiben Pflicht - sie aendern keine Reihenfolge."""
        self.assertEqual(self._pruefen('class="bwa-table" data-sort-aus'), (False, True))

    def test_sortierbare_tabelle_braucht_die_marke_nicht(self):
        self.assertEqual(
            self._pruefen('class="mail-table sortable" data-sort-key="mail-liste"'), (False, False)
        )

    # ---------------------------------------------- Kopfzeile je Tabelle

    def _tabellen(self, quelle):
        """``datentabellen()`` auf einer Wegwerf-Vorlage mit `quelle`."""
        ordner = Path(tempfile.mkdtemp(prefix="djb_tabellen_"))
        self.addCleanup(shutil.rmtree, ordner, True)
        (ordner / "templates").mkdir()
        (ordner / "templates" / "seite.html").write_text(quelle, encoding="utf-8")
        with override_settings(
            BASE_DIR=str(ordner), DJANGOBASE_KONFORM_AUS=[], DJANGOBASE_KONFORM_TABELLEN_AUS=[]
        ):
...
"""
    ...
    def test_kopfzeile_aus_einem_include_zaehlt_weiter(self):
        """Gegenprobe: Wer seine Kopfzeile einbindet, faellt nicht heraus."""
