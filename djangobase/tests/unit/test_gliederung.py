# -*- coding: utf-8 -*-
"""Die Gliederung nach Rolle — von Klassen UND Funktionen benutzt.
...
class DieRolleStehtAmPfad(BasisTest):
    ...
    def test_nicht_am_klassennamen(self):
        """`VideoCodecProbe` in `views/` ist keine Prüfung."""
...
class DieUntergruppe(BasisTest):
    ...
    def test_ohne_verzeichnis_gilt_der_dateiname(self):
        """„(Wurzel)" sagte nichts — jetzt steht dort `models.py`."""
...
class DieSummeStimmt(BasisTest):
    ...
    def test_die_summe_ist_die_zahl_der_eintraege(self):
        ...
        self.assertEqual(sum(r["zahl"] for r in gruppen), len(self.EINTRAEGE))

    def test_kein_name_steht_doppelt(self):
        alle = [n for r in nach_rolle(self.EINTRAEGE) for g in r["gruppen"] for n in g["namen"]]
    ...
    def test_die_untergruppen_zaehlen_zur_rolle(self):
        for r in nach_rolle(self.EINTRAEGE):
            self.assertEqual(r["zahl"], sum(g["zahl"] for g in r["gruppen"]))
...
class KleineRollenStehenFlach(BasisTest):
    """Die Ebene der Verzeichnisse nur, wo sie etwas austrägt.
    ...
    def test_eine_einzige_untergruppe_ist_immer_flach(self):
        """Auch bei vielen Einträgen: Die Ebene sagt dann nichts."""
    ...
    def test_flach_heisst_nicht_weniger(self):
        """Die Namen stehen dann direkt an der Rolle — keiner fällt weg."""
