# -*- coding: utf-8 -*-
"""Die Beurteilung eines ``data-sort``-Werts — ohne laufenden Server.
...
class DieDeutscheLesart(SimpleTestCase):
    """``zahl()`` bildet ``tabellen_sortierung._zahl`` nach — wörtlich.
    ...
    def test_jeder_punkt_gilt_als_tausenderzeichen(self):
        """Der Kern des Befunds: „20.9" wird nicht 20,9 sondern 209."""
        self.assertEqual(Sortierwert.zahl("20.9"), 209.0)

    def test_eine_einheit_dahinter_stoert_nicht(self):
        self.assertEqual(Sortierwert.zahl("17,4 GB"), 17.4)

    def test_ein_waehrungszeichen_hinten_auch_nicht(self):
        """Der erste Fehlalarm: „1.234,5 €" galt als unlesbar."""
        self.assertEqual(Sortierwert.zahl("1.234,5 €"), 1234.5)

    def test_text_ist_keine_zahl(self):
        self.assertIsNone(Sortierwert.zahl("Gemma 4"))
...
class DieDreiEchtenBefunde(SimpleTestCase):
    """Alle drei sind am 01.09.2026 auf Hilfe → KI-Modelle aufgetreten."""

    def test_eine_einheit_im_schluessel(self):
        """``data-sort="137M"`` stellte 137 Millionen über 122 Milliarden."""
        befund = Sortierwert("137M", "137M").befund()
        self.assertIsNotNone(befund)
        self.assertIn("Einheit", befund)

    def test_ein_dezimalpunkt_im_schluessel(self):
        """Der Anlassfall des Werkzeugs, wörtlich aus ``_WIEDERHOLT``."""
        befund = Sortierwert("20.9", "20,9 B").befund()
        self.assertIsNotNone(befund)
        self.assertIn("Dezimalpunkt", befund)

    def test_ein_leerer_schluessel_bei_gefuellter_zelle(self):
        """``{{ x|default:'' }}`` mit ``x = 0`` — Django hält 0 für leer,
        und die Spalte sortiert danach gar nicht."""
        befund = Sortierwert("", "12 GB").befund()
        self.assertIsNotNone(befund)
        self.assertIn("leerer Sortierschlüssel", befund)

    def test_der_schluessel_des_werkzeugs_wird_wirklich_gemeldet(self):
        """Gegenprobe am Werkzeug selbst: ``_WIEDERHOLT`` ist der Fall,
        den es melden MUSS. Wer ihn nur als Kommentar führt, hat nichts."""
        self.assertIn("20.9B", Sortierwerte._WIEDERHOLT)
        self.assertIsNotNone(Sortierwert("20.9B", "20.9B").befund())


class WasNichtGEMELDETWerdenDarf(SimpleTestCase):
    """Die Fehlalarme des ersten Laufs — jeder einzeln festgehalten.
    ...
    def test_ein_iso_datum_ist_kein_befund(self):
        """ISO sortiert als Text völlig richtig."""
        self.assertIsNone(Sortierwert("2026-08-11", "11.08.2026").befund())

    def test_ein_deutsches_datum_in_der_zelle_auch_nicht(self):
        self.assertIsNone(Sortierwert("2026-08-11", "11.08.2026").befund())

    def test_ein_sauberer_zahlenschluessel_ist_kein_befund(self):
        """Ohne Punkt und ohne Einheit ist alles in Ordnung.
        ...
        """
        self.assertIsNone(Sortierwert("17,4", "17,4 GB").befund())
        self.assertIsNone(Sortierwert("1234", "1.234").befund())
        self.assertIsNone(Sortierwert("-3", "-3").befund())

    def test_ein_prozentzeichen_ist_erlaubt(self):
        """``%`` steht in der JS-Regel ausdrücklich drin."""
        self.assertIsNone(Sortierwert("42%", "42 %").befund())

    def test_ein_leerer_schluessel_bei_leerer_zelle_ist_richtig(self):
        """„Nichts da" ist ein Zustand, kein Fehler."""
        self.assertIsNone(Sortierwert("", "—").befund())
        self.assertIsNone(Sortierwert("", "").befund())

    def test_ein_textschluessel_bei_textzelle_ist_richtig(self):
        self.assertIsNone(Sortierwert("gemma", "Gemma 4").befund())


class DasWerkzeugSagtWarumEsKeinenAnlassfallHat(SimpleTestCase):
    """Ein Werkzeug ohne Beispiel muss einen Grund nennen — und der Grund
    ...
    def test_er_nennt_diese_pruefdatei(self):
        """Sonst liest sich der Grund wie „geht nicht" statt wie
        „geht anders, und zwar dort"."""
        self.assertIn("test_sortierwerte", Sortierwerte.ohne_anlassfall_weil)
