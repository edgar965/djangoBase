# -*- coding: utf-8 -*-
"""Der Job-Verlauf: was lief wann, wie lange, mit welchem Ausgang.
...
class EinLeererVerlauf(MitTempordner, SimpleTestCase):
    """Gegeben: Es wurde noch nie ein Job ausgefuehrt."""
...
class EinVerlaufMitEinemErfolgreichenLauf(MitTempordner, SimpleTestCase):
    """Gegeben: ``mail_sync`` lief einmal und war erfolgreich."""
...
class EinVerlaufMitMehrerenLaeufen(MitTempordner, SimpleTestCase):
    """Gegeben: Zwei Jobs sind mehrfach gelaufen."""
    ...
    def test_nennt_den_juengsten_zuerst(self):
        """Wer auf die Seite sieht, will den letzten Stand, nicht den ersten."""
        self.assertEqual(self.verlauf.letzter("mail_sync")["dauer_s"], 3.0)

    def test_zaehlt_alle_laeufe_des_jobs(self):
        self.assertEqual(self.verlauf.zusammenfassung()["mail_sync"]["laeufe"], 2)

    def test_trennt_die_jobs_voneinander(self):
        """Ein langer Indexlauf darf die Zahlen des Mail-Abrufs nicht faerben."""
...
class EinVerlaufMitEinemFehlschlag(MitTempordner, SimpleTestCase):
    """Gegeben: Der letzte Lauf ist mit einer Ausnahme geendet."""

    def setUp(self):
        super().setUp()
        self.verlauf = Jobverlauf(pfad=self.datei("joblaeufe.jsonl"))
        self.verlauf.notieren("mail_sync", dauer_s=0.4, erfolg=False, fehler="ConnectionError: kein Server")

    def test_nennt_den_fehlertext(self):
        """Ohne Text muesste man ins Log — die Seite soll es selbst sagen."""
        self.assertIn("ConnectionError", self.verlauf.letzter("mail_sync")["fehler"])
    ...
    def test_zaehlt_ihn_als_fehler(self):
        self.assertEqual(self.verlauf.zusammenfassung()["mail_sync"]["fehler"], 1)

    def test_kuerzt_sehr_lange_fehlertexte(self):
        """Ein Traceback mit 40 kB darf die Datei nicht sprengen."""
        self.verlauf.notieren("gross", dauer_s=0.1, erfolg=False, fehler="x" * 5000)
...
class EineBeschaedigteVerlaufsdatei(MitTempordner, SimpleTestCase):
    """Gegeben: Eine Zeile ist unvollstaendig (Absturz beim Schreiben)."""
    ...
    def test_ueberspringt_die_kaputte_zeile(self):
        """Eine halbe Zeile darf die Laeufe davor und danach nicht kosten."""
...
class EinVerlaufAmSchreibenGehindert(MitTempordner, SimpleTestCase):
    """Gegeben: Der Zielort ist nicht beschreibbar (Ordner ist eine Datei)."""
    ...
    def test_wirft_nicht(self):
        """Der Verlauf beobachtet nur — er darf nie einen Job scheitern lassen."""
        self.verlauf.notieren("mail_sync", dauer_s=1.0, erfolg=True)
