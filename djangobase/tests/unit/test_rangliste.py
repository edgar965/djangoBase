# -*- coding: utf-8 -*-
"""Rangliste und Werkzeugwahl - die stillen Wege.
...
class Attrappe:
    """Ein Werkzeug, so weit die beiden Klassen es anfassen."""
...
class RanglisteTest(SimpleTestCase):
    ...
    def setUp(self):
        ...
        for n, b in enumerate(BEREICHE):
            k = b["kriterien"][0]
            self.werkzeuge += [Attrappe("w%d_%s" % (n, i), kriterium=k) for i in ("a", "b")]
    ...
    def test_grundordnung_folgt_den_bereichen(self):
        """Ohne Ablage bestimmt das Kriterium die Reihenfolge."""
    ...
    def test_jede_nummer_genau_einmal(self):
        """Der Kern der Ansage: EINE eindeutige Nummer je Eintrag.
    ...
    def test_verschieben_laesst_andere_rutschen(self):
        ...
        self.assertEqual(
            neu.rang_von(vorher_erster, self.werkzeuge), 2, "der vorher Erste muss auf 2 rutschen"
        )

    def test_verschieben_wechselt_den_bereich(self):
        """Die Ansage wörtlich: „dann rutscht er in den neuen Bereich".
    ...
    def test_bereichsgroessen_bleiben(self):
        """Ein Bereich wächst nicht - das letzte Werkzeug fällt hinaus.
        ...
        nachher = [len(a["eintraege"]) for a in Rangliste(self.pfad).abschnitte(self.werkzeuge)]
        self.assertEqual(vorher, nachher)

    def test_zurueck_auf_grundordnung_loescht_die_datei(self):
        """Eine Ablage, die nur wiederholt was der Code sagt, legt ihn still."""
        r = self._liste()
        letzter = self.werkzeuge[-1].slug
    ...
    def test_neues_werkzeug_landet_an_seiner_stelle(self):
        """Nicht hinten anhängen - dort sucht es niemand."""
        r = self._liste()
        r.verschieben(self.werkzeuge[-1].slug, 1, self.werkzeuge)
        ...
        self.assertLess(folge.index("frisch"), len(folge) - 1, "darf nicht am Ende kleben")

    def test_kaputte_datei_wirft_nicht(self):
        """Lieber die Grundordnung als eine tote Seite."""
        self.pfad.write_text("{kein json", encoding="utf-8")
        self.assertEqual(len(Rangliste(self.pfad).reihenfolge(self.werkzeuge)), len(self.werkzeuge))

    def test_ziel_ausserhalb_wird_begrenzt(self):
        """0 oder 999 ist ein Vertipper, kein Wunsch nach einer Lücke."""
        r = self._liste()
        r.verschieben(self.werkzeuge[-1].slug, 999, self.werkzeuge)
        raenge = [Rangliste(self.pfad).rang_von(w.slug, self.werkzeuge) for w in self.werkzeuge]
        self.assertEqual(sorted(raenge), list(range(1, len(self.werkzeuge) + 1)))
...
class WerkzeugwahlTest(SimpleTestCase):
    ...
    def setUp(self):
        self.w = [
            ...
            Attrappe("fix-vermerk", 11, "Vermerk setzen", tut="setzt Vermerke", behebt=("dict-versprechen",)),
        ]
        self.f = [Attrappe("fix-importe", 5, "Importe entfernen", tut="entfernt tote Importe")]
    ...
    def test_naechster_treffer_steht_oben(self):
        """Der Fehler des ersten Laufs: „tote-importe" empfahl alles AUSSER
    ...
    def test_fixer_und_pruefer_beide_dabei(self):
        """Finden und Beheben sind zwei Werkzeuge - beide gehören genannt."""
    ...
    def test_fremdes_erbt_nichts(self):
        self.assertEqual(self.wahl.fuer("voellig-anderes", 0), {"sicher": [], "vermutlich": []})

    def test_zeilen_leer_wenn_nichts_gefunden(self):
        """Keine Überschrift ohne Inhalt - das stünde unter jedem Fehlschlag."""
        self.assertEqual(self.wahl.zeilen("voellig-anderes", 0), [])

    def test_zeilen_nennen_die_art(self):
        """``tote-importe`` gibt es als Prüfer UND als Fixer."""
    ...
    def test_slug_im_testnamen_ist_sicher(self):
        """``test_tote_importe`` enthält den Slug - das ist kein Raten.
    ...
    def test_gruener_lauf_bekommt_nichts(self):
        """Eine Werkzeugliste unter einem erfolgreichen Lauf wäre genau das
    ...
    def test_ausgabe_deckelt_bei_drei(self):
        """Vierzig rote Tests heißt nicht vierzig Empfehlungsblöcke."""
