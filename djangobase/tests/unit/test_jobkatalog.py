# -*- coding: utf-8 -*-
"""Der Job-Bestand: gemerkt, taeglich aufgefrischt, auf Knopfdruck neu.
...
class EinNochNieErmittelterBestand(MitTempordner, SimpleTestCase):
    """Gegeben: Es gibt noch keine Katalogdatei."""
...
class EinBestandVonHeute(MitTempordner, SimpleTestCase):
    """Gegeben: Der Bestand wurde gerade eben ermittelt."""
    ...
    def test_wird_beim_lesen_nicht_neu_ermittelt(self):
        """Das ist der Sinn des Merkens: 93 Importe je Seitenaufruf sparen."""
        self.katalog.jobs()
        self.katalog.jobs()
    ...
    def test_liefert_die_gemerkten_jobs(self):
        jobs = self.katalog.jobs()
        self.assertEqual([j["kennung"] for j in jobs], ["index_pdfs", "mail_sync"])

    def test_wird_auf_knopfdruck_trotzdem_neu_ermittelt(self):
        """ "Jetzt aktualisieren" fragt nicht, wie alt der Bestand ist."""
        self.katalog.aktualisieren()
        self.assertEqual(self.erkennung.aufrufe, 1)


class EinBestandVonGestern(MitTempordner, SimpleTestCase):
    """Gegeben: Der Bestand ist gestern ermittelt worden."""
            ...
            json.dumps(
                {
                    ...
                    "jobs": [{"kennung": "alt", "name": "alt", "app": "", "art": "befehl", "hilfe": ""}],
                }
            ),
            encoding="utf-8",
        )
    ...
    def test_wird_beim_lesen_neu_ermittelt(self):
        """Taegliche Auffrischung, ohne dass jemand einen Knopf druecken muss."""
        jobs = self.katalog.jobs()
        self.assertEqual(self.erkennung.aufrufe, 1)
        self.assertEqual([j["kennung"] for j in jobs], ["mail_sync"])
...
class EineBeschaedigteKatalogdatei(MitTempordner, SimpleTestCase):
    """Gegeben: Die Datei ist unlesbar (halb geschrieben, von Hand geaendert)."""
    ...
    def test_stuerzt_nicht_ab_sondern_ermittelt_neu(self):
        """Eine kaputte Merkdatei darf die Seite nicht kosten."""
        jobs = self.katalog.jobs()
        self.assertEqual([j["kennung"] for j in jobs], ["mail_sync"])
