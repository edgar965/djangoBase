# -*- coding: utf-8 -*-
"""Welches Modul wirklich eine Klasse werden sollte — und welches nicht.
...
def _fragen(satz):
    """``{pfad: {Nummern}}`` — welche Fragen je Modul mit Ja beantwortet
    wurden.
    ...
    for b in satz.befunde:
        ...
        raus[b.ort] = {int(z) for z in treffer.group(1).split(",")} if treffer else set()
    return raus


class Frage1Zustand(BasisTest):
    """Gibt es Zustand, den jemand besitzen und zurücksetzen muss?
    ...
    def test_der_name_steht_im_beleg(self):
        """„Frage 1 mit Ja" allein hilft niemandem beim Nachsehen."""
        satz = _lauf({"speicher.py": self.MIT_GLOBAL})
    ...
    def test_eine_konstante_ist_kein_zustand(self):
        """GROSS geschrieben heißt Vorgabe. Beides mitzuzählen macht aus
...
class Frage2Gefaedelt(BasisTest):
    """Werden dieselben Werte durch viele Funktionen gefädelt?
    ...
    def test_ein_gemeinsames_argument_reicht_nicht(self):
        """`(request)` haben alle Ansichten — das ist kein Konstruktor."""
        satz = _lauf(
            {
                "tore.py": (
                    "def eins(x):\n    return 1\n\n\n"
...
class NurWennDerAufruferDieWerteHaelt(BasisTest):
    """Frage 2 gilt nur, wenn es etwas zu halten GIBT.
        ...
        satz = _lauf(
            {
                "leser.py": self.VIER,
                "nutzer.py": (
                    "from leser import get_int, get_str\n\n\n"
                    "def laden():\n"
    ...
    def test_ein_attribut_zaehlt_als_gehalten(self):
        """``cam.ip_address`` ist gehalten — genau der onvif-Fall."""
        satz = _lauf(
            {
                "leser.py": self.VIER,
                "nutzer.py": (
    ...
    def test_ein_literal_unter_gehaltenen_reicht_nicht(self):
        """``get_int('THUMB_FFMPEG_TIMEOUT', _GRENZE)`` — der Schlüssel
    ...
    def test_ohne_jeden_aufrufer_bleibt_der_fund_stehen(self):
        """Was niemand aufruft, lässt sich nicht widerlegen."""
        satz = _lauf({"leser.py": self.VIER})
        self.assertIn(2, _fragen(satz).get("leser.py", set()))


class Frage3ZweiAnliegen(BasisTest):
    """Liegen zwei Anliegen im selben Modul und teilen sich Zustand?
    ...
    def test_wer_beide_anfasst_hat_ein_anliegen(self):
        """Zwei Wörterbücher, die dieselbe Funktion führt, sind EIN
...
class Frage4MehrAlsEines(BasisTest):
    """Braucht es mehr als ein Exemplar?
    ...
    def test_ein_fester_schluessel_ist_eine_tabelle(self):
        """`_state['host']` ist eine Ablage mit festen Feldern, keine
        Instanzverwaltung."""
...
class WoDieFunktionRichtigSteht(BasisTest):
    """DIE GEGENSEITE (Edgar, 26.08.2026)
    ...
    def test_urls_und_admin_ebenso(self):
        satz = _lauf({"urls.py": self.MIT_ZUSTAND, "admin.py": self.MIT_ZUSTAND})
        self.assertEqual(satz.befunde, [])

    def test_reine_funktionen_ohne_zustand_bleiben_still(self):
        """`_in_static_window(t, start, end)` und `_erste_ipv4(...)` haben
        keinen Zustand und gehören in kein Objekt."""
        satz = _lauf(
            {
                "rechnen.py": (
                    "def fenster(t, start, ende):\n"
                    "    return start <= t <= ende\n\n\n"
                    "def erste(liste):\n"
    ...
    def test_der_kopf_nennt_die_ausgenommenen(self):
        """Weglassen wäre schlimmer als melden: Dann wüsste niemand, dass
        das Werkzeug dort bewusst nicht hinsieht."""
        satz = _lauf({"views/a.py": self.MIT_ZUSTAND, "rechnen.py": "def f(x):\n    return x\n"})
        self.assertTrue(any("gar nicht erst gefragt" in z for z in satz.kopf))


class DerPreisStehtDabei(BasisTest):
    """„Der Preis: 20 Aufrufstellen ändern, um ein Modul aus einem
...
class NurGeleseneSammlungen(BasisTest):
    """Eine Sammlung, die niemand veraendert, ist eine Konstante.
    ...
    def test_ein_beschriebenes_woerterbuch_schon(self):
        """Die Gegenprobe: EINE schreibende Zeile macht den Unterschied."""
...
class DasGewichtFolgtDerSchwere(BasisTest):
    ...
    def test_zwei_fragen_sind_ein_fehler(self):
        """Zustand UND zwei Anliegen ist genau der Fall, an dem
...
class AngemeldeteEmpfaenger(BasisTest):
    """Was ein Dekorator beim Rahmenwerk anmeldet, faedelt nichts.
    ...
    def test_ohne_den_dekorator_bleibt_es_ein_befund(self):
        ...
        satz = _lauf(
            {
                "tore.py": self.EMPFAENGER.replace("@receiver(post_save, dispatch_uid='a')\n", "")
                .replace("@receiver(post_save, dispatch_uid='b')\n", "")
                .replace("@receiver(post_save, dispatch_uid='c')\n", "")
            }
        )
        self.assertIn(2, _fragen(satz).get("tore.py", set()))
