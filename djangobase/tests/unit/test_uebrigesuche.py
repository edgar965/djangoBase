# -*- coding: utf-8 -*-
"""Die „Übrigen" einer Endung finden und löschen.
...
from djangobase.umbau.uebrigesuche import GESCHUETZT, UebrigeSuche, geschuetzt
...
class UebrigeFindenUndLoeschen(BasisTest):
    ...
    def test_findet_nur_die_gesuchte_endung(self):
        o = self._bauen({"a.dump": "x", "b.dump": "x", "c.tmp": "x"})
    ...
    def test_bekannte_dateiarten_sind_nie_zu_finden(self):
        """Der wichtigste Fall: `.py` darf über diesen Weg nicht
        löschbar sein, auch wenn jemand die Endung von Hand schickt."""
        o = self._bauen({"a.py": "x = 1\n", "b.html": "<p>x</p>"})
    ...
    def test_die_letzte_pruefung_weist_fremdes_ab(self):
        """``_pruefen`` ist der eigentliche Schutz — er läuft
        unmittelbar vor jedem ``unlink`` und traut dem Aufrufer nicht."""
        o = self._bauen({"a.dump": "x"})
        fremd = Path(tempfile.mkdtemp(prefix="fremd_")) / "fremd.dump"
        fremd.write_text("x", encoding="utf-8")
        suche = UebrigeSuche(o)
        self.assertEqual(suche._pruefen(fremd, ".dump"), "ausserhalb des Projekts")
        self.assertEqual(suche._pruefen(o / "a.dump", ".tmp"), "andere Endung")
        self.assertTrue(fremd.exists())

    def test_die_vorschau_nennt_menge_und_pfade(self):
        o = self._bauen(dict(("d%02d.dump" % i, "x" * 100) for i in range(7)))
...
class MehrereEndungenAufEinmal(BasisTest):
    """Mehrfachauswahl und Sammellöschung (Edgar, 02.09.2026: „mach auch
    Multi Auswahl (check boxen) und batch delete")."""

    DATEIEN = {"a.dump": "x", "b.dump": "xx", "c.tmp": "xxx", "Makefile": "all:\n", "bleibt.py": "x = 1\n"}
    ...
    def test_sammeln_trennt_die_endungen(self):
        ...
        self.assertEqual(
            dict((e, sorted(p.name for p in pfade)) for e, pfade in gefunden.items()),
            {".dump": ["a.dump", "b.dump"], ".tmp": ["c.tmp"], "": ["Makefile"]},
        )

    def test_eine_endung_ohne_treffer_bleibt_leer_statt_zu_fehlen(self):
        """Ein fehlender Schlüssel liesse den Aufrufer raten."""
        o = self._bauen()
        self.assertEqual(UebrigeSuche(o).sammeln([".gibtsnicht"]), {".gibtsnicht": []})
    ...
    def test_loeschen_mehrere_raeumt_alle_gewaehlten(self):
        ...
        self.assertEqual(sorted(p.name for p in o.iterdir()), ["Makefile", "bleibt.py"])

    def test_gegenprobe_nicht_gewaehltes_ueberlebt(self):
        """Ohne diesen Fall belegt der Test oben nur, dass etwas weg ist."""
        o = self._bauen()
    ...
    def test_der_bericht_nennt_jede_endung_einzeln(self):
        ...
        b = UebrigeSuche(o).loeschen_mehrere([".dump", ".tmp"])
        self.assertEqual(
            sorted((e["endung"], e["geloescht"]) for e in b["je_endung"]), [(".dump", 2), (".tmp", 1)]
        )


class DerBaumWirdBeschnitten(BasisTest):
    """Der Geschwindigkeits-Umbau darf das Ergebnis nicht ändern.
    ...
    def test_ein_ausgeschlossener_baum_wird_nicht_betreten(self):
        ...
        for tief in ("logs/a/b/c", "media/x/y", "node_modules/p/q"):
            ...
            (ziel / "tief.dump").write_text("x", encoding="utf-8")
        (ordner / "echt.dump").write_text("x", encoding="utf-8")
        namen = [p.name for p in UebrigeSuche(ordner).finden(".dump")]
        self.assertEqual(namen, ["echt.dump"])

    def test_zu_grosse_dateien_bleiben_draussen(self):
        """Chrome-Cache-Dateien im Projektbaum sind mehrere MB gross —
        ...
        (ordner / "riesig.dump").write_text("x" * (3 * 1024 * 1024), encoding="utf-8")
        (ordner / "klein.dump").write_text("x", encoding="utf-8")
        namen = [p.name for p in UebrigeSuche(ordner).finden(".dump")]
        self.assertEqual(namen, ["klein.dump"])


class GeschuetzteDateiartenSindNichtLoeschbar(BasisTest):
    """Der Schutz aus `GESCHUETZT` — und warum es ihn gibt.
    ...
    def test_der_letzte_blick_weist_sie_ab(self):
        """``_pruefen`` ist der Schutz unmittelbar vor dem ``unlink``.
        ...
        self.assertEqual(UebrigeSuche(o)._pruefen(o / "audit.log", ".log"), "geschützte Dateiart")

    def test_loeschen_laesst_sie_stehen(self):
        """Die Gegenprobe am ganzen Weg, nicht nur an einer Methode."""
    ...
    def test_und_das_ungeschuetzte_verschwindet_doch(self):
        """Ohne diesen Fall belegen die vier oben nur, dass NICHTS
    ...
    def test_die_liste_ist_nicht_leer(self):
        """Sabotageschutz: Eine leere `GESCHUETZT` bestuende jeden Fall
