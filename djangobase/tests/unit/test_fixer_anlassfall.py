# -*- coding: utf-8 -*-
"""Auch die Fixer müssen ihren eigenen Fall noch sehen.
...
class JederFixerHatSeinenFall(BasisTest):
    def test_keiner_ohne_anlassfall(self):
        ...
        self.assertEqual(ohne, [], "Fixer ohne Anlassfall: %s" % ohne)

    def test_jeder_fall_nennt_seinen_grund(self):
        """``warum`` steht im Bericht — ohne ihn ist der Fall eine
        Behauptung ohne Begründung."""
        for klasse in FIXER:
            fall = getattr(klasse, "anlassfall", None)
            self.assertTrue(getattr(fall, "warum", ""), "%s: Anlassfall ohne warum" % klasse.slug)

    def test_jeder_fall_bringt_dateien_mit(self):
        for klasse in FIXER:
            fall = getattr(klasse, "anlassfall", None)
            self.assertTrue(getattr(fall, "dateien", None), "%s: Anlassfall ohne Dateien" % klasse.slug)


class DerSammellaufSiehtSieAuch(BasisTest):
    def test_der_check_geht_ueber_werkzeuge_UND_fixer(self):
        """Die Schleife stand nur auf ``WERKZEUGE``."""
        import inspect
    ...
    def test_ein_fixer_meldet_ueber_vorschau_nicht_ueber_laufen(self):
        """Ein Fixer hat kein ``laufen()`` — dieselbe Bauart-Falle, die
    ...
    def test_der_probelauf_schreibt_nicht(self):
        """Ein Selbsttest, der Dateien ändert, ist kein Selbsttest mehr.
        ...
        baum = ast.parse(textwrap.dedent(inspect.getsource(Probelauf._fixerlauf)))
        gerufen = {
            k.func.attr
...
class KeineKennungZweimal(BasisTest):
    """`tote-importe` war bis zum 25.08.2026 doppelt vergeben.
    ...
    def test_jede_kennung_gehoert_einem(self):
        ...
        self.assertEqual(doppelt, [], "doppelt vergeben: %s" % doppelt)
    ...
    def test_das_pruefwerkzeug_behaelt_seine_kennung(self):
        """Auf `tote-importe` verweisen Texte und Prüfungen — die Kennung
