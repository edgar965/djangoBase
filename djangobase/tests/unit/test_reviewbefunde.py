# -*- coding: utf-8 -*-
"""`review-befunde`: Was aus 1,8 MB Mitschrift herauskommt — und was nicht.
...
ATTRAPPE = _mitschrift("Bereich A", "attrappe", "ok", sekunden=0)


class _Werkzeug(ReviewBefunde):
    """Ein `ReviewBefunde`, das in einem Wegwerf-Verzeichnis sucht."""
    ...
    def projektdateien(self, endung=".py", ausser=None):
        return sorted(p for p in self._ordner.rglob("*" + endung) if "review" not in p.parts)
...
class BefundeFindenTest(SimpleTestCase):
    def test_nummerierte_ueberschriften(self):
        ...
        self.assertIn("Ungepruefter", erg.befunde[0].ort + erg.befunde[1].ort)

    def test_dateikoepfe_ohne_nummern(self):
        """Die zweite Gliederung — nur sie zu koennen hiesse, ein Drittel
    ...
    def test_bereich_laesst_sich_eingrenzen(self):
        erg = _lauf(
            {"review_a_probe.md": NUMMERIERT, "review_b_zweiter.md": DATEIKOEPFE},
            ["dienst.py"],
            bereich="Zweiter",
        )
...
class ErledigtesTest(SimpleTestCase):
    """Ein Befund zu einer Datei, die es nicht mehr gibt, ist ERLEDIGT."""

    def test_vorhandene_datei_wird_zur_warnung(self):
        erg = _lauf({"review_a_probe.md": NUMMERIERT}, ["dienst.py"])
        self.assertTrue(all(b.gewicht == Befund.WARNUNG for b in erg.befunde), [b.warum for b in erg.befunde])

    def test_verschwundene_datei_wird_zum_hinweis(self):
        """DIE GEGENPROBE: Ohne die Datei darf es keine Warnung sein."""
        erg = _lauf({"review_a_probe.md": NUMMERIERT}, [])
        self.assertTrue(all(b.gewicht == Befund.HINWEIS for b in erg.befunde), [b.warum for b in erg.befunde])
        self.assertIn("nicht mehr", erg.befunde[0].warum)

    def test_gleicher_dateiname_andere_datei_zaehlt_nicht(self):
        """DER FEHLALARM DES WERKZEUGS SELBST (28.08.2026).
...
""",
        ...
        self.assertEqual(
            erg.befunde[0].gewicht,
            Befund.HINWEIS,
            "`dienst.py` gibt es, `gibtesnichtmehr` nicht — das ist Namensgleichheit, keine Fundstelle",
        )
    ...
    def test_die_zahl_steht_in_der_kopfzeile(self):
        ...
        text = " ".join(erg.kopf)
        self.assertIn(
            "erledigt", text, "Ein Befund zu `alt_und_weg.py` muss als erledigt gezaehlt werden: %s" % text
        )


class AttrappenTest(SimpleTestCase):
    """Mitschriften aus Testlaeufen sind keine Befunde."""
    ...
    def test_eine_echte_neben_einer_attrappe(self):
        erg = _lauf({"review_a_probe.md": NUMMERIERT, "review_c_a.md": ATTRAPPE}, ["dienst.py"])
        self.assertEqual(len(erg.befunde), 2)
        self.assertIn("1 Attrappen", " ".join(erg.kopf))

    def test_die_LANGE_frage_macht_keine_echte_daraus(self):
        """DER FEHLER VOM ERSTEN WURF, festgenagelt.
        ...
        """
        lang = ATTRAPPE.replace("# Codebereich", "# Codebereich\n\n" + "x = 1\n" * 400)
        self.assertGreater(len(lang), 2000)
        erg = _lauf({"review_c_a.md": lang}, ["dienst.py"])
        self.assertEqual(erg.befunde, [], "an der Antwort messen, nicht an der Datei")

    def test_kurze_antwort_ohne_attrappen_namen(self):
        """Auch ein echtes Modell kann „ok" antworten — das ist kein Befund."""
        kurz = _mitschrift("Kurz", "grossmodell", "ok")
        erg = _lauf({"review_d_kurz.md": kurz}, ["dienst.py"])
        self.assertEqual(erg.befunde, [])


class KopfdatenTest(SimpleTestCase):
    """Bereich, Modell und Dauer stehen in der ersten Zeile."""
    ...
    def test_kopfzeile_wird_gelesen(self):
        with tempfile.TemporaryDirectory() as ordner:
            ...
            m = Mitschrift(pfad)
        self.assertEqual(m.bereich, "Probebereich")
        self.assertEqual(m.modell, "grossmodell")
        self.assertEqual(m.sekunden, 12)

    def test_ohne_kopfzeile_gilt_der_dateiname(self):
        with tempfile.TemporaryDirectory() as ordner:
            pfad = Path(ordner) / "review_abc_mein_bereich.md"
            pfad.write_text("nur Text", encoding="utf-8")
            m = Mitschrift(pfad)
        self.assertEqual(m.bereich, "mein_bereich")

    def test_dateinamen_werden_erkannt(self):
        befund = Reviewbefund(None, "T", "siehe `core/api/x.py` und `y.js`")
