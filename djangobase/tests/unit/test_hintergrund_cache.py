...
class ZuAlterWertTest(SimpleTestCase):
    def test_nur_einer_rechnet_wenn_der_wert_zu_alt_ist(self):
        ...
        ergebnisse = []
        faeden = [threading.Thread(target=lambda: ergebnisse.append(c.holen())) for _ in range(6)]
        for f in faeden:
            f.start()
        for f in faeden:
            f.join(5)

        self.assertEqual(zaehler["n"], 2, "es liefen %d Berechnungen statt einer" % (zaehler["n"] - 1))
        self.assertEqual(len(ergebnisse), 6)
        self.assertTrue(all(e is not None for e in ergebnisse), "ein Abruf kam ohne Wert zurück")
    ...
    def test_fehler_gibt_die_sperre_wieder_frei(self):
        ...
        with self.assertRaises(RuntimeError):
            c.holen()
        self.assertFalse(c.zustand()["laeuft"], 'nach einem Fehler steht der Cache auf „baut gerade"')
