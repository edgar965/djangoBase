# -*- coding: utf-8 -*-
"""Groesse wird an CODE gemessen, nicht an Zeilen.
...
def _lauf(klasse, dateien, **argumente):
    with tempfile.TemporaryDirectory() as ordner:
        ...
        return werkzeug.laufen() if klasse is _Groesse else werkzeug.pruefen(**argumente)


class CodezeilenTest(SimpleTestCase):
    """Die Zaehlung selbst - Kommentare aus ``tokenize``, Docstrings aus AST."""

    def _zaehlen(self, text):
        """``(Code-Zeilen, Zeilen gesamt, Quelldatei)``.
    ...
    def test_kommentar_HINTER_code_laesst_die_zeile_code(self):
        """Sonst fällt jede gut kommentierte Zeile aus der Zählung."""
        code, _g, _d = self._zaehlen("a = 1  # erklaert\nb = 2  # erklaert\n")
        self.assertEqual(code, 2)

    def test_ein_string_in_einer_zuweisung_ist_code(self):
        """Nur ein ALLEIN stehendes Literal ist Dokumentation.
...
class KlassenZusammenTest(SimpleTestCase):
    ...
    def test_kleine_datei_ist_kein_verstoss(self):
        """Unter 300 Code-Zeilen macht Aufteilen die Uebersicht schlechter."""
    ...
    def test_models_gehoeren_zusammen(self):
        """Django FINDET seine Modelle in ``models.py``."""
        gross = self.ZWEI_GROSSE + "".join("x%d = %d\n" % (i, i) for i in range(320))
        self.assertEqual(self._warnungen({"models.py": gross}), [])

    def test_grosse_datei_bleibt_ein_verstoss(self):
        """DIE GEGENPROBE: zwei eigenstaendige Klassen, 300+ Code-Zeilen."""
        gross = self.ZWEI_GROSSE + "".join("x%d = %d\n" % (i, i) for i in range(320))
    ...
    def test_der_grund_steht_am_hinweis(self):
        ...
        self.assertTrue(
            any("Aufteilen schadet" in b.warum for b in satz.befunde), [b.warum for b in satz.befunde]
        )
