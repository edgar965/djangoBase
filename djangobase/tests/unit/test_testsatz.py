# -*- coding: utf-8 -*-
"""Testsatz — liest eine Prüf-Kennung als deutschen Satz.
...
class DerSatzIstLesbarOhneCode(BasisTest):
    """Die Zusage, um die es geht: Ein Nicht-Programmierer versteht ihn."""

    def test_unterstriche_werden_zu_leerzeichen(self):
        self.assertEqual(_satz("m.PersonTest.test_person_bleibt_erhalten"), "Person: Person bleibt erhalten")

    def test_der_vorsatz_test_faellt_weg(self):
        """``test_`` sagt nur, DASS es eine Prüfung ist — das sieht man."""
        self.assertNotIn("test_", _satz("m.AbcTest.test_etwas_geht_gut"))

    def test_der_nachsatz_test_faellt_auch_weg(self):
        self.assertTrue(_satz("m.KameraTests.test_bild_kommt_an").startswith("Kamera:"))

    def test_camelcase_wird_getrennt(self):
        self.assertTrue(_satz("m.AliasVerbundTest.test_kette_wird_aufgeloest").startswith("Alias Verbund:"))

    def test_fuellwoerter_bleiben_klein(self):
        """``KameraUndPerson`` ist CamelCase, kein englischer Titel."""
        self.assertTrue(_satz("m.KameraUndPerson.test_nur_bekannte").startswith("Kamera und Person:"))

    def test_das_erste_wort_bleibt_gross(self):
        """Auch wenn es ein Füllwort ist: Ein Satz fängt groß an."""
        self.assertTrue(_satz("m.DerZaehlerLaeuftVoll.test_es_faellt_auf").startswith("Der "))


class DerSatzNenntGegenstandUndErgebnis(BasisTest):
    """Zwei Teile, wie bei Gherkin: worum es geht, und was stimmen muss."""

    KENNUNG = "m.AufbewahrungTraegt.test_person_bleibt_erhalten"

    def test_der_gegenstand_kommt_aus_der_klasse(self):
        self.assertEqual(Testsatz(self.KENNUNG).gegenstand(), "Aufbewahrung traegt")

    def test_das_ergebnis_kommt_aus_der_methode(self):
        self.assertEqual(Testsatz(self.KENNUNG).ergebnis(), "Person bleibt erhalten")

    def test_beides_steht_mit_doppelpunkt_zusammen(self):
        self.assertEqual(_satz(self.KENNUNG), "Aufbewahrung traegt: Person bleibt erhalten")


class EineUnvollstaendigeKennungBRICHTNICHT(BasisTest):
    """Die Kennung kommt aus der Test-Erkennung — sie ist nicht immer
    ...
    def test_etwas_voellig_fremdes_bleibt_stehen(self):
        """Lieber unverändert anzeigen als etwas erfinden."""
        self.assertEqual(_satz("xyz"), "Xyz")


class DerTextWIRDNICHTUMGESCHRIEBEN(BasisTest):
    """Was im Bezeichner steht, steht auch im Satz.
    ...
    def test_ein_wort_bleibt_wie_es_dasteht(self):
        self.assertIn("behaelt", _satz("m.AbcTest.test_falte_behaelt_die_reihenfolge"))
    ...
    def test_es_gibt_keine_wortliste_mehr(self):
        """Gegenprobe: Sonst kaeme sie beim naechsten Mal zurueck."""
        import djangobase.testsatz as ts

        self.assertFalse(
            hasattr(ts, "UMLAUTE"), "Die Wortliste ist zurueck — sie war Pflegeaufwand ohne Gegenwert."
        )


class DieTestSeiteZeigtSaetze(BasisTest):
    """Die Zusage gilt erst, wenn sie auf der Seite ankommt."""

    def test_die_ansicht_benutzt_den_satzleser(self):
        from djangobase.views.tests import _kurz

        self.assertEqual(
            _kurz("a.b.PersonTest.test_person_bleibt_erhalten"), "Person: Person bleibt erhalten"
        )
