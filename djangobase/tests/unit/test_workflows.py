# -*- coding: utf-8 -*-
"""Workflows — werden sie gefunden, richtig sortiert und gezeichnet?
...
ABZUG = {
    ...
    "    def fuenf(self):\n"
    "        return 5\n",
    "management/commands/machwas.py": "class Command:\n"
    "    def handle(self, *a, **k):\n"
    "        from ..kette import Erst\n"
    "        Erst().eins()\n",
    "live/schleife.py": "class Dauerlauf:\n    def run(self):\n        return 1\n",
}
...
class EinstiegeWerdenGefundenStattAufgeschrieben(BasisTest):
    """Eine Liste von Hand waere beim naechsten neuen Endpunkt falsch."""
    ...
    def test_eine_dauerschleife_wird_gefunden(self):
        """Der Teil, den man beim Lesen der Routen gerade NICHT findet."""
...
class DieListeIstNachKomplexitaetSortiert(BasisTest):
    """Nicht nach Name und nicht nach Adresse: nach der Zahl der
...
class EinHandgriffIstKeinWorkflow(BasisTest):
    """Eine Seite, die nur eine Vorlage ausliefert, erzaehlt nichts.
...
class ZweiBefehleMitGleichemHandleBleibenZweiWorkflows(BasisTest):
    """Alle Management-Befehle heissen ``handle``.
...
class JederWegLaesstSichZeichnen(BasisTest):
    """Ein Weg ohne Bild waere eine Liste — davon gibt es genug."""
    ...
    def test_das_bild_ist_breiter_als_hoch(self):
        """Ein Streifen von 966 mal 4214 Punkten war der erste Wurf —
...
class DieReiterVerteilenDieWegeVollstaendig(BasisTest):
    """Kein Weg darf beim Gliedern verlorengehen — sonst zeigt die Seite
