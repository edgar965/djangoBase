# -*- coding: utf-8 -*-
"""`Stilnamen` — erkennt es den Fall, und schweigt es beim Rest?
...
class RegelnLesenTest(BasisTest):
    ...
    def test_einfacher_name_zaehlt(self):
        self.assertEqual([("", "a", "color: red")], _finden("<style>.a { color: red }</style>"))

    def test_vervielfachter_name_ist_derselbe(self):
        """`.x.x.x` ist die Schreibweise der Umsteller für Spezifität."""
        self.assertEqual([("", "x", "color: red")], _finden("<style>.x.x.x { color: red }</style>"))

    def test_gruppen_bleiben_draussen(self):
        """DER Fehlalarm, der beim ersten Anlauf Schaden angerichtet hat."""
        text = "<style>\n.a, .b { background: #111 }\n.a, .c { padding: 4px }\n</style>"
        self.assertEqual([], _finden(text))

    def test_ein_name_faellt_auch_dann_raus_wenn_er_daneben_allein_steht(self):
        """Steht `.a` einmal in einer Gruppe, ist jede Aussage über `.a`
...
class ErzeugtErkennenTest(BasisTest):
    ...
    def test_erzeugte_namen(self):
        for name, rumpf in (
            ("hb-margin-top-20px", "margin-top: 20px; color: #fff"),
    ...
    def test_handgeschriebene_namen(self):
        for name, rumpf in (
            ("panel-tab", "flex: 1; text-align: center"),
...
class AtBloeckeTest(BasisTest):
    """Eine Ueberschreibung fuer Druck oder schmale Fenster ist kein Streit.
    ...
    def test_die_beiden_ebenen_werden_getrennt_gelesen(self):
        ...
        self.assertEqual(["", "@media print"], sorted(b for b, _n, _r in gefunden))
