# -*- coding: utf-8 -*-
"""Vorlagen-Kontext — was die Ansicht liefert und was die Vorlage liest.
...
class Vorlagenprobe(Vorlagensicht):
    """`Vorlagensicht` für eine Vorlage aus dem Speicher.
...
class WasDieVorlageUnbedingtLiest(BasisTest):
    ...
    def test_ein_wirklich_fehlender_name_wird_gemeldet(self):
        """OHNE DIESEN TEST IST ALLES ANDERE WERTLOS.
    ...
    def test_auch_der_else_zweig_zaehlt_als_bedingt(self):
        """Django baut aus `{% if %}`/`{% elif %}`/`{% else %}` EINEN
        Knoten."""
        probe = Vorlagenprobe("{% if d.ok %}x{% elif d %}{{ d.error }}{% else %}{{ leer }}{% endif %}")
        self.assertNotIn("d", probe.fest)
        self.assertNotIn("leer", probe.fest)

    def test_verschachtelt_bleibt_es_bedingt(self):
        probe = Vorlagenprobe("{% if a %}{% for z in liste %}{{ tief }}{% endfor %}{% endif %}")
        self.assertNotIn("tief", probe.fest)

    # -------------------------------------------------------- `|default:`
    def test_ein_ersatzwert_macht_den_namen_wahlfrei(self):
        """`{{ main_probe.width|default:'?' }}` sagt daneben, was bei
    ...
    def test_ein_anderer_filter_macht_ihn_nicht_wahlfrei(self):
        """`|upper` liefert keinen Ersatz — fehlt der Name, steht dort
        nichts."""
...
class DasWerkzeugAmEchtenProjekt(BasisTest):
    """Der ganze Weg: `render()`-Aufruf im Quelltext gegen echte Vorlage."""
    ...
    def test_ein_uebergebener_name_den_niemand_liest_ist_TOT(self):
        ...
        self.assertTrue(any("TOT: ungenutzt" in b.was for b in satz.befunde), [b.was for b in satz.befunde])

    def test_ein_gelesener_name_den_niemand_liefert_ist_FEHLEND(self):
        """Der Name muss einer sein, den KEIN Kontextprozessor liefert.
        ...
        self.assertTrue(
            any("FEHLEND: pruefname_ohne_lieferant" in b.was for b in satz.befunde),
            [b.was for b in satz.befunde],
        )

    def test_ein_name_hinter_einer_bedingung_ist_kein_befund(self):
        """Der Fall, der am 23.08.2026 siebenmal falsch gemeldet wurde."""
        satz = self._lauf(
            "def x(request):\n    return render(request, 'a.html', {'is_edit': False})\n",
            {"a.html": "{% if is_edit %}{{ camera.name }}{% endif %}"},
        )
        self.assertFalse([b for b in satz.befunde if "camera" in b.was], [b.was for b in satz.befunde])
    ...
    def test_einfach_gequotetes_extends_setzt_die_kette_fort(self):
        satz = self._lauf(
            ...
            {"a.html": "{% extends 'basis.html' %}", "basis.html": "<p>{{ wert }}</p>"},
        )
        self.assertFalse([b for b in satz.befunde if "TOT: wert" in b.was], [b.was for b in satz.befunde])

    def test_einfach_gequotetes_include_setzt_die_kette_fort(self):
        satz = self._lauf(
            "def x(request):\n    return render(request, 'a.html', {'wert': 1})\n",
            {"a.html": "{% include 'teil.html' %}", "teil.html": "<p>{{ wert }}</p>"},
        )
        self.assertFalse([b for b in satz.befunde if "TOT: wert" in b.was], [b.was for b in satz.befunde])
    ...
    def test_einfach_gequotet_eingebunden_ist_nicht_verwaist(self):
        satz = self._lauf(
            "def x(request):\n    return render(request, 'a.html', {})\n",
            {"a.html": "{% include 'teil.html' %}", "teil.html": "<p>fest</p>"},
        )
        self.assertFalse([b for b in satz.befunde if "teil.html" in b.ort], [b.ort for b in satz.befunde])

    def test_gegenprobe_der_tote_name_bleibt_auch_einfach_gequotet_tot(self):
        """Das Werkzeug ist nicht einfach still geworden.
        ...
        satz = self._lauf(
            "def x(request):\n    return render(request, 'a.html', {'ungenutzt': 1})\n",
            {"a.html": "{% extends 'basis.html' %}", "basis.html": "<p>nichts</p>"},
        )
        self.assertTrue(any("TOT: ungenutzt" in b.was for b in satz.befunde), [b.was for b in satz.befunde])
