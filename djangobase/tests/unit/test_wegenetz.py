# -*- coding: utf-8 -*-
"""Wegenetz — loest es Aufrufe auf, ohne zu raten?
...
def _abzug(dateien):
    """Ein kleines Projekt auf der Platte, damit der AST etwas zu lesen hat."""
...
class EinEindeutigerMethodennameWirdVerbunden(BasisTest):
    """Traegt im ganzen Projekt genau eine Klasse den Namen, ist klar,
...
class EinMehRDEUTIGERNameWirdNichtVerbunden(BasisTest):
    """Die Kernzusage: Im Zweifel lieber eine Luecke als eine erfundene
    ...
    def test_die_luecke_wird_gemeldet_statt_verschwiegen(self):
        """Sonst sieht ein unvollstaendiges Bild aus wie ein vollstaendiges."""
        weg = _weg(self.QUELLE, "Dienst", "arbeiten")
        self.assertIn("start", weg.offen)


class EinFeldMitBekanntemTypLoestDenNamenDochAuf(BasisTest):
    """``self.motor = Motor()`` steht im Code — damit ist ``self.motor.
...
class EineOertlicheZuweisungZaehltGenauso(BasisTest):
    """``p = Pumpe()`` im selben Rumpf ist derselbe Beleg wie ein Feld."""
...
class PruefcodeGehoertNichtInEinenWeg(BasisTest):
    """Ein Doppelgaenger aus dem Pruefordner traegt oft einen eindeutigen
    ...
    QUELLE = {
        ...
        "tests/test_b.py": "class FalscherCv2:\n    def weichzeichnen(self):\n        return 1\n",
    }
...
class DieTiefeBegrenztDenWeg(BasisTest):
    """Ohne Grenze laufen alle Wege bei den Hilfsfunktionen zusammen und
...
class JederSchrittZeigtAufEineStelleImQuelltext(BasisTest):
    """Ohne Datei und Zeile waere der Kasten eine Behauptung."""
...
class EinKlassenkastenIstDerKonstruktor(BasisTest):
    """Was ``Klasse()`` auslöst — nicht, was die Klasse alles kann.
...
class DieAusschlusslisteDesProjektsGilt(BasisTest):
    """Was das Projekt ausschliesst, darf nicht im Verzeichnis landen.
    ...
    def test_mit_liste_bleibt_die_kopie_draussen(self):
        ...
        self.assertIn("Eigen", verzeichnis.klassen, "der eigene Code muss bleiben")
