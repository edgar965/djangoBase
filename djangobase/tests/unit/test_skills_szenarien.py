# -*- coding: utf-8 -*-
"""Szenarien — findet es stumme Prüfungen, und lässt es gute in Ruhe?
...
class EineStummePruefungIstEinFehler(BasisTest):
    def test_ohne_zusicherung_wird_gemeldet(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "    def test_person_bleibt_erhalten(self):\n"
                "        x = 1\n"
                "        print(x)\n"
            }
        )
    ...
    def test_assertraises_zaehlt(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "    def test_person_wird_nicht_geloescht(self):\n"
    ...
    def test_eine_eigene_hilfsmethode_zaehlt(self):
        """`self._pruefe_dass(...)` sichert vielleicht zu — lieber ein
...
class EinNameSollDasVerhaltenNennen(BasisTest):
    def test_zu_kurzer_name_wird_gemeldet(self):
        satz = _lauf({"tests/test_a.py": KOPF + "    def test_grid(self):\n        self.assertTrue(True)\n"})
        self.assertEqual(_gewichte(satz).get(Befund.HINWEIS), 1)

    def test_nichtssagender_name_wird_auch_gemeldet(self):
        """``test_basic`` ist lang genug und sagt trotzdem nichts."""
        satz = _lauf({"tests/test_a.py": KOPF + "    def test_basic(self):\n        self.assertTrue(True)\n"})
        self.assertEqual(_gewichte(satz).get(Befund.HINWEIS), 1)

    def test_ein_satz_als_name_ist_in_ordnung(self):
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "    def test_person_bleibt_nach_merge_erhalten(self):\n"
                "        self.assertTrue(True)\n"
            }
        )
        self.assertEqual(satz.befunde, [])
    ...
    def test_ein_stummer_test_mit_gutem_namen_wiegt_schwerer(self):
        """Der Name ist ein Hinweis, die fehlende Zusicherung ein Fehler."""
        satz = _lauf(
            {
                "tests/test_a.py": KOPF + "    def test_person_bleibt_nach_merge_erhalten(self):\n"
                "        pass\n"
...
class ProduktivcodeIstKeinePruefung(BasisTest):
    """Der Fehlalarm aus dem ersten Wurf, als Prüfung festgehalten."""
    ...
    def test_eine_ansicht_namens_connection_test_zaehlt_nicht(self):
        ...
        self.assertEqual(
            satz.befunde,
            [],
            "Eine Ansicht, deren Methoden test_ heißen, ist keine Prüfung — sie heißt nur so.",
        )
    ...
    def test_mit_einer_test_basis_zaehlt_sie_sehr_wohl(self):
        """Die echte Gegenprobe: Sonst prüft der Test darüber nur, dass
        das Werkzeug gar nichts findet."""
        satz = _lauf({"tests/echt.py": "class Echte(TestCase):\n    def test_x(self):\n        pass\n"})
        self.assertEqual(_gewichte(satz).get(Befund.FEHLER), 1)
...
class DerKopfSagtDenAnteil(BasisTest):
    ...
    def test_die_zahl_der_stummen_steht_im_kopf(self):
        satz = _lauf({"tests/test_a.py": KOPF + "    def test_person_bleibt_erhalten(self):\n        pass\n"})
        self.assertTrue(any("ohne jede Zusicherung" in z for z in satz.kopf), satz.kopf)


class JedesWerkzeugHatEinBeispielOderEinenGrund(BasisTest):
    """Die erste der drei BDD-Zusicherungen — als Prüfung, nicht als
    Vorsatz.
    ...
    def test_keines_schweigt(self):
        ...
        stumm = [
            ...
            if getattr(w, "anlassfall", None) is None and not getattr(w, "ohne_anlassfall_weil", "")
        ]
        self.assertEqual(
            stumm, [], "Diese Werkzeuge haben weder ein Beispiel noch einen Grund, warum nicht: %s" % stumm
        )
        ...
        for w in list(werkzeuge()) + list(fixer()):
            ...
            if grund:
                with self.subTest(werkzeug=w.slug):
                    self.assertGreater(len(grund), 30, "Ein Grund in drei Worten ist keiner: %r" % grund)
