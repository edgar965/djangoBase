# -*- coding: utf-8 -*-
"""Tests der Frontend-Werkzeuge in Skills2 (jsbefunde, jsfaenger, jswaisen, …).
...
class FrontendBasis(BasisTest):
    ...
    def orte(self, ergebnis):
        return " ".join(z.get("ort", "") + " " + str(z.get("art", "")) for z in ergebnis.zeilen)
...
class JsWaisenTest(FrontendBasis):
    ...
    def test_erwaehnung_im_kommentar_ist_kein_import(self):
        """Der Fall, der beim Bau Fehlalarm ausloeste: Ein Kommentar, der einen
    ...
    def test_laeufer_ist_keine_waise(self):
        """Node-Skripte und Bau-Konfigurationen MUESSEN unerreichbar sein.
    ...
    def test_echte_waise_bleibt_trotz_laeufer_erkennung(self):
        """Gegenprobe: Ein Browser-Modul ohne Node-Merkmale bleibt ein Befund."""
        ergebnis = self.laufen(
            "jswaisen",
            {
                "templates/seite.html": VORLAGE,
    ...
    ZWEI_INDEX = {
        "templates/seite.html": "{% load static %}\n"
    ...
    def test_gleicher_dateiname_in_anderem_ordner_zaehlt_nicht_als_ladung(self):
        """Der Fall aus dem Projekt assistant (31.08.2026).
    ...
    def test_die_wirklich_geladene_bleibt_ohne_befund(self):
        """Gegenrichtung: Die Verschaerfung darf nichts Lebendes melden."""
    ...
    def test_blanker_dateiname_laedt_weiter_alle_gleichnamigen(self):
        """Ohne Ordner in der Angabe bleibt es beim alten Verhalten.
...
class FrontendadressenTest(FrontendBasis):
    """Der Fall: acht Aufrufe auf eine Adresse, die es nicht gibt.
    ...
    def laufen_mit_urls(self, dateien):
        """Wie `laufen`, aber mit der URL-Konfiguration dieses Testmoduls."""
    ...
    def test_konstante_ist_kein_aufruf(self):
        """`static ENDPUNKT = '/api/x/'` ist ein Anfang, keine Adresse."""
        ergebnis = self.laufen_mit_urls(
            {
    ...
    def test_verkettung_ueber_zeilen_ist_kein_befund(self):
        """Der Zusatz stand 42 Leerzeichen eingerueckt in der nächsten Zeile —
    ...
    def test_platzhalter_wird_probiert(self):
        """Ohne mehrere Kandidaten meldet jede `<uuid:...>`-Route Fehlalarm."""
        ergebnis = self.laufen_mit_urls(
            {
...
class VorlagenblockTest(FrontendBasis):
    """Der Fall: `{% block extra_styles %}`, den `base.html` nicht kennt.
    ...
    def test_block_der_grosseltern_zaehlt(self):
        """`extends`-Ketten werden verfolgt, nicht nur eine Ebene."""
        ergebnis = self.laufen(
            "vorlagenblock",
            {
                "templates/base.html": VorlagenblockTest.ELTERN,
...
class JsRegistrierungTest(FrontendBasis):
    def test_findet_gerufen_ohne_anmeldung(self):
        ...
        self.assertIn(
            "startWizard", self.orte(ergebnis) + " ".join(z.get("name", "") for z in ergebnis.zeilen)
        )
...
class JsFaengerTest(FrontendBasis):
    ...
    def test_einzeiliges_try_catch_zaehlt(self):
        """`try { await x(); } catch (e) { … }` in EINER Zeile.
        ...
        ergebnis = self.laufen(
            "jsfaenger",
            {
                "static/a.js": "export async function laden() {\n"
    ...
    def test_aufrufer_deckt_den_helfer(self):
        """Der Fall, der 18 von 20 Zeilen zu Fehlalarmen machte."""
        ergebnis = self.laufen(
            "jsfaenger",
            {
                "static/a.js": "async function _helfer(u) {\n"
    ...
    def test_ungefangener_aufrufer_bleibt_ein_befund(self):
        """Gegenprobe zum vorigen Test: Faengt der Aufrufer NICHT, bleibt es
    ...
    def test_gleichnamige_methode_woanders_deckt_nicht(self):
        """`load()` heisst auch die Methode von Three.js' GLTFLoader.
    ...
    def test_name_in_einer_meldung_ist_kein_aufruf(self):
        """`throw new Error('laden() must be called first')` zaehlte als
        ungefangener Aufruf — der Name steht dort in einer Zeichenkette."""
        ergebnis = self.laufen(
            "jsfaenger",
            {
                "static/a.js": "let _wert = null;\n"
    ...
    def test_sammelstelle_zaehlt_als_sichtweg(self):
        """`fn.X = X` trägt den Namen ohne Import weiter.
    ...
    def test_allerweltsname_geht_nicht_ueber_die_sammelstelle(self):
        """Gegenprobe zum vorigen Test: Bei `load()` zaehlt nur der Import-Weg.
...
class JsBefundeTest(FrontendBasis):
    ...
    def test_ok_pruefung_hinter_mehrzeiligem_optionsobjekt_zaehlt(self):
        """Fehlalarm-Gegenprobe: Das Fenster muss ab dem ENDE der
    ...
    def test_django_vergleich_in_vorlage_ist_kein_javascript_befund(self):
        """Der teuerste Fehlalarm des Werkzeugs: `<script src=…></script>` in
        EINER Zeile liess die ganze Vorlage als JavaScript gelten - 107
        Django-Vergleiche erschienen als Befund."""
        ergebnis = self.laufen(
            "jsbefunde",
            {
                "templates/seite.html": '<script src="/static/x.js"></script>\n'
    ...
    def test_marker_im_dateikopf_nimmt_die_ganze_datei_aus(self):
        """Eine Debugseite, deren Konsolenausgabe ihr Ergebnis IST.
...
class JsFunktionenTest(FrontendBasis):
    ...
    def test_grenze_aus_den_einstellungen(self):
        ...
        with override_settings(BASE_DIR=str(ordner), DJANGOBASE={"skills2_funktionsgrenze": 20}):
...
class JsSyntaxTest(FrontendBasis):
    def test_findet_kaputten_import(self):
        """Der Fall, der das Werkzeug ausgeloest hat: eine Import-Zeile MITTEN
    ...
    def test_ohne_node_kein_falsches_gruen(self):
        """Fehlt Node, darf das Werkzeug NICHT sagen, alles sei in Ordnung."""
