# -*- coding: utf-8 -*-
"""Pruefcode — findet er JEDE Pruefung, und nur Pruefungen?
...
def _pruefcode(*quellen):
    return Pruefcode().lesen([(Path("x%d.py" % i), _baum(q)) for i, q in enumerate(quellen)])


class EineKlasseMitTestBasisIstEinePruefung(BasisTest):
    """Der einfache Fall — und die Schreibweise darf egal sein."""
    ...
    def test_der_punkt_davor_aendert_nichts(self):
        """``unittest.TestCase`` ist dasselbe wie ``TestCase``."""
...
class EineEigeneBasisWirdUEBERMEHREREStufenErkannt(BasisTest):
    """Der Fall, an dem eine feste Namensliste scheitert.
    ...
    def test_die_klasse_am_ende_der_kette_zaehlt(self):
        ...
        letzte = [k for k in _baum(self.QUELLE).body if isinstance(k, ast.ClassDef)][-1]
...
class EineKlasseOhneBasisIstKeinePruefung(BasisTest):
    """Die Gegenrichtung — sonst waere die Regel wertlos.
...
class DerDateinameEntscheidetNICHTS(BasisTest):
    """Die eigentliche Korrektur vom 27.08.2026.
...
class NurWasDerTestlaeuferAusfuehrt(BasisTest):
    """Eine Hilfsmethode ist keine Prüfung."""
...
class JedePruefungErbtVonEinerTestBasis(BasisTest):
    """DER WÄCHTER — heute grün, und genau darum hier.
    ...
    ORTE = ("tests",)
    #: `_wegwerf`/`_anlassfall` (18.09.2026): Dort schreiben andere Prüffälle
    ...
    def test_keine_pruefklasse_bleibt_ohne_basis(self):
        ...
        self.assertTrue(
            dateien, "Keine Prüfdateien gefunden — der Wächter wäre sonst grün, ohne etwas zu prüfen."
        )
        ...
        for pfad, baum in dateien:
            for knoten in ast.walk(baum):
                ...
                if not pruefcode.ist_pruefklasse(knoten):
                    verwaist.append("%s:%d %s" % (pfad.name, knoten.lineno, knoten.name))
        self.assertEqual(
            verwaist,
            [],
    ...
    def test_es_gibt_ueberhaupt_pruefungen_zu_finden(self):
        """Gegenprobe: Sonst wäre der Wächter oben trivial grün."""
        dateien = list(self._dateien())
        pruefcode = Pruefcode().lesen(dateien)
        gefunden = sum(len(m) for _p, baum in dateien for _k, m in pruefcode.pruefklassen(baum))
        self.assertGreater(gefunden, 100)

    def test_die_wurzelbasen_sind_die_von_unittest(self):
        """Wer hier etwas einträgt, das keine Testbasis ist, macht den
        Wächter blind."""
