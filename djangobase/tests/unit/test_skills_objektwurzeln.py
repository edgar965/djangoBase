# -*- coding: utf-8 -*-
"""Objektwurzeln — misst die Form des Objektmodells, nicht eine Stelle.
...
class ObjektwurzelnTest(BasisTest):
    ...
    def test_mehrere_stellen_werden_zusammengefasst(self):
        ...
        self.assertEqual(len(satz.befunde), 1, "eine Klasse gehört einmal in den Bericht")
    ...
    def test_wer_schon_einen_platz_im_baum_hat_ist_eine_warnung(self):
        ...
        self.assertIn("Kamera", satz.befunde[0].warum, "die Meldung muss sagen, WER den Platz schon hat")
    ...
    def test_die_zahl_der_wurzeln_steht_im_kopf(self):
        ...
        self.assertTrue(any(z.startswith("Wurzeln") and ": 2 " in z for z in satz.kopf), satz.kopf)
    ...
    def test_ein_baum_mit_einer_wurzel_ist_sauber(self):
        ...
        self.assertFalse(
            satz.befunde, "ein Baum mit EINER Wurzel darf nichts melden: %s" % [b.was for b in satz.befunde]
        )
    ...
    def test_was_das_rahmenwerk_verlangt_bleibt_draussen(self):
        ...
        satz = self._lauf(
            {"meine_filter.py": ("from django import template\n\n\nregister = template.Library()\n")}, ab="0"
        )
        self.assertFalse(satz.befunde)
    ...
    def test_eine_instanz_in_einer_funktion_ist_keine_wurzel(self):
        ...
        satz = self._lauf({"a.py": ("class Ding:\n    pass\n\n\ndef mach():\n    return Ding()\n")}, ab="0")
        self.assertFalse(satz.befunde)
