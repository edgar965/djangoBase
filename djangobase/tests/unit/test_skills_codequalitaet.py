# -*- coding: utf-8 -*-
"""Das eine Werkzeug im Ordner, das nicht selbst misst.
...
def _fehlendes_werkzeug(*module):
    """Name des ersten fehlenden Moduls - oder '' wenn alle da sind."""
...
BRAUCHT_WERKZEUGE = unittest.skipIf(FEHLT, '%s ist nicht installiert - siehe Extra „codequalitaet"' % FEHLT)
...
class DasWerkzeugIstAngemeldet(BasisTest):
    ...
    def test_es_nennt_seine_werkzeuge_im_titel(self):
        """Damit nachlesbar ist, WER das behauptet."""
    ...
    def test_es_hat_einen_anlassfall(self):
        """Ein Werkzeug ohne Selbsttest kann still nichts mehr finden."""
...
class WasEsMeldet(BasisTest):
    ...
    def test_ein_sauberes_projekt_bleibt_still(self):
        satz = self._satz({"gut.py": "def wenig():\n    return 1\n"})
    ...
    def test_der_ort_traegt_die_zeilennummer(self):
        """Ohne sie ist ein Befund eine Behauptung ohne Adresse."""
        satz = self._satz({"schlecht.py": self.QUELLE})
    ...
    def test_der_kopf_nennt_jedes_verfahren(self):
        satz = self._satz({"schlecht.py": self.QUELLE})
        kopf = " ".join(satz.kopf)
        for teil in ("Komplexität", "Wartbarkeitsindex", "Echte Fehler", "Stil"):
            self.assertIn(teil, kopf)


@BRAUCHT_WERKZEUGE
class NichtAllesIstGleichDringend(BasisTest):
    """Ein undefinierter Name ist ein Fehler, eine lange Zeile eine
    ...
    def test_stil_ist_nur_ein_hinweis(self):
        gewichte = self._gewichte({"a.py": 'x = "%s"\n' % ("y" * 120)})
    ...
    def test_ein_undefinierter_name_ist_ein_fehler(self):
        gewichte = self._gewichte({"a.py": "def machen():\n    return gibtsnicht\n"})
...
class NoqaZaehltNichtAlsFund(BasisTest):
    """DER FUND VOM 24.08.2026: 211 von 245 waren ausdrücklich gewollt."""
    ...
    def test_eine_markierte_einfuhr_ist_kein_befund(self):
        zeile, satz = self._fehlerzeile("import os  # noqa: F401\n")
    ...
    def test_sie_wird_aber_gezaehlt_und_genannt(self):
        """Weggelassen wäre sie unsichtbar — und dann wüsste niemand,
        wie viele Ausnahmen im Projekt stehen."""
        zeile, _satz = self._fehlerzeile("import os  # noqa: F401\n")
        self.assertIn("1 ausdrücklich erlaubt", zeile)

    def test_ohne_marke_zaehlt_es_beim_zustaendigen_werkzeug(self):
        """Eine tote Einfuhr führt `tote-importe` — hier steht nur noch
        ...
        """
        zeile, satz = self._fehlerzeile("import os\n")
        self.assertFalse([b for b in satz.befunde if "Echte Fehler" in b.was])
        self.assertIn("tote-importe", zeile)

    def test_eine_unbenutzte_variable_bleibt_hier(self):
        """Nicht alles wandert ab — nur was ein eigenes Werkzeug hat."""
        _zeile, satz = self._fehlerzeile("def machen():\n    x = 1\n    return 2\n")
...
class GarKeineKappung(BasisTest):
    ...
    def test_das_schwerste_steht_vorn(self):
        ...
        self.assertEqual(
            gewichte,
            sorted(
                gewichte, key=lambda g: {Befund.FEHLER: 0, Befund.WARNUNG: 1, Befund.HINWEIS: 2}.get(g, 3)
            ),
        )
...
class EineGescheiterteMessungIstEinFund(BasisTest):
    ...
    def _satz(self):
        ...
        (ordner / "gut.py").write_text("def eins():\n    return 1\n", encoding="utf-8")
        (ordner / "kaputt.py").write_text("def (:\n", encoding="utf-8")
        werkzeug = werkzeug_finden("code-qualitaet")
        werkzeug.wurzel = lambda: ordner
        return werkzeug.pruefen()

    def test_die_kaputte_datei_wird_gemeldet(self):
        gescheitert = [b for b in self._satz().befunde if "gescheitert" in b.was]
    ...
    def test_der_grund_steht_dabei(self):
        """„Ging nicht" ist keine Auskunft."""
        gescheitert = [b for b in self._satz().befunde if "gescheitert" in b.was]
        self.assertIn("SyntaxError", gescheitert[0].warum)

    def test_je_datei_eine_zeile(self):
        """Eine kaputte Datei lässt alle Verfahren scheitern — vier
    ...
    def test_der_kopf_nennt_die_zahl(self):
        """Sonst steht darunter eine Statistik über 1 Datei, die wie eine
        über 2 aussieht."""
