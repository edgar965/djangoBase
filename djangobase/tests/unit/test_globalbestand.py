...
class WasAufModulebeneSteht(BasisTest):
    ...
    def test_die_ausfuhrliste_ist_kein_zustand(self):
        """DER FALL: `__all__` stellte ein Viertel aller Funde."""
...
class WelcheSeiteWelchesSkript(BasisTest):
    ...
    def test_die_abhaengigkeiten_stehen_daneben(self):
        """Ein Skript zieht weitere — das gehört zur Seite."""
    ...
    def test_ein_fehlendes_skript_wirft_nicht(self):
        ...
        b = _bestand({"templates/s.html": "<script src=\"{% static 'gibts/nicht.js' %}\"></script>"})
...
class DieHauptaeste(BasisTest):
    def test_verzeichnisse_mit_klassen_werden_gefunden(self):
        """Mit KLASSEN, nicht mit Python (geaendert 24.08.2026).
    ...
    def test_die_zahl_der_klassen_steht_dabei(self):
        """Die Zahl im Auswahlfeld sind KLASSEN, nicht Dateien.
        ...
        (ordner / "app").mkdir()
        for i in range(3):
            (ordner / "app" / ("x%d.py" % i)).write_text("class K%d:\n    pass\n" % i, encoding="utf-8")
...
class DieZahlenSagenDasselbe(BasisTest):
    """Auswahlfeld und Ergebnis müssen übereinstimmen.
    ...
    def _projekt(self):
        ...
        (ordner / "echt" / "tests" / "test_x.py").write_text("class DreiTest:\n    pass\n", encoding="utf-8")
        # Gleicher Name zweimal — zaehlt einmal, wie im Klassenmodell.
        (ordner / "echt" / "b.py").write_text("class Eins:\n    pass\n", encoding="utf-8")
        (ordner / "leer").mkdir()
        (ordner / "leer" / "nur_funktionen.py").write_text("def tu():\n    pass\n", encoding="utf-8")
        return ordner
    ...
    def test_die_quelle_nennt_dieselbe_zahl_wie_das_modell(self):
        ...
        self.assertEqual(
            quellen["echt"],
            echt,
            "Auswahlfeld und Ergebnis nennen verschiedene Zahlen — dann glaubt man keiner von beiden.",
        )

    def test_gleichnamige_klassen_zaehlen_einmal(self):
        """In CamTrack heißen 82 Klassen doppelt: 1086 gegen 1004."""
