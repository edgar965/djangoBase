# -*- coding: utf-8 -*-
"""Die Baumsicht — misst, ob das Objektmodell wirklich ein Baum ist.
...
class BaumsichtTest(BasisTest):
    ...
    def _zahl(satz, stichwort):
        ...
        raise AssertionError("%r steht nicht im Kopf: %s" % (stichwort, satz.kopf))

    # ---------------------------------------------------- die vier Toepfe
    def test_die_vier_toepfe_stehen_im_kopf(self):
        satz = self._lauf({"a.py": "class A:\n    pass\n"}, ab="0")
        for stichwort in ("im Baum", "Wurzeln", "nur oertlich", "nirgends erzeugt"):
            self.assertTrue(
                any(z.startswith(stichwort) for z in satz.kopf), "%r fehlt: %s" % (stichwort, satz.kopf)
            )
    ...
    def test_jede_klasse_liegt_in_genau_einem_topf(self):
        ...
        self.assertEqual(gesamt, alle, "die Toepfe ueberschneiden sich oder es fehlt einer")
    ...
    def test_eine_fabrik_methode_zaehlt_als_erzeugung(self):
        ...
        self.assertEqual(
            self._zahl(satz, "nirgends erzeugt"), 0, "eine Klasse mit Fabrik-Methode gilt als tot"
        )
