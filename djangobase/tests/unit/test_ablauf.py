# -*- coding: utf-8 -*-
"""Ablauf — steht im Bild dieselbe Reihenfolge wie im Quelltext?
...
class _Bezug:
    """Ein Bezug ohne Verzeichnis — mehr braucht der Ablauf nicht."""
...
class DieReihenfolgeIstDieDesQuelltextes(BasisTest):
    ...
    def test_die_schritte_stehen_in_der_reihenfolge_des_codes(self):
        ...
        self.assertEqual([k.text for k in lauf.knoten], ["self.eins()", "self.zwei()", "self.drei()"])

    def test_jeder_schritt_nennt_seine_zeile(self):
        """Ohne Zeilennummer waere der Kasten eine Behauptung."""
        lauf = _lauf(self.QUELLE)
        self.assertEqual([k.zeile for k in lauf.knoten], [2, 3, 4])


class EineFrageTraegtBeideZweige(BasisTest):
    """Der Entscheidungsbaum, nach dem gefragt war."""
...
class EinAusgangIstKeinSchritt(BasisTest):
    """``return`` und ``raise`` beenden den Ablauf — sie tun nichts."""
    ...
    def test_ein_frueher_ausstieg_haengt_am_ja_zweig(self):
        """Das haeufigste Muster in echtem Code: pruefen, dann raus."""
        lauf = _lauf("def f(self):\n    if not self.geht():\n        return None\n    self.arbeiten()\n")
...
class SchleifeUndAbsicherungTragenIhrenRumpf(BasisTest):
    """Was mehrfach laeuft und was schiefgehen darf."""
    ...
    def test_aus_dem_for_wird_eine_schleife(self):
        ...
        self.assertEqual([k.text for k in schleife.rumpf], ["self.pruefen(kamera)"])

    def test_das_finally_steht_getrennt_vom_rumpf(self):
        """„danach immer" ist etwas anderes als „im Normalfall" — im Bild
        muss man das sehen."""
        lauf = _lauf(
            "def f(self):\n    try:\n        self.arbeiten()\n    finally:\n        self.aufraeumen()\n"
        )
        ...
        self.assertEqual([k.text for k in sicher.immer], ["self.aufraeumen()"])


class NICHTSWirdErfunden(BasisTest):
    """Die Gegenrichtung — ein Ablauf, der mehr zeigt als dasteht, ist
    schlimmer als keiner."""

    def test_eine_zeile_ohne_aufruf_erzeugt_keinen_kasten(self):
        """``x = 1`` ist kein Ablaufschritt."""
        lauf = _lauf("def f(self):\n    x = 1\n    self.tun()\n")
...
class AusJedemAblaufWirdEinBild(BasisTest):
    """Ein Ablauf ohne Bild waere wieder eine Liste."""
    ...
    def test_die_frage_wird_als_raute_gezeichnet(self):
        """Rechteck und Raute sind im Bild der Unterschied zwischen
    ...
    def test_der_ja_zweig_steht_eingerueckt(self):
        """Die Einrückung IST die Aussage: Was weiter rechts steht,
    ...
    def test_und_der_schritt_danach_wieder_auf_hoehe_der_frage(self):
        """Die Zusammenführung: Nach dem Zweig geht es links weiter."""
        bild = Ablaufbild(_lauf(self.QUELLE)).anordnen()
        frage = [k for k in bild.kaesten if k.knoten.art == "frage"][0]
