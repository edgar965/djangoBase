# -*- coding: utf-8 -*-
"""Tests der Gitabfrage — der Cache darf NICHTS am Ergebnis aendern.
...
class GitabfrageTest(BasisTest):
    ...
    def test_andere_argumente_sind_ein_anderer_schluessel(self):
        """Sonst liefert `rev-parse` die Antwort von `status`."""
    ...
    def test_haltbarkeit_null_schaltet_den_cache_ab(self):
        """Damit ein Projekt, dem 20 s zu lang sind, ihn ausschalten kann."""
        Gitabfrage.lauf("/repo", "status")
        Gitabfrage.lauf("/repo", "status")
        self.assertEqual(len(self.laeufe), 2, self.laeufe)
...
class GitabfrageEchtTest(BasisTest):
    """Gegen das echte `git` — ohne Attrappe."""
    ...
    def test_kein_repo_gibt_leerstring(self):
        """Die Versionen-Seite muss auch ohne git stehen — das war schon vor
    ...
    def test_eigenes_repo_liefert_einen_head(self):
        ...
        self.assertEqual(Gitabfrage.lauf(wurzel, "rev-parse", "--short=7", "HEAD").strip(), head)
