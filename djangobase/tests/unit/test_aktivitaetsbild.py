# -*- coding: utf-8 -*-
"""Aktivitaetsbild — ist die Schrift neben den Bloecken lesbar?
...
class JedeBeschriftungHatEinenWeissenHof(BasisTest):
    """Ohne ihn schneidet jede Linie die Schrift, die ueber ihr liegt."""
...
class EineMarkePasstInDenPlatzDenSieHat(BasisTest):
    """``[not os.path.isdir(self.zwischenlager_pfad_lang)]`` ist laenger
    als die Spur breit ist. Ungekuerzt lief sie ueber die Raute."""

    def setUp(self):
        self.svg = _bild(VERZWEIGT).svg()
        self.marken = re.findall(r'<text class="ak-m"[^>]*>(?:<title>[^<]*</title>)?([^<]*)</text>', self.svg)

    def test_es_gibt_ueberhaupt_eine_marke(self):
        self.assertTrue(self.marken, "Ohne Marke sagt die Raute nicht, wofuer sie sich entscheidet.")

    def test_keine_marke_ist_breiter_als_die_spur(self):
        """Grob gerechnet 6 Punkte je Zeichen bei 11er Schrift."""
        for marke in self.marken:
            self.assertLessEqual(len(marke) * 6, SPUR, "zu lang: %r" % marke)

    def test_der_volle_text_bleibt_als_tooltip(self):
        """Kuerzen ist noetig — die Bedingung ganz zu verlieren waere zu
...
class DieZeilennummerStehtLinks(BasisTest):
    """Rechts lag sie genau auf der Achse und der naechsten Raute."""
    ...
    def test_die_nummer_sitzt_in_der_linken_haelfte_des_kastens(self):
        ...
        self.assertLess(int(stelle.group(1)), kasten.x, "Die Nummer steht rechts und damit an der Achse.")


class ZwischenNebenspurUndAchseIstPlatz(BasisTest):
    """Die Marke steht in dieser Luecke — sie darf nicht null sein."""

    def test_die_luecke_traegt_mehr_als_ein_paar_zeichen(self):
        luecke = SPUR - KASTEN_B / 2.0 - 22
        self.assertGreater(
            luecke, 100, 'Bei 43 Punkten stand „[ja]" als Buchstabensalat zwischen Kasten und Raute.'
        )


class DasBildBringtSeinenGrundMit(BasisTest):
    """Als STIL, nicht als Attribut: Ein ``fill="…"`` verliert gegen jede
...
class DerTextIstProsaUndKeinQuelltext(BasisTest):
    """Der eigentliche Unterschied zur Workflow-Seite."""

    def test_aus_einem_methodennamen_werden_woerter(self):
        self.assertEqual(Beschriftung("_install_signal_handlers").satz(), "install signal handlers")

    def test_eine_abkuerzung_bleibt_ein_wort(self):
        """Sonst wurde aus ``SUCCESS`` ein „S U C C E S S"."""
        self.assertEqual(Beschriftung("SUCCESS").satz(), "SUCCESS")

    def test_der_empfaenger_steht_als_gegenstand_davor(self):
        self.assertEqual(Beschriftung("prepare", None, "service").satz(), "service: prepare")

    def test_self_ist_kein_gegenstand(self):
        """``self`` steht in jedem zweiten Kasten und waere Rauschen."""
    ...
    def test_ein_docstring_schlaegt_den_namen(self):
        ...
        self.assertEqual(Beschriftung("helfen", Bezug()).satz(), "Raeumt die Zwischenablage auf")

    def test_ein_docstring_der_mit_einer_ueberschrift_anfaengt_zaehlt_nicht(self):
        """``>>>`` oder ``===`` beschreiben keine Handlung."""
        quelle = 'def helfen(self):\n    """>>> helfen()"""\n    pass\n'
...
class AusgabeIstKeineHandlung(BasisTest):
    """Im ersten Bild stand neunmal „stdout: write"."""
...
class JederKastenSagtWoherErKommt(BasisTest):
    """Klasse, Methode und Fundstelle als Daten am Kasten.
    ...
    def test_die_methode_steht_am_kasten(self):
        ...
        self.assertEqual(Beschriftung.herkunft(knoten, v).get("methode"), "vorbereiten")

    def test_die_fundstelle_der_definition_steht_dabei(self):
        """Wo der AUFRUF steht und wo die Funktion DEFINIERT ist, sind
    ...
    def test_die_quellzeile_bleibt_erhalten(self):
        knoten, v = self._knoten()
        self.assertIn("vorbereiten", Beschriftung.herkunft(knoten, v).get("quelle", ""))

    def test_eine_erzeugte_klasse_heisst_nicht_methode(self):
        """``self.x = RecordingService(...)`` stand als „Methode:
    ...
    def test_im_bild_haengen_die_daten_am_kasten(self):
        ...
        svg = Aktivitaetsbild(
            ...
            herkunft=lambda k: Beschriftung.herkunft(k, v),
        ).svg()
    ...
    def test_jeder_kasten_sagt_wo_er_selbst_steht(self):
        """Auch ein Schritt ohne Ziel gehört zu einer Funktion.
        ...
        v = _EinVerzeichnis()
        lauf = Ablauf(_Bezug(self.QUELLE), v).lesen()
        svg = Aktivitaetsbild(lauf, herkunft=lambda k: Beschriftung.herkunft(k, v)).svg()
        self.assertIn('data-gehoertzu="A.f', svg)


class ZweiKaestenDuerfenNichtDasselbeBehaupten(BasisTest):
    """Das Stottern, das Edgar am 27.08.2026 gefunden hat.
    ...
    def test_der_name_wird_nicht_verdoppelt(self):
        """„signal: signal" ist der Fehler, um den es geht."""
    ...
    def test_ein_ausdruck_als_argument_kommt_NICHT_in_den_kasten(self):
        """Sonst wäre der Kasten wieder Quelltext — genau davon soll das
        Bild ja wegkommen."""
        lauf = Ablauf(_Bezug("def f(self):\n    warte.warte(options['poll'] or 5)\n")).lesen()
        self.assertEqual(lauf.knoten[0].merkmal, "")


class DasFensterNenntDieRufer(BasisTest):
    """„auch von wem die aufgerufen wird" (27.08.2026).
    ...
    class _MitRufern(_EinVerzeichnis):
        ...
        def rufer(_schluessel):
            return [type("B", (), {"anzeige": "Erster.tun"})(), type("B", (), {"anzeige": "Zweiter.tun"})()]

    def test_bei_vielen_rufern_wird_gekuerzt(self):
        """Eine Liste mit dreißig Namen beantwortet die Frage nicht mehr,
        sie verdeckt sie."""

        class Viele(_EinVerzeichnis):
            @staticmethod
            def rufer(_schluessel):
                return [type("B", (), {"anzeige": "K%02d.tun" % i})() for i in range(20)]
    ...
    def test_ohne_rufer_gibt_es_kein_feld(self):
        """Kein „0 Rufer" — was leer ist, zeigt die Seite gar nicht an."""
        v = _EinVerzeichnis()
        knoten = Ablauf(_Bezug("def f(self):\n    self.service.vorbereiten()\n"), v).lesen().knoten[0]
        self.assertNotIn("gerufenvon", Beschriftung.herkunft(knoten, v))

    def test_die_rufer_kommen_als_liste_nicht_als_satz(self):
        """Vier Namen im Fließtext kann niemand auseinanderhalten."""
        v = self._MitRufern()
        knoten = Ablauf(_Bezug("def f(self):\n    self.service.vorbereiten()\n"), v).lesen().knoten[0]
        angaben = Beschriftung.herkunft(knoten, v)
        self.assertEqual(angaben["gerufenvon"].split("|"), ["Erster.tun", "Zweiter.tun"])
        self.assertEqual(angaben["ruferzahl"], 2)


class DasFensterSagtNICHTSZweimal(BasisTest):
    """Die Ansage vom 27.08.2026.
    ...
    def test_die_kennung_steht_oben_als_klasse_punkt_methode(self):
        v = _EinVerzeichnis()
        knoten = Ablauf(_Bezug("def f(self):\n    self.service.vorbereiten()\n"), v).lesen().knoten[0]
        self.assertEqual(Beschriftung.herkunft(knoten, v).get("voll"), "Dienst.vorbereiten")

    def test_eine_freie_funktion_traegt_ihr_modul_als_gegenstand(self):
        """``get_trt_cache_dir`` allein sagt nicht, wo es herkommt;
        ...
        self.assertEqual(Beschriftung._voll(Frei()), "path_resolver.get_trt_cache_dir")
