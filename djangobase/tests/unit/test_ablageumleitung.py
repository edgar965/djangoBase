# -*- coding: utf-8 -*-
"""`Ablageumleitung._verwaiste_raeumen` — was weg darf und was nicht.
...
class VerwaisteRaeumenTest(BasisTest):
    def setUp(self):
        ...
        self.eltern = Path(tempfile.mkdtemp(prefix="verwaist_", dir=tempfile.gettempdir()))
    ...
    def test_ordner_eines_toten_prozesses_faellt_sofort(self):
        """Ohne Frist — der Name sagt, wessen Rest es ist."""
        tot = self._anlegen("p999999999")
        Ablageumleitung._verwaiste_raeumen(self.eltern, self.eigener)
        self.assertFalse(tot.exists(), "Der Rest eines toten Prozesses muss sofort fallen")

    def test_ordner_ohne_prozessnummer_faellt_nach_der_frist(self):
        """Ueber ihn weiss man nichts — dann entscheidet wieder das Alter."""
    ...
    def test_ordner_eines_laufenden_prozesses_bleibt(self):
        """Die gefaehrliche Richtung: Er schreibt vielleicht gerade hinein."""
        import subprocess
        import sys
        ...
        Ablageumleitung._verwaiste_raeumen(self.eltern, self.eigener)
        self.assertTrue(
            lebend.exists(), "Ein laufender Prozess darf seinen Ordner behalten — auch einen uralten"
        )
