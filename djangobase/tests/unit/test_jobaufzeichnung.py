# -*- coding: utf-8 -*-
"""Die Aufzeichnung: jeder Befehlslauf landet im Verlauf - und stoert nie.
...
class _Befehl(BaseCommand):
    """Ein Befehl, der tut, was der Test vorgibt."""
...
class AufzeichnungBasis(MitTempordner):
    """Haengt die Aufzeichnung um EINEN Befehl - nicht um BaseCommand.
    ...
    def setUp(self):
        ...
        Jobaufzeichnung._notieren = staticmethod(
            ...
            lambda kennung, dauer_s, erfolg, fehler, *rest: self.verlauf.notieren(
                kennung, dauer_s, erfolg, fehler
            )
        )
    ...
    def laufen_lassen(self, befehl, kennung="mail_sync"):
        """Den Befehl durch die Umhuellung schicken."""
        typ = type(befehl)
        alt = typ.__module__
...
class EinBefehlDerDurchlaeuft(AufzeichnungBasis, SimpleTestCase):
    """Gegeben: Ein Befehl, der ohne Fehler endet."""
    ...
    def test_liefert_sein_ergebnis_unveraendert_zurueck(self):
        """Die Messung darf am Ergebnis des Befehls nichts aendern."""
        self.assertEqual(self.laufen_lassen(_Befehl(ergebnis="fertig")), "fertig")
...
class EinBefehlDerWirft(AufzeichnungBasis, SimpleTestCase):
    """Gegeben: Ein Befehl, der mit einer Ausnahme endet."""

    def test_reicht_die_ausnahme_unveraendert_weiter(self):
        """Die wichtigste Zusicherung — siehe Modul-Doku."""
...
class EinAbgebrochenerBefehl(AufzeichnungBasis, SimpleTestCase):
    """Gegeben: Der Befehl wird mit Strg+C abgebrochen."""

    def test_wird_ebenfalls_notiert(self):
        """Sonst staende der Lauf fuer immer als unbeendet da."""
...
class EinAusgeschlossenerBefehl(AufzeichnungBasis, SimpleTestCase):
    """Gegeben: ``runserver`` - ein Werkzeug, kein Ablauf des Projekts."""

    def test_wird_nicht_notiert(self):
        """``runserver`` liefe stundenlang und saehe aus wie ein Job."""
...
class EinBefehlAusEinerFremdenApp(AufzeichnungBasis, SimpleTestCase):
    """Gegeben: Ein Befehl von djangoBase oder Django selbst.
    ...
    def test_aus_einer_projekt_app_wird_notiert(self):
        """Die Gegenprobe: Der Filter darf nicht zu viel wegnehmen."""
...
class EineZweimalEingeschalteteAufzeichnung(SimpleTestCase):
    """Gegeben: ``einschalten()`` wird zweimal gerufen (zwei AppConfigs)."""

    def test_legt_sich_nicht_doppelt_um_den_befehl(self):
        """Sonst staende jeder Lauf zweimal im Verlauf."""
