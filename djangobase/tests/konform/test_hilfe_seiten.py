# -*- coding: utf-8 -*-
"""Sind die Hilfe-Seiten eingehängt, konfiguriert und unter deutschen Adressen?
...
SEITEN = (("versionen", "/hilfe/versionen/"), ("logs", "/hilfe/logs/"), ("tests", "/hilfe/tests/"))
...
class AdressenTest(KonformTest):
    """Eingehängt, deutsch benannt, und auf djangoBase zeigend."""

    databases = []  # ausdrücklich keine: dieser Test fasst nie Daten an

    def test_alle_pflichtseiten_sind_eingehaengt(self):
        for name, _pfad in SEITEN:
            with self.subTest(seite=name):
                try:
                    reverse("djangobase:%s" % name)
                except NoReverseMatch:
                    self.fail(
                        "„djangobase:%s“ löst nicht auf. Ist "
                        "djangobase.urls unter /hilfe/ mit "
                        "namespace='djangobase' eingehängt?" % name
                    )

    def test_deutsche_slugs(self):
        """Zwei Adressen für dieselbe Seite lassen Lesezeichen und Doku
        auseinanderlaufen."""
        for name, pfad in SEITEN:
            with self.subTest(seite=name):
                ...
                self.assertEqual(adresse, pfad, "„%s“ liegt unter %s statt unter %s." % (name, adresse, pfad))

    def test_adressen_zeigen_auf_djangobase(self):
        """``reverse`` sagt nur, dass ein Name existiert. Erst ``resolve``
        ...
        for name, pfad in SEITEN:
            with self.subTest(seite=name):
                try:
                    ...
                    self.fail("%s ist nicht auflösbar." % pfad)
                modul = getattr(treffer.func, "__module__", "")
                eigen = getattr(getattr(treffer.func, "view_class", None), "__module__", "")
                self.assertTrue(
                    "djangobase" in modul or "djangobase" in eigen,
                    "%s wird von %r bedient, nicht von djangoBase." % (pfad, modul or eigen),
                )

    def test_keine_englischen_dubletten(self):
        """``/versions/`` neben ``/versionen/`` wäre die Dublette, die der
        deutsche Slug vermeiden soll."""
        for englisch in ("/hilfe/versions/", "/hilfe/settings/", "/hilfe/logs-clear/"):
            with self.subTest(pfad=englisch):
                ...
                self.fail(
                    "%s ist auflösbar (%s) — eine englische Dublette."
                    % (englisch, getattr(treffer.func, "__name__", "?"))
                )


class KonfigurationTest(KonformTest):
    """„Gefüllt" — der Teil, den man der Seite nicht ansieht."""
    ...
    def test_logs_seite_hat_quellen(self):
        ...
        self.assertTrue(
            quellen or anbieter,
            "Weder DJANGOBASE['log_sources'] noch "
            "['log_source_provider'] gesetzt. Hilfe → Logs zeigt "
            "dann dauerhaft nichts an, ohne das zu sagen — und "
            "eine leere Logseite liest sich wie „keine Fehler“.",
        )

    def test_logs_quellen_zeigen_auf_vorhandene_dateien(self):
        """Ein Eintrag auf eine Datei, die es nicht gibt, ergibt einen leeren
        ...
        self.assertFalse(fehlend, "Diese Log-Quellen zeigen ins Leere: %s" % ", ".join(fehlend[:5]))

    @staticmethod
    def _befehle():
        """Was die Seite WIRKLICH anzeigt: Konfiguration oder Ableitung.
    ...
    def test_tests_seite_hat_befehle(self):
        """Ohne Befehle gibt es im UI nichts zu starten — die
        Konvention „Suite aus dem UI startbar" ist dann still unerfüllt."""
        self.assertTrue(
            self._befehle(),
            "Weder DJANGOBASE['test_befehle'] noch die Ableitung aus dem "
            "Dateibestand liefern etwas. Hilfe → Tests zeigt dann keinen "
            "Startknopf für eine Suite.",
        )

    def test_testbefehle_sind_vollstaendig(self):
        """Ein Eintrag ohne ``cmd`` ist ein Knopf, der nichts tut."""
        befehle = self._befehle()
        self.assertTrue(befehle, "keine Befehle - siehe Test darüber")
        luecken = []
        for b in befehle:
            ...
            if fehlt:
                luecken.append("%s: ohne %s" % (b.get("slug") or b.get("name") or "?", ", ".join(fehlt)))
        self.assertFalse(luecken, "Unvollständige test_befehle: %s" % "; ".join(luecken[:5]))


class GegenprobeTest(SimpleTestCase):
    """Prüfen die Regeln überhaupt etwas?"""

    databases = []

    def test_erfundene_adresse_ist_nicht_aufloesbar(self):
        """Löste ALLES auf (Catch-all-Route), wäre die Dubletten-Prüfung
        wertlos."""
        with self.assertRaises(Resolver404):
            resolve("/hilfe/gibt-es-nicht-xyz/")

    def test_resolve_liefert_ein_modul(self):
        """Ohne ``__module__`` prüfte test_adressen_zeigen_auf_djangobase
