# -*- coding: utf-8 -*-
"""Sieht der Nutzer je eine gecachte Seite?
...
def _durch(antwort, pfad="/dax-handel/", **get):
    """Eine Antwort durch die Middleware schicken."""
    anfrage = RequestFactory().get(pfad, data=get)
    return CacheHeaderMiddleware(lambda r: antwort)(anfrage)


class SeitenNichtCachenTest(KonformTest):
    """Der eigentliche Auftrag: keine gecachten Seiten."""
    ...
    def test_html_traegt_no_store(self):
        ...
        self.assertIn(
            "no-store",
            steuerung,
            "HTML-Antworten müssen no-store tragen, sonst zeigt der "
            "Browser nach einem Deploy die alte Seite. Gefunden: %r" % antwort.get("Cache-Control"),
        )
    ...
    def test_alte_zwischenspeicher_werden_mitgenommen(self):
        """``Pragma`` und ``Expires`` sind für HTTP/1.0-Proxys — sie kosten
    ...
    def test_json_bleibt_unberuehrt(self):
        """Eine API-Antwort ist keine Seite; wer sie cachen will, soll das
        selbst entscheiden."""
    ...
    def test_ausnahmen_werden_geachtet(self):
        """Wer eine Seite bewusst cachen lässt, trägt ihren Pfad ein."""
...
class StatikCachenTest(KonformTest):
    """Die andere Hälfte: versionierte Statik DARF lange liegen bleiben."""

    databases = []

    def test_statik_mit_kennung_wird_lange_gecacht(self):
        antwort = _durch(
            HttpResponse(b"/* css */", content_type="text/css"), pfad="/static/app/x.css", v="123"
        )
        steuerung = (antwort.get("Cache-Control") or "").lower()
        self.assertIn(
            "max-age=",
            steuerung,
            "Versionierte Statik soll gecacht werden — sonst lädt "
            "jeder Seitenaufruf alles neu, und das ?v=-Busting "
            "daneben ist wirkungslos.",
        )
        self.assertIn("immutable", steuerung)
        self.assertNotIn("no-store", steuerung)

    def test_statik_ohne_kennung_wird_nachgefragt(self):
        """Ohne Kennung: liegen bleiben ja, ungefragt ausliefern nein.
        ...
        """
        antwort = _durch(HttpResponse(b"/* css */", content_type="text/css"), pfad="/static/app/x.css")
        steuerung = (antwort.get("Cache-Control") or "").lower()
        self.assertIn("no-cache", steuerung)

    def test_und_dabei_nicht_verboten(self):
        """`no-cache` heißt „vor der Benutzung fragen", nicht „nicht
...
class EingehaengtTest(KonformTest):
    """Die Middleware muss auch laufen."""
    ...
    def test_middleware_steht_in_der_kette(self):
        self.assertIn(
            ...
            "djangobase.apps.ready() trägt sie normalerweise selbst "
            "nach — steht sie nicht drin, setzt niemand die Header.",
        )

    def test_keine_pauschale_nocache_middleware(self):
        """Eine projekteigene Middleware, die ALLES auf no-store setzt, hebt
        ...
        verdaechtig = [
            ...
            if "nocache" in m.lower().replace("_", "") and not m.startswith("djangobase.")
        ]
        self.assertFalse(
            verdaechtig,
            "Diese Middleware setzt vermutlich pauschal "
            "no-store: %s\n\nDamit lädt der Browser bei JEDEM "
            "Seitenaufruf sämtliche JS/CSS neu, und das "
            "?v=-Cache-Busting ist wirkungslos. djangoBase "
            "unterscheidet HTML und versionierte Statik — die "
            "eigene kann raus." % ", ".join(verdaechtig),
        )


class GegenprobeTest(SimpleTestCase):
    """Greifen die Prüfungen?"""

    databases = []

    def test_ohne_middleware_fehlen_die_header(self):
        """Sonst wäre „no-store gefunden" auch ohne die Middleware wahr."""
        antwort = HttpResponse(HTML)
        self.assertIsNone(antwort.get("Cache-Control"))

    def test_html_wird_an_content_type_erkannt(self):
        """Nicht am Pfad: Eine Seite unter /api/ ist genauso eine Seite."""
    ...
    def test_fehler_in_der_middleware_kostet_keine_seite(self):
        """Header sind Beiwerk — eine Ausnahme darf die Antwort nicht killen."""
