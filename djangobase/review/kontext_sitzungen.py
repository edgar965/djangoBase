# -*- coding: utf-8 -*-
"""Welche Claude-Code-Protokolle es zu diesem Projekt gibt.
...
class Sitzungen:
    """Die Protokolldateien zu einem Projektverzeichnis."""
    ...
    def __init__(self, projektpfad, wurzel=None):
        ...
        self.wurzel = Path(wurzel) if wurzel else (Path.home() / ".claude" / "projects")

    def ordner(self):
        """Der Protokollordner dieses Projekts — oder ``None``."""
    ...
    def _suchen(self):
        """Der Ordner, dessen Name auf den Projektnamen endet.
        ...
        treffer = [p for p in self.wurzel.iterdir() if p.is_dir() and p.name.lower().endswith("-" + name)]
        if not treffer:
            return None
        return max(treffer, key=lambda p: p.stat().st_mtime)

    def liste(self):
        """``[{name, pfad, mb, wann}]`` — die neuesten zuerst."""
        ordner = self.ordner()
        if ordner is None:
            return []
        dateien = sorted(ordner.glob("*.jsonl"), key=lambda p: p.stat().st_mtime, reverse=True)
        raus = []
            ...
            raus.append(
                {
                    ...
                    "wann": datetime.fromtimestamp(stand.st_mtime).strftime("%d.%m.%Y %H:%M"),
                }
            )
        return raus
    ...
    def gueltig(self, pfad):
        """Liegt ``pfad`` wirklich im Protokollordner dieses Projekts?
        ...
        try:
            ...
            return False
        return ziel.parent == ordner.resolve() and ziel.suffix == ".jsonl" and ziel.is_file()
