# -*- coding: utf-8 -*-
"""Wohin der Kontext einer Claude-Code-Sitzung geht — gemessen.
...
class Kontextanalyse:
    """Zaehlt, welche Art von Eintrag wie viele Zeichen belegt."""
    ...
    # ── Lesen ───────────────────────────────────────────────────
    def lesen(self):
        """Das Protokoll einmal durchgehen. Liest nur, schreibt nichts."""
    ...
    def _teil(self, teil):
        ...
        if sorte == "tool_use":
            self._zaehlen(
                "%s (Aufruf)" % teil.get("name", "?"),
                len(json.dumps(teil.get("input", {}), ensure_ascii=False)),
            )
        elif sorte == "tool_result":
            gross = len(json.dumps(teil.get("content", ""), ensure_ascii=False))
            kennung = teil.get("tool_use_id")
            name = self._namen.get(kennung, "Werkzeug")
            self._zaehlen("%s (Ergebnis)" % name, gross)
            if gross > BROCKEN_AB:
                self.brocken.append((gross, name, self._befehle.get(kennung, "")))
        elif sorte == "image":
            self._zaehlen("Bildschirmfoto", len(json.dumps(teil.get("source", {}), ensure_ascii=False)))
    ...
    @staticmethod
    def _aufruf(teil):
        """Woran man den Aufruf wiedererkennt — Befehl, Pfad, Muster."""
    ...
    def kennzahlen(self):
        ...
        return {
            ...
            "werkzeugaufrufe": sum(n for k, n in self.anzahl_werkzeug.items() if k.endswith("(Aufruf)")),
        }

    def arten(self):
        """``[{name, mb, token, anzahl}]`` — nach Groesse."""
        return [self._zeile(k, v, self.anzahl_art[k]) for k, v in self.nach_art.most_common()]

    def werkzeuge(self, wie_viele=15):
        return [
            self._zeile(k, v, self.anzahl_werkzeug[k]) for k, v in self.nach_werkzeug.most_common(wie_viele)
        ]

    def groesste(self, wie_viele=15):
        """Die dicksten Einzelergebnisse — dort liegt der Hebel."""
        return [
            {"zeichen": g, "token": int(g / ZEICHEN_JE_TOKEN), "werkzeug": n, "aufruf": b}
