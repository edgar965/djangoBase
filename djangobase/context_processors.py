...
def _hilfe_extra(c):
    """Die Hilfeseiten des Projekts — Liste ODER Funktion.
...
def _offen_setzen(eintraege, aktiv):
    """Ein Klappmenü, dessen Seite gerade offen ist, muss offen stehen.
    ...
    for eintrag in eintraege or ():
        ...
        for schluessel in ("untermenu", "abschnitt"):
            ...
            if any(
                k.get("offen") or (k.get("aktiv") and k["aktiv"] == aktiv)
                for k in kinder
    ...
    return {
        "djangobase": {
            ...
            "hilfe_extra": _offen_setzen(
                _hilfe_extra(c), getattr(getattr(request, "resolver_match", None), "url_name", "") or ""
            ),
...
def _test_arten(c):
    ...
    try:
        ...
        logging.getLogger("djangobase.tests").exception(
            "Test-Kategorien für das Menue nicht ermittelbar - der Eintrag Tests zeigt dann nur 'Alle'"
        )
        return []
    # Dictionary gewollt: geht unveraendert in die Vorlage.
    return [
        {"art": a["art"], "kurz": a["kurz"], "anzahl": len(a["befehle"])} for a in arten if a.get("sammel")
    ]
