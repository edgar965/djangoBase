...
class Jobuebersicht:
    """Baut die Tabelle der Jobs-Seite.
    ...
    def zeilen(self, neu=False):
        """Je Job eine Zeile, Jobs mit Fehlern zuerst.
        ...
        raus = [self._zeile(job, historie.pop(job["kennung"], None)) for job in bestand]
    ...
    @staticmethod
    def _zeitpunkt(iso):
        """Aus dem gespeicherten ISO-Text ein ``datetime`` machen.
    ...
    def _reihenfolge(zeile):
        """Erst die Fehlgeschlagenen, dann die Gelaufenen, dann der Rest.
    ...
    def stand(self):
        """Wann der Bestand zuletzt ermittelt wurde (``datetime``/``None``)."""
    ...
    def zahlen(self):
        """Kopfzeile der Seite: wie viele Jobs, wie viele mit Fehler."""
        zeilen = self.zeilen()
        return {
            "gesamt": len(zeilen),
...
def _umgekehrt(text):
    """Sortierschluessel, der Zeitstempel ABSTEIGEND ordnet.
