...
def beim_login(sender, request, user, **kwargs):
    ...
    LoginSitzung.objects.create(user=user, benutzer=user.get_username(), beginn=timezone.now(), ip=ip)
...
def beim_logout(sender, request, user, **kwargs):
    ...
    if user:
        offen = LoginSitzung.objects.filter(user=user, ende__isnull=True).order_by("-beginn").first()
        if offen:
...
def beim_login_fehlversuch(sender, credentials, request=None, **kwargs):
    ...
    auth_log.warning(
        "LOGIN-FEHLER für %r von %s",
        (credentials or {}).get("username") or (credentials or {}).get("email") or "?",
...
try:
    ...
    def beim_email_bestaetigt(sender, request, email_address, **kwargs):
        Teilnehmer.objects.filter(user_id=email_address.user_id, email_bestaetigt_am__isnull=True).update(
