# -*- coding: utf-8 -*-
"""Der gemerkte Bestand der Jobs - taeglich aufgefrischt.
...
class Jobkatalog:
    """Haelt die ermittelten Jobs und weiss, wann sie zu alt sind.
    ...
    # -------------------------------------------------------------- lesen
    def gemerkt(self):
        """Der gespeicherte Bestand als dict - oder ``None``.
    ...
    def ermittelt_am(self):
        """Wann zuletzt ermittelt wurde - als ``datetime`` oder ``None``."""
        daten = self.gemerkt()
        if not daten:
            return None
        try:
    ...
    def veraltet(self):
        """``True``, wenn nichts da ist oder die Frist abgelaufen ist."""
    ...
    def jobs(self):
        """Der Bestand. Ermittelt selbst neu, wenn er zu alt ist."""
        if self.veraltet():
    ...
    def aktualisieren(self):
        """Neu ermitteln und merken. Gibt die gefundenen Jobs zurueck."""
    ...
    def _schreiben(self, jobs):
        daten = {
            "ermittelt_am": datetime.now(timezone.utc).isoformat(timespec="seconds"),
            "jobs": jobs,
        }
        try:
