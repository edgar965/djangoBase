# -*- coding: utf-8 -*-
"""Bereiche - die ZWEITE Einteilung der Tests, quer zur Kategorie.
...
class Bereiche:
            ...
            self.angaben.append(
                {
                    "slug": slug,
                    "name": name,
                    "beschreibung": str(e.get("beschreibung") or e.get("description") or ""),
                    "praefixe": [],
                    "index": self.reihung[slug],
                }
            )
    ...
    def zeile_lesen(zeile):
        """``musik | Musik | search.tests.musik, x.y`` -> Angabe-Dictionary.
        ...
        return {
            ...
            "praefixe": [p.strip() for p in (teile[2] if len(teile) > 2 else "").split(",") if p.strip()],
    ...
    def als_zeilen(cls, angabe):
        """Angabe -> Zeilenformat der Oberflaeche (Umkehrung von `zeile_lesen`).
        ...
        for e in cls(angabe).liste():
            aus.append(
                " | ".join([e["slug"], e["name"], ", ".join(e["praefixe"]), e["beschreibung"]]).rstrip(" |")
            )
        return aus

    @classmethod
    def aus_einstellungen(cls):
        """Angabe aus ``DJANGOBASE["test_bereiche"]`` bzw. den Einstellungen.
    ...
    def slug_von(self, test_id):
        """Bereichs-Slug einer Test-ID - ``""`` wenn keiner ableitbar."""
    ...
    def _aus_ordner(self, kennung):
        """Rueckfall ohne Projekt-Angabe: der Ordner unter ``tests``.
    ...
    def liste(self):
        """Die Bereiche in der ANGEGEBENEN Reihenfolge - für Navigationen."""
    ...
    def ziele(self):
        """Die Bereiche, in die verschoben werden DARF (gecacht).
        ...
        if self._ziele is None:
            ...
            for slug, praefix in self.praefixe.items():
                if any(p != praefix and p.startswith(praefix + ".") for p in self.praefixe.values()):
                    continue
                aus[slug] = praefix
            self._ziele = aus
        return self._ziele
    ...
    def auswahl(self, slug, moeglich=True):
        """Die Eintraege der Combo-Box „Bereich" - ``[(wert, name, gesetzt)]``.
    ...
    def platz(self, slug):
        """Sortierplatz eines Bereichs - Angabe-Reihenfolge, dann Name."""
        return (self.reihung.get(slug, 10**6), self.name_von(slug).lower())

    def sortiert(self, tests, schluessel=lambda t: t.get("id", "")):
        """Faelle nach Bereich (in der ANGEGEBENEN Reihenfolge), dann nach ID.
        ...
        """
        return sorted(tests, key=lambda t: (self.platz(self.slug_von(schluessel(t))), schluessel(t)))

    def gruppieren(self, tests, schluessel=lambda t: t.get("id", "")):
        """[{'slug','name','tests','anzahl'}] - alphabetisch, „Sonstige" zuletzt.
        ...
        aus = [
            {"slug": s, "name": self.name_von("" if s == self.REST[0] else s), "tests": v, "anzahl": len(v)}
