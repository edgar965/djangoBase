# -*- coding: utf-8 -*-
"""Cache-Kopfzeilen fuer die Statik — auch im Entwicklungsbetrieb.
...
class StatikKopfzeilen:
    """ASGI-Huelle, die den Statik-Antworten ihre Cache-Kopfzeile gibt."""

    def __init__(self, app):
        self.app = app

    async def __call__(self, scope, receive, send):
        """``scope``/``receive``/``send`` heissen hier NICHT deutsch.
        ...
        async def gesetzt(nachricht):
            if nachricht.get("type") == "http.response.start":
                ...
                nachricht["headers"] = self._ergaenzt(nachricht.get("headers") or [], kopfzeile)
    ...
    def _kopfzeile(scope):
        """Mit ``?v=`` ein Jahr, ohne Kennung nachfragen.
    ...
    def _ergaenzt(kopfzeilen, wert):
        """``Cache-Control`` setzen und ein vorhandenes ersetzen.
        ...
        """
        behalten = [(k, w) for k, w in kopfzeilen if k.lower() != b"cache-control"]
