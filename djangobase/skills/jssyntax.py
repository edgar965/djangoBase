# -*- coding: utf-8 -*-
"""JsSyntax - jede .js-Datei als ES-Modul pruefen.
...
class JsSyntax(Werkzeug):
    ...
    zweck = (
        "Kopiert jede .js-Datei als .mjs und lässt `node --check` darauf "
        "laufen - findet kaputte Importe, die als CommonJS durchgehen."
    )
    befund = (
        "3DTools: drei Dateien hatten eine Import-Zeile mitten in einem "
        "mehrzeiligen Import. `node --check` auf der .js-Datei war grün, "
        "als .mjs rot. Die Seiten blieben weiß."
    )
    abhilfe = (
        "Die gemeldete Zeile ansehen. Meist steht dort eine Zeile in "
...
""",
    ...
    def laufen(self):
        ...
        if not node:
            return Ergebnis(
                ...
                hinweis="Ohne Node ist diese Prüfung nicht möglich. Sie gilt NICHT als grün.",
            )
        ...
        with tempfile.TemporaryDirectory(prefix="jssyntax_") as ordner:
            ...
            for pfad in dateien:
                ...
                if meldung:
                    kaputt.append({"ort": pfad.relative_to(self.wurzel()).as_posix(), "meldung": meldung})
        return Ergebnis(
            ["ort", "meldung"],
            kaputt,
            zusammenfassung="%d Dateien als ES-Modul geprüft, %d mit Fehler" % (len(dateien), len(kaputt)),
        )
    ...
    def _pruefen(self, node, pfad, ziel):
        ...
        try:
            ...
            fertig = subprocess.run([node, "--check", str(ziel)], capture_output=True, text=True, timeout=30)
