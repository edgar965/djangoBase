...
class Endpunktzeiten(BefundWerkzeug):
    ...
    zweck = (
        "Ruft jede GET-Route mehrfach auf und listet Bestzeit, Median und "
        "Antwortgröße — die langsamsten zuerst."
    )
    abhilfe = (
        "Als erster Schritt jedes Performance-Durchgangs. Erst messen, dann "
        "optimieren: Die teuerste Stelle liegt fast nie dort, wo man sie "
        "vermutet."
    )
    befund = (
        "Im Ursprungsprojekt standen so acht Endpunkte über 200 ms auf der "
    ...
    dauer = "mehrere Minuten — gemessen: 172 s für 188 Routen bei EINEM Aufruf je Route"
    ...
    ohne_anlassfall_weil = "fragt den LAUFENDEN Server - der Fall ist eine Antwortzeit, keine Datei"
    ...
    def pruefen(self, laeufe="3", **_argumente):
        ...
        befunde = [
            Befund(
                ...
                gewicht=(
                    Befund.FEHLER
                    if m.best_ms > self.GRENZE_MS * 5
                    else Befund.WARNUNG
                    if m.best_ms > self.GRENZE_MS
                    else Befund.HINWEIS
                ),
            )
        ...
        if len(messwerte) > self.ZEILEN:
            kopf.append("angezeigt: die %d langsamsten von %d Routen" % (self.ZEILEN, len(messwerte)))
