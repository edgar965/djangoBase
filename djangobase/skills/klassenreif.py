# -*- coding: utf-8 -*-
"""Klassenreif — welches Modul wirklich eine Klasse werden sollte.
...
class Argumentquellen:
    """Woher die Aufrufer die Argumente nehmen: gehalten oder frisch getippt.
    ...
    @classmethod
    def aus(cls, dateien):
        """Alle Aufrufe im ganzen Projekt einsammeln — auch aus Ansichten
    ...
    def lesen(self, baum):
        for knoten in ast.walk(baum):
            ...
            if not name:
                continue
            self.stellen.setdefault(name, []).append(tuple(isinstance(a, self.GEHALTEN) for a in knoten.args))

    def gehalten(self, namen, breite):
        """Reicht irgendein Aufrufer in ALLEN ersten `breite` Stellen
...
PREIS_HINWEIS = (
    "Preis: %d Datei(en) führen dieses Modul ein; eine rein "
...
class Modulbefund:
    """Ein Modul und die Fragen, die es mit Ja beantwortet."""
    ...
    def als_befund(self):
        ...
        warum = " | ".join("%d. %s: %s" % (n, k, b) for n, k, b in sorted(self.fragen))
...
class Modulsicht:
    """Was auf Modulebene steht — und wer davon was anfasst."""

    def __init__(self, baum):
        self.funktionen = [k for k in baum.body if isinstance(k, (ast.FunctionDef, ast.AsyncFunctionDef))]
        self.klassen = [k for k in baum.body if isinstance(k, ast.ClassDef)]
        self.zustand = self._zustand(baum)

    @staticmethod
    def _zustand(baum):
        """Veraenderliche Namen auf Modulebene — ohne Konstanten.
        ...
        for k in baum.body:
            ...
            for ziel in k.targets:
                if not isinstance(ziel, ast.Name):
                    continue
                if ziel.id.isupper() or ziel.id.startswith("__") or ziel.id in NIE:
                    continue
                if isinstance(k.value, SAMMLUNG) or (
                    isinstance(k.value, ast.Call) and getattr(k.value.func, "id", "") in SAMMLUNG_RUF
                ):
                    raus[ziel.id] = k.lineno
        return {n: z for n, z in raus.items() if n in Modulsicht._beschrieben(baum, set(raus))}

    @staticmethod
    def _beschrieben(baum, kandidaten):
        """Welche dieser Namen VERAENDERT jemand?
        ...
        for knoten in ast.walk(baum):
            if isinstance(knoten, ast.Call):
                fn = knoten.func
                if isinstance(fn, ast.Attribute) and fn.attr in SCHREIBT and isinstance(fn.value, ast.Name):
                    beschrieben.add(fn.value.id)
            elif isinstance(knoten, ast.Global):
                beschrieben.update(knoten.names)
            elif isinstance(knoten, (ast.Assign, ast.AugAssign, ast.AnnAssign, ast.Delete)):
                ziele = (
                    knoten.targets
                    if isinstance(knoten, ast.Assign)
                    else getattr(knoten, "targets", None) or [knoten.target]
                )
                for ziel in ziele:
    ...
    def beruehrt(self, funktion):
        """Welche Modulzustaende fasst DIESE Funktion an?"""
        namen = set()
...
class Klassenreif(BefundWerkzeug):
    ...
    zweck = (
        "Stellt die vier Fragen, bei denen sich eine Klasse ihren Platz "
        "verdient: eigener Zustand, gefädelte Argumente, zwei Anliegen "
        "in einer Datei, mehr als ein Exemplar nötig."
    )
    abhilfe = (
        'Bevor man ein Modul „aufraeumt". Ein Modul IST bereits ein '
    ...
    befund = (
        "CamTrack, 26.08.2026: 806 Funktionen auf Modulebene, aber nur "
    ...
    anlassfall = Anlassfall(
        {
            ...
            "zwischenspeicher.py": "import threading\n"
            "\n"
            "_client = None\n"
            ...
            "tore.py": "def tor_eins(regel, daten, jetzt):\n"
            "    return bool(regel)\n"
            "\n\n"
        ...
        mindestens=3,
        erwartet_in="zwischenspeicher.py",
    ...
    def pruefen(self, **_argumente):
        ...
        kopf = [
            "%d Module gelesen" % geprueft,
            "%d beantworten mindestens eine der vier Fragen mit Ja" % len(befunde),
            "%d davon zwei oder mehr — dort steckt der Zustand, an dem sich Fehler festmachen" % schwer,
    ...
    def _ausgenommen(kurz):
        """Steht die Funktion hier richtig, egal was die vier Fragen sagen?"""
        teile = [t.lower() for t in kurz.replace("\\", "/").split("/")]
        letzter = teile[-1][:-3] if teile[-1].endswith(".py") else teile[-1]
        return any(t in AUSGENOMMEN_TEILE for t in teile[:-1]) or letzter in AUSGENOMMEN_TEILE

    def _einfuhren(self):
        """Wie viele Dateien führen welches Modul ein?
    ...
    def _frage1_zustand(fund, sicht):
        """``global`` oder ein veraenderter Modulzustand.
        ...
        if globale:
            fund.dazu(
                1,
                "Zustand mit `global`",
                "%s — wer ihn zuruecksetzen will, muss an Interna fassen" % ", ".join(sorted(globale)),
            )
            return
        if sicht.zustand:
            fund.dazu(
                1,
                "veraenderlicher Modulzustand",
                "%s (Zeile %s)"
                % (
                    ", ".join(sorted(sicht.zustand)),
                    ", ".join(str(z) for z in sorted(sicht.zustand.values())),
                ),
            )

    @staticmethod
    def _meldet_an(funktion):
        """Meldet ein Dekorator diese Funktion beim Rahmenwerk an?
    ...
    def _frage2_gefaedelt(fund, sicht, quellen=None):
        """Gleiche fuehrende Argumente in mehreren Funktionen.
        ...
        beste = [(len(f), k, f) for k, f in koepfe.items() if len(f) >= GEFAEDELT_AB]
        if not beste:
            return
        ...
        fund.dazu(
            2,
            "%d Funktionen faedeln dieselben Werte" % zahl,
            "(%s) in %s%s" % (", ".join(kopf), ", ".join(namen[:5]), " …" if len(namen) > 5 else ""),
        )

    # ── 3. Zwei Anliegen, die sich Zustand teilen ───────────────────
    @staticmethod
    def _frage3_zwei_anliegen(fund, sicht):
        """Zwei getrennte Zustands-Haeufchen in einer Datei.
        ...
        if getrennt:
            ...
            fund.dazu(
                ...
                "`%s` und `%s` werden von verschiedenen Funktionen geführt — keine fasst beide an" % (a, b),
            )

    # ── 4. Mehr als ein Exemplar noetig ─────────────────────────────
    @staticmethod
    def _frage4_exemplare(fund, sicht, baum):
        """Ein Woerterbuch, das von Hand nach einem Schlüssel geführt wird.
        ...
        for teil in ast.walk(baum):
            if not isinstance(teil, ast.Subscript):
                continue
            if not (isinstance(teil.value, ast.Name) and teil.value.id in sicht.zustand):
                continue
            ...
            fund.dazu(
                4,
                "Instanzverwaltung von Hand",
                "`%s[…]` wird nach einem berechneten Schlüssel "
                "geführt — je Exemplar ein Eintrag, aber keine Klasse" % teil.value.id,
            )
            return
