# -*- coding: utf-8 -*-
"""ScheinDocstring - Text, der wie Doku aussieht und keine ist.
...
class ScheinDocstring(BefundWerkzeug):
    ...
    @staticmethod
    def _traeger(baum):
        """Alles, was einen Docstring haben KANN und mit einem Ausdruck beginnt."""
        arten = (ast.Module, ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)
        return [
            k
            for k in ast.walk(baum)
            if isinstance(k, arten)
            and k.body
            and isinstance(k.body[0], ast.Expr)
            and not (isinstance(k.body[0].value, ast.Constant) and isinstance(k.body[0].value.value, str))
        ]

    @staticmethod
    def _aus_literal(wert):
        """Ist dieser Ausdruck an der WURZEL aus einem String-Literal gebaut?"""
        if isinstance(wert, ast.JoinedStr):  # f"…"
            return True
        if isinstance(wert, ast.BinOp) and isinstance(wert.op, ast.Mod):
            return isinstance(wert.left, ast.Constant) and isinstance(wert.left.value, str)
        if isinstance(wert, ast.Call) and isinstance(wert.func, ast.Attribute):
            ziel = wert.func.value  # "…".format(…)
            return (
                wert.func.attr == "format" and isinstance(ziel, ast.Constant) and isinstance(ziel.value, str)
            )
        return False
    ...
    def _hinweis(wert):
        if isinstance(wert, ast.JoinedStr):
            return "f vor den Anfuehrungszeichen entfernen und die Werte im Text benennen statt einsetzen"
        return (
            "Formatierung entfernen und den Wert bei seinem Namen nennen - "
            "dann steht die Zahl weiter an genau einer Stelle"
        )
