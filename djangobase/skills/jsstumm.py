# -*- coding: utf-8 -*-
"""JsStumm - Rueckmeldungen, die im Browser lautlos verschwinden.
...
class JsStumm(Werkzeug):
    ...
    MELDEPARAM = re.compile(
        r"^(text|txt|msg|nachricht|meldung|html|inner\w*|"
        r"titel|title|label|inhalt|content|body|wert|"
        r"fehler|hinweis|message)$",
        re.I,
    )
...
"""
    ...
    def laufen(self):
        ...
        rang = {
            "Meldung hängt am Element": 0,
            "Antwortfehler verschluckt": 1,
            "catch ohne Meldung": 2,
            ".catch verschluckt": 3,
        }
        ...
        return Ergebnis(
            ...
            zeilen,
            "%d Stellen: %s"
            % (len(zeilen), ", ".join("%d× %s" % (n, a) for a, n in sorted(nach_art.items())) or "keine"),
    ...
    def _meldung(self, kurz, zeilen):
        """Eine Funktion bekommt Text und wirft ihn weg, weil ein Element fehlt."""
        aus = []
        for nummer, zeile in enumerate(zeilen):
            ...
            if not any(
                self.MELDEPARAM.match(p.strip().lstrip(".")) for p in parameter.split(",") if p.strip()
            ):
                continue
            ...
            aus.append(
                self._als_zeile(
                    "Meldung hängt am Element",
                    kurz,
                    nummer,
                    zeile,
                    "`%s` bekommt einen Text, gibt ihn aber nur weiter, wenn `%s` "
                    "da ist — sonst passiert lautlos nichts" % (name, geprueft),
                )
            )
        return aus

    def _funktion(self, zeilen, nummer):
        """(Name, Parameterliste, Endzeile) der Funktion um ``nummer`` - oder None."""
    ...
    def _wird_beschriftet(cls, zeilen, von, bis, name):
        ...
        return any("%s.%s" % (name, eigenschaft) in text for eigenschaft in cls.BESCHRIFTET)
    ...
    def _catch(self, kurz, zeilen):
        return self._bloecke(
            ...
            self.CATCH,
            "catch ohne Meldung",
            "der Fehler ist hier zu Ende — nichts geschieht, der Ablauf läuft weiter als wäre nichts gewesen",
        )
    ...
    def _promise(self, kurz, zeilen):
        return self._bloecke(
            ...
            self.PROMISE_CATCH,
            ".catch verschluckt",
            "das Kettenglied fängt und sagt nichts — die abgebrochene Aktion sieht erfolgreich aus",
        )
    ...
    def _antwort(self, kurz, zeilen):
        """``if (!antwort.ok) return;`` - der Server hat den Grund mitgeschickt."""
        aus = []
        for nummer, zeile in enumerate(zeilen):
            if zeile.lstrip().startswith(("*", "//")):
                continue
    ...
    def _blockklammer(zeile, ab):
        """Position der ``{``, die den if-Block öffnet — oder None.
    ...
    def _als_zeile(art, kurz, nummer, zeile, warum):
        ...
        return {"art": art, "ort": "%s:%d" % (kurz, nummer + 1), "text": zeile.strip()[:110], "warum": warum}
