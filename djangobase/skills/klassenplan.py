# -*- coding: utf-8 -*-
"""Klassenplan - was aus freien Modulfunktionen eine Klasse machen wuerde.
...
class Klassenplan(Werkzeug):
    ...
    AB_FUNKTIONEN = 3
    UNINTERESSANT = {"self", "cls", "request", "args", "kwargs", "x", "y", "n", "i", "k", "v", "s", "t"}
...
"""
    ...
    def laufen(self):
        ...
        for d in self.dateien():
            ...
            funktionen = [
                ...
                if isinstance(k, (ast.FunctionDef, ast.AsyncFunctionDef)) and not k.decorator_list
            ]
            ...
            zeilen.append(
                {
                    ...
                    "felder": ", ".join("%s (%d×)" % (a.name, a.traegt) for a in geteilt[:4]),
                    "art": "Felder"
                    if felder and len(felder) == len(geteilt)
        ...
        return Ergebnis(
            ["datei", "zeile", "funktionen", "felder", "art", "klasse"],
            zeilen,
            "%d Dateien mit geteiltem Zustand, davon %d mit echten Feldern" % (len(zeilen), len(echte)),
    ...
    def _argumente(self, funktionen):
        ...
        for f in funktionen:
            zurueck = {
                k.value.id for k in ast.walk(f) if isinstance(k, ast.Return) and isinstance(k.value, ast.Name)
            }
