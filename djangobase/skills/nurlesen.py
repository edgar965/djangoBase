# -*- coding: utf-8 -*-
"""NurLesen — schreibt jemand in ein Verzeichnis, das nur gelesen werden darf?
...
class NurLesen(BefundWerkzeug):
    ...
    zweck = (
        "Findet Stellen, die in ein als read-only erklaertes Verzeichnis "
        "schreiben. Konfiguriert ueber "
        '`DJANGOBASE["daten_nur_lesen"]`.'
    )
    befund = (
        "3DTools: Die maennlichen Morphdaten schrumpften von 437 KB auf "
        "218 KB — halbe Vertexzahl, Charakter zerstoert. Ohne "
        "Fehlermeldung; nur das Netz im Browser sah falsch aus."
    )
    abhilfe = (
        "In ein Wegwerfverzeichnis schreiben. Wenn es wirklich die "
    ...
    def wurzeln(self):
        eigene = [str(w).replace("\\", "/") for w in (self.einstellung().get("wurzeln") or [])]
    ...
    def namen(self):
        return tuple(str(n) for n in (self.einstellung().get("einstellungen") or ()))
    ...
    def pruefen(self, **_argumente):
        ...
        kopf = ["%d Python-Dateien gelesen" % dateien, "%d schreibende Zugriffe" % len(befunde)]
        if eingerichtet:
            kopf.append("Wurzeln: %s" % ", ".join(self.einstellung().get("wurzeln") or ()))
        else:
            kopf.append('nicht eingerichtet — `DJANGOBASE["daten_nur_lesen"]["wurzeln"]` setzen')
        ...
        for knoten in ast.walk(baum):
            ...
            raus.append(
                Befund(
                    ...
                    "Zielpfad liegt unter einem Verzeichnis, das als nur lesbar erklaert ist.",
                    Befund.FEHLER,
                )
            )
        return raus
