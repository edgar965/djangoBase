# -*- coding: utf-8 -*-
"""Dateigroesse - Dateien und Funktionen, die zu groß geworden sind.
...
class Dateigroesse(Werkzeug):
    ...
    zweck = (
        "Dateien über 300 Zeilen, Klassen über 300 und Funktionen über 60 — "
        "je mit der Zahl der Zuständigkeiten, die drinstecken."
    )
    befund = (
        "Eine views.py mit 12.223 Zeilen und zwei Browser-Module mit 478 "
        "und 427. Beim Aufteilen fielen mehrere Fehler auf, die vorher "
        "niemand sehen konnte."
    )
    abhilfe = (
        "Nach Zuständigkeiten schneiden, nicht nach Zeilenzahl. Vor und "
    ...
    def laufen(self):
        ...
        for d in self.dateien():
            if d.codezeilen > self.GRENZE_DATEI:
                zeilen.append(
                    self._zeile(d, 1, "Datei", d.pfad.name, d.codezeilen, d.zeilen, self.GRENZE_DATEI)
                )
            ...
            for k in d.knoten(ast.ClassDef):
                ...
                if n > self.GRENZE_KLASSE:
                    zeilen.append(self._zeile(d, k.lineno, "Klasse", k.name, n, ganz, self.GRENZE_KLASSE))
                elif ganz > self.GRENZE_KLASSE:
                    doku += 1
            for k in d.knoten(ast.FunctionDef, ast.AsyncFunctionDef):
                n, ganz = self._laenge(d, k)
                if n > self.GRENZE_FUNKTION:
                    zeilen.append(self._zeile(d, k.lineno, "Funktion", k.name, n, ganz, self.GRENZE_FUNKTION))
        ...
        return Ergebnis(
            ["datei", "zeile", "art", "name", "code", "gesamt", "grenze"],
            zeilen,
            kopf,
    ...
    def _laenge(datei, knoten):
        """``(Code-Zeilen, Zeilen gesamt)`` eines Knotens."""
        ende = getattr(knoten, "end_lineno", None) or knoten.lineno
        return (datei.codezeilen_zwischen(knoten.lineno, ende), ende - knoten.lineno + 1)
