...
class Dateimass:
    ...
    __slots__ = ("pfad", "zeilen", "klassen", "funktionen", "laengste", "laengste_name")

    def __init__(self, pfad, zeilen, klassen=0, funktionen=0, laengste=0, laengste_name=""):
        self.pfad = pfad
        self.zeilen = zeilen
...
class Grossdateien(BefundWerkzeug):
    ...
    zweck = (
        "Listet Python- und JavaScript-Dateien über der Zeilengrenze, mit "
        "Anzahl Klassen und Funktionen und der laengsten Funktion je Datei."
    )
    abhilfe = (
        "Zu Beginn eines Umbaus, um die Schnittkandidaten zu finden — und "
        "danach als Nachweis, dass keine neue Monsterdatei entstanden ist."
    )
    befund = (
        "Ausgangslage im Ursprungsprojekt: eine Datei mit 6.495 Zeilen und "
    ...
    def pruefen(self, grenze="300", **_argumente):
        ...
        ueber = sorted((m for m in masse if m.zeilen > schwelle), key=lambda m: -m.zeilen)
        befunde = []
        for mass in ueber:
            gewicht = (
                Befund.FEHLER
                if mass.zeilen > schwelle * 3
                else Befund.WARNUNG
                if mass.zeilen > schwelle * 1.5
                else Befund.HINWEIS
            )
            befunde.append(Befund(mass.pfad, mass.beschreibung(), gewicht=gewicht))
    ...
    def _python(self, datei):
        ...
        return Dateimass(self.kurz(datei), zeilen, klassen, funktionen, laengste, laengste_name)
    ...
    def _javascript(self, datei):
        ...
        funktionen = len(
            re.findall(
                r"^\s*(?:export\s+)?(?:async\s+)?function\s+\w+|^\s*\w+\s*\([^)]*\)\s*\{", quelle, re.M
            )
        )
