# -*- coding: utf-8 -*-
"""Seitenwurzeln — wie viele Objekte erzeugt eine Vorlage selbst?
...
class Seite:
    """Eine Vorlage und das, was sie selbst erzeugt."""
...
class Seitenwurzeln(BefundWerkzeug):
    ...
    zweck = (
        "Zählt, wie viele Objekte eine HTML-Vorlage selbst erzeugt. "
        "Idealwert: EINE Seiten-Klasse, die den Rest hält."
    )
    abhilfe = (
        "Eine Klasse je Seite anlegen, die die Teile im Konstruktor "
        "hält — dann steht die Reihenfolge dort statt in einem "
        "`setTimeout`. Die Vorlage enthält danach eine Zeile."
    )
    befund = (
        "CamTrack: `live_view.html` erzeugt neun Objekte nebeneinander "
    ...
    AUS_FENSTER = re.compile(r"(?:const|let|var)\s+(\w+)\s*=\s*window\.\w+")
    ...
    TEILE_AUS = ("_teil_", "_list_content", "_person_card", "_sidebar", "_live_calendar_pane")
    ...
    def pruefen(self, ab="1", **_argumente):
        ...
        for pfad in self.projektdateien(".html"):
            ...
            if not wurzeln:
                continue
            seiten.append(Seite(self.kurz(pfad), wurzeln, len(self.WARTEN.findall(text))))

        seiten.sort(key=lambda s: (s.gewicht != Befund.WARNUNG, -len(s.wurzeln), s.pfad))
        gesamt = sum(len(s.wurzeln) for s in seiten)
        kopf = [
            "%d Vorlagen geprüft, %d erzeugen selbst Objekte" % (vorlagen, len(seiten)),
        ...
        befunde = [self._befund(s) for s in seiten if len(s.wurzeln) > grenze]
        return Befundsatz(self.titel, kopf, befunde)

    def _wurzeln(self, text: str) -> list:
        """``[(name, zeile)]`` — was diese Vorlage selbst erzeugt."""
        raus = []
        ...
        for treffer in self.NEU.finditer(text):
            if treffer.group(1) in self.FREMD or treffer.group(1) in geliehen:
                continue
            raus.append((treffer.group(1), text.count("\n", 0, treffer.start()) + 1))
    ...
    def _befund(seite: Seite) -> Befund:
        ...
        if seite.wartezeiten:
            warum = (
                "Dazu %d mal `setTimeout` — die Reihenfolge zwischen "
                "ihnen steht nirgends und wird mit Wartezeiten geraten. "
                "Eine Seiten-Klasse hält sie im Konstruktor." % seite.wartezeiten
            )
        else:
            warum = (
                "Jedes davon ist eine eigene Wurzel. Eine Seiten-Klasse "
