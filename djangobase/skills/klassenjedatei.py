...
class KlassenJeDatei(BefundWerkzeug):
    ...
    zweck = (
        "Listet Dateien mit mehr als einer Klasse — mit Zeilenzahl je "
        "Klasse, damit man sieht, ob es sich um eine Sammlung oder um eine "
        "Hauptklasse mit kleinen Helfern handelt."
    )
    abhilfe = (
        'Wenn die Regel "eine Klasse je Datei" gilt. Kleine Datentraeger '
    ...
    befund = (
        "Im Ursprungsprojekt lagen 110 Endpunkte und ein Dutzend Klassen "
    ...
    def pruefen(self, ab="2", **_argumente):
        ...
        for datei in self.projektdateien(".py"):
            ...
            if len(grosse) <= 1:
                warum = (
                    "nur %d davon größer als %d Zeilen — das sind "
                    "Datentraeger bei ihrer Klasse, kein Verstoß" % (len(grosse), self.KLEIN)
                )
                ...
                warum = "%d eigenstaendige Klassen in einer Datei — trennen" % len(grosse)
        ...
        kopf = [
            "%d Klassen insgesamt" % gesamt,
            "%d Dateien mit mindestens %d Klassen — davon %d mit mehr als "
            "EINER eigenstaendigen Klasse (die Verstöße)" % (len(befunde), grenze, verstoesse),
        ]
        return Befundsatz(self.titel, kopf, befunde)

    def _gehoeren_zusammen(self, datei):
        """Warum diese Klassen in EINER Datei stehen duerfen - oder ``''``."""
        kurz = "/" + self.kurz(datei)
        for muster in self.ZUSAMMEN_MUSTER:
            if muster in kurz:
                return (
                    "%s — hier verlangt das Framework bzw. die Aufgabe "
                    "mehrere Klassen, kein Verstoß" % muster.strip("/")
                )
        n = Quelldatei(datei, self.wurzel()).codezeilen
        if n <= self.ZUSAMMEN_BIS:
            return "nur %d Code-Zeilen — unter %d ist die Übersicht ohnehin da, Aufteilen schadet" % (
                n,
                self.ZUSAMMEN_BIS,
            )
        return ""
