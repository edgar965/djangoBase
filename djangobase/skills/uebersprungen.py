# -*- coding: utf-8 -*-
"""Übersprungen — jede Prüfung, die grün meldet, ohne geprüft zu haben.
...
class Sprungstelle:
    """Eine Stelle, an der ein Lauf abgekürzt wird."""
...
class Sprungsucher(ast.NodeVisitor):
    """Findet alle Sprungstellen in EINEM Prüf-Modul."""
    ...
    def _liest_umgebung(knoten):
        """Hängt die Bedingung an einer Umgebungsvariablen OHNE Vorgabe?
    ...
    @staticmethod
    def _text(knoten):
        """Der erste Zeichenketten-Wert im Aufruf — meist der Grund."""
        for teil in ast.walk(knoten):
    ...
    def _dekoratoren(self, knoten):
        for deko in knoten.decorator_list:
            ...
            self.stellen.append(
                Sprungstelle(
                    self.datei, deko.lineno, art, ".".join(self._wobei + [knoten.name]), self._text(deko)
                )
            )
    ...
    def visit_Call(self, knoten):
        ...
        if self._name(knoten.func) in RUMPFAUFRUFE:
            ...
            passt = self._name(knoten.func) == "skipTest" or (
                isinstance(eigner, ast.Name) and eigner.id in ("pytest", "unittest")
            )
            if passt:
                self.stellen.append(
                    Sprungstelle(
                        self.datei, knoten.lineno, "rumpf", ".".join(self._wobei) or "?", self._text(knoten)
                    )
                )
        self.generic_visit(knoten)

    def _versteckter_waechter(self, knoten):
        """``unittest.skipUnless(...)`` als RUECKGABEWERT einer Hilfsfunktion.
        ...
        if (knoten.lineno, knoten.col_offset) in self._gemeldet:
            return
        eigner = getattr(knoten.func, "value", None)
        if not (isinstance(eigner, ast.Name) and eigner.id in ("unittest", "pytest")):
            return
        self.stellen.append(
            Sprungstelle(
                self.datei, knoten.lineno, "versteckt", ".".join(self._wobei) or "?", self._text(knoten)
            )
        )
...
class Uebersprungen(BefundWerkzeug):
    ...
    kriterium = 17
    slug = "uebersprungen"
    titel = "Übersprungene Prüfungen"
    zweck = (
        "Findet jede Stelle, an der eine Prüfung abgekürzt wird — "
        "Dekorator, Umgebungsschalter oder skipTest im Rumpf. Ein "
        "übersprungener Test meldet grün, ohne geprüft zu haben."
    )
    abhilfe = (
        "Vor jedem Release und nach jedem Umbau an den Prüfungen. "
        "Ein Schalter, den niemand setzt, ist eine Abschaltung."
    )
    befund = (
        "Im Ursprungsprojekt sechs Prüfungen hinter EINER "
        "Umgebungsvariablen: fünf liefen mit Schalter in 21 s durch "
        "und bestanden, die sechste konnte gar nicht laufen — sie "
        "fragte die leere Testdatenbank nach Aufnahmen."
    )
    ...
    anlassfall = Anlassfall(
        ...
        mindestens=2,
        erwartet_in="ECHT",
        warum="Zwei Stellen in einer Datei — ein Umgebungsschalter auf der "
        "Klasse und ein skipTest im Rumpf; die zweite Datei ist "
        "sauber und darf nicht mitgemeldet werden",
    )
    ...
    def _ist_pruefcode(kurz):
        ...
        name = teile[-1]
        return any(t in PRUEFORTE for t in teile[:-1]) or any(m in name for m in PRUEFDATEI)

    ART_TEXT = {
        "umgebung": "Umgebungsschalter",
        "bedingt": "Bedingung",
        "dauerhaft": "dauerhaft stillgelegt",
        "rumpf": "skipTest im Rumpf",
    }

    def _befund(self, stelle):
        was = "%s: %s" % (self.ART_TEXT.get(stelle.art, stelle.art), stelle.wobei)
        warum = stelle.grund or "(kein Grund angegeben)"
        if stelle.art == "umgebung":
            warum += (
                " — läuft im Alltag NIE. Vor dem Entfernen einmal "
                "mit gesetztem Schalter fahren: Besteht die Prüfung, "
                "gehört der Schalter weg; überspringt sie sich "
                "weiterhin, kann sie gar nicht laufen."
            )
        elif stelle.art == "rumpf":
            warum += (
                ' — „nichts zu prüfen" ist ein ERGEBNIS und lässt '
                "sich zusichern: `assertTrue(nichts_da or bedingung)` "
                "läuft immer und fällt auf, wenn der Sucher bricht."
            )
