...
class Endpunktprofil(BefundWerkzeug):
    ...
    zweck = (
        "Profiliert eine einzelne Route mit cProfile und zeigt beide "
        "Sichten: tottime (wo gerechnet wird) und cumulative (wer es "
        "veranlasst)."
    )
    abhilfe = (
        "Sobald die Endpunkt-Zeiten einen Ausreisser zeigen. Beide Listen "
        "sind nötig — die eigene Zeit allein verraet nicht, warum eine "
        "Funktion 40.000-mal läuft."
    )
    befund = (
        "So kamen die größten Funde zustande: eine Doppelschleife mit "
    ...
    def pruefen(self, weg="/", **_argumente):
        ...
        kopf = ["%s -> Status %s, %d Byte" % (ziel, antwort.status_code, len(antwort.content))]
        befunde = []
        for sortierung, erklaerung in (
            ...
            befunde.append(Befund("— %s —" % sortierung, erklaerung, gewicht=Befund.HINWEIS))
            befunde.extend(self._zeilen(profil, sortierung))
        return Befundsatz(self.titel, kopf, befunde)

    def _zeilen(self, profil, sortierung):
        puffer = io.StringIO()
        pstats.Stats(profil, stream=puffer).sort_stats(sortierung).print_stats(self.ZEILEN)
        befunde = []
